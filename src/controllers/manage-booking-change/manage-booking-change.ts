...
export class ManageBookingChangeController {
  ...
  public get = async (req: Request, res: Response): Promise<void> => {
    ...
    const voiceover = TestVoiceover.fromCRMVoiceover(
      booking.details.voiceoverLanguage
    );
    const voiceoverRequested = voiceover !== Voiceover.NONE;

    const bslRequested =
      booking.details.additionalSupport ===
      CRMAdditionalSupport.BritishSignLanguage;

    let testLanguage = null;
    try {
      testLanguage = TestLanguage.fromCRMTestLanguage(
        booking.details.testLanguage
      );
    } catch (error) {
      logger.error(
        error,
        `Test Language was null in booking product ${booking.details.reference}`
      );
      logger.event(
    ...
    const testType = fromCRMProductNumber(
      booking.details?.product?.productnumber as CRMProductNumber
    );

    const viewData = {
      booking: booking.details,
      canChangeLanguage: TestLanguage.canChangeTestLanguage(
    ...
    return res.render("manage-booking/change", {
      ...
      hasSupportedBeenRequested: !mapToSupportType(
        booking.details.testSupportNeed
      )[0].includes(TestSupportNeed.NoSupport),
    });
  };
}

export default new ManageBookingChangeController(CRMGateway.getInstance());

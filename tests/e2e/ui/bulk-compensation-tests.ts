...
import { runningTestsLocally, setAcceptCookies } from "../utils/helpers";
import {
  Locale,
  Origin,
  PreferredDay,
...
test("SA Candidate OBP booking - GB - Email", async () => {
  ...
  sessionData.currentBooking.centre.accountId =
    "f3f5a7c8-937d-ea11-a811-00224801bc51";
  ...
  console.log(
    `${bookingRef} ${drivingLicence} ${sessionData.currentBooking.dateTime}`
  );
});
...
test("SA Candidate OBP booking - NI - Email", async () => {
  ...
  sessionData.currentBooking.centre.accountId =
    "b1f6a7c8-937d-ea11-a811-00224801bc51";
  sessionData.currentBooking.origin = Origin.CitizenPortal;
  sessionData.candidate.email = "a.hussein@kainos.com";
...
test("NSA Candidate OBP booking - GB", async () => {
  ...
  console.log(
    `${bookingRef} ${licenceNumber} ${sessionData.currentBooking.dateTime}`
  );
});

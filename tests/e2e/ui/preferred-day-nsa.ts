...
test("Verify page title, heading and content are displayed correctly", async () => {
  ...
  await verifyExactText(
    preferredDayNSAPage.charCountHintText,
    preferredDayNSAPage.characterCountText
  );
  await t
    .expect(
      Selector(preferredDayNSAPage.getRadioSelector(PreferredDay.DecideLater))
        .checked
    )
    .notOk();
  await t
    .expect(
      Selector(preferredDayNSAPage.getRadioSelector(PreferredDay.ParticularDay))
        .checked
    )
    .notOk();
});
...
test("Verify user can proceed by selecting 'I want a particular day' and without entering any preferred day details", async () => {
  await preferredDayNSAPage.selectPreferredDayOption(
...
test("Verify user can proceed by selecting 'I want a particular day' and entering some valid preferred day details", async () => {
  ...
  await preferredDayNSAPage.enterPreferredDayInformation(
    "I prefer Saturdays only"
  );
  await click(preferredDayNSAPage.continueButton);
  await verifyTitleContainsText(
    `${preferredLocationNSAPage.pageHeading} ${generalTitle}`
...
test("Verify a validation error message is displayed when not selecting any option", async () => {
  ...
  await verifyExactText(
    preferredDayNSAPage.errorMessageLocator,
    preferredDayNSAPage.errorMessageHeader
  );
  await verifyExactText(
    preferredDayNSAPage.errorMessageList,
    preferredDayNSAPage.errorMessageOptionNotSelectedText
  );
  await verifyContainsText(
    preferredDayNSAPage.errorMessageRadioLocator,
    preferredDayNSAPage.errorMessageOptionNotSelectedText
  );
...
test("Verify a validation error message is displayed when entering more than 4000 characters and the text is retained", async () => {
  ...
  await verifyExactText(
    preferredDayNSAPage.errorMessageList,
    preferredDayNSAPage.errorMessageTooManyCharsText
  );
  await verifyContainsText(
    preferredDayNSAPage.errorMessageTextAreaLocator,
    preferredDayNSAPage.errorMessageTooManyCharsText
  );

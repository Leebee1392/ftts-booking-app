...
export const toCRMOrigin = (origin: Origin): CRMOrigin =>
  ORIGIN_TO_CRMORIGIN.get(origin) as CRMOrigin;
...
export function mapToCRMBooking(
  ...
  return {
    ...
    "ftts_selectedtestcentrelocation@odata.bind": `accounts(${
...
function mapNsaAttributesToBooking(
  ...
  return {
    ...
    ftts_nsastatus: isStandardAccommodation
      ? undefined
...
export function mapCRMLicenceToLicenceResponse(
  crmLicence: CRMLicenceResponse[]
): LicenceResponse | undefined {
  if (crmLicence.length === 0) {
    logger.info(
      "CRM-HELPER::mapCRMLicenceToLicenceResponse: Candidate was not found in CRM"
    );
    return undefined;
  }
  return {
...
export function mapCRMBookingResponseToBookingResponse(
  ...
  if (!crmBookingResponse.ftts_reference) {
    throw new Error(
      "CRM-HELPER::mapCRMBookingResponseToBookingResponse: Missing ftts_reference in crm response"
    );
  }
  return {
...
export function mapToBookingDetails(
  ...
  if (
    !crmBookingDetails.ftts_bookingid.ftts_reference ||
    !crmBookingDetails.ftts_bookingstatus ||
    !crmBookingDetails.ftts_testdate ||
    !crmBookingDetails.ftts_productid ||
    !crmBookingDetails.ftts_testlanguage ||
    !crmBookingDetails.ftts_voiceoverlanguage ||
    !crmBookingDetails.ftts_salesreference ||
    !crmBookingDetails.ftts_bookingid.ftts_origin ||
    crmBookingDetails.ftts_additionalsupportoptions === undefined ||
    crmBookingDetails.ftts_bookingid.ftts_governmentagency === null
  ) {
    throw new Error(
  ...
  if (crmBookingDetails.ftts_bookingid.ftts_testcentre) {
    if (
      !crmBookingDetails.ftts_bookingid.ftts_testcentre.ftts_remit ||
      !crmBookingDetails.ftts_bookingid.ftts_testcentre.ftts_siteid
    ) {
      throw new Error(
        "CRMHelper::mapToBookingDetails: Missing expected fields in crm test centre response"
      );
    }
    booking.testCentre = {
      testCentreId:

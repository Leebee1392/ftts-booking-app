...
const setupQueueItKnownUser = (
  ...
  try {
    ...
    if (result.doRedirect()) {
      logger.debug(
        "setupQueueItKnownUser:: Redirecting Candidate to the QueueIt Service",
    ...
    if (
      ...
      logger.debug(
        "setupQueueItKnownUser:: Removing Queue It Token and Redirecting",
        { requestedUrl: requestUrlWithoutToken }
      );
    ...
    logger.debug(
      "setupQueueItKnownUser:: Request does not contain Queue It token and can continue",
      { requestedUrl: requestUrlWithoutToken }
    );
    next();
  } catch (err) {
 ...
 */
const setupQueueItJSImplementation = (
  req: Request,
  res: Response,
  next: NextFunction
): void => {

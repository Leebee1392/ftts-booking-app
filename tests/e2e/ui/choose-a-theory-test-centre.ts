...
test("Verify that 5 test centre results are shown initially", async (t) => {
  ...
  const testCentreCount = await createSelector(
    chooseATestCentrePage.testCentres
  ).count;
...
test("Verify that the entered search term is displayed", async () => {
  await verifyExactText(
    chooseATestCentrePage.searchQueryValue,
...
test("Verify that distance in Miles is shown by default in ascending order", async (t) => {
  const centreDistances: Selector = createSelector(
    chooseATestCentrePage.centreDistance
  );
  const centreDistancesCount = await centreDistances.count;
  await t.expect(centreDistancesCount).eql(5);

  await checkAscendingOrderByDistance(
    t,
    chooseATestCentrePage.centreDistance,
    centreDistancesCount,
    "miles"
  );
});
...
test("Verify after clicking the 'See kilometres' link it shows the 'See miles' link", async () => {
  await verifyExactText(
    chooseATestCentrePage.changeUnitsLink,
    chooseATestCentrePage.seeKilometresLinkText
  );
  ...
  await verifyExactText(
    chooseATestCentrePage.changeUnitsLink,
    chooseATestCentrePage.seeMilesLinkText
  );
});
...
test("Verify the 'See 5 more' button is hidden when we've reached 50 test centres", async (t) => {
  ...
  for (let i = 0; i < 9; i++) {
    ...
    await chooseATestCentrePage.see5MoreTestCentres();
    await verifyExactText(
      chooseATestCentrePage.testCentreCount,
      expectedNumberOfTestCentres.toString()
    );
    const testCentreCount = await createSelector(
      chooseATestCentrePage.testCentres
    ).count;
...
test("Verify the 'See 5 more' button is hidden when there are no more test centres to show (less than 50)", async (t) => {
  ...
  await chooseATestCentrePage.see5MoreTestCentres();
  await chooseATestCentrePage.see5MoreTestCentres();
  const testCentreCount = parseInt(
    await getText(chooseATestCentrePage.testCentreCount),
    10
  );
...
test("Verify that after viewing more test centres then changing the search term, resets the results to 5", async (t) => {
  await chooseATestCentrePage.see5MoreTestCentres();
  let testCentreCount = await createSelector(chooseATestCentrePage.testCentres)
    .count;
  await t.expect(testCentreCount).eql(10);
  ...
  await verifyExactText(chooseATestCentrePage.testCentreCount, "5");
  testCentreCount = await createSelector(chooseATestCentrePage.testCentres)
    .count;
  await t.expect(testCentreCount).eql(5);
});

test("Verify the 'View on Google Maps' link is constructed correctly", async () => {
  const centreAddressLine1 = await getText(
    chooseATestCentrePage.centreAddressLine1
  );
  const centrePostalCode = await getText(
    chooseATestCentrePage.centreAddressPostalCode
  );

  await clickLinkContainsURL(
    chooseATestCentrePage.viewOnGoogleMapsLinkText,
...
test("Verify address details are shown for each test centre", async (t) => {
  ...
  await t
    .expect(foundAddressLine2)
    .ok("Did not find at least one Address Line 2!");
});
...
test("Verify the 'About this test centre' shows additional information for each test centre", async (t) => {
  for (let i = 0; i < 5; i++) {
    ...
    await verifyContainsText(
      chooseATestCentrePage.centreAdditionalInfoHeading,
      "Description:",
      i
    );
    await t
      .expect(await getText(chooseATestCentrePage.centreAdditionalInfoText, i))
      .notEql("");
  }
});
...
test("Verify that 2 tabs are visible when JavaScript is enabled: 'List of centres' and 'See on map'", async () => {
  ...
  await verifyExactText(
    chooseATestCentrePage.listOfCentresTab,
    chooseATestCentrePage.listOfTestCentresTabText
  );

  await verifyIsVisible(chooseATestCentrePage.mapOfCentresTab);
  await verifyExactText(
    chooseATestCentrePage.mapOfCentresTab,
    chooseATestCentrePage.seeOnMapTabText
  );
});
...
test("Verify when no test centres have been selected on the 'See on map' tab, it displays a hint text", async () => {
  ...
  await verifyExactText(
    chooseATestCentrePage.mapSelectHint,
    chooseATestCentrePage.mapSelectHintText
  );
});
...
test("Verify the 'See on map' tab tab has rendered the map with the correct number of markers as the 'List of centres' tab", async (t) => {
  ...
  if (t.browser.name.toLowerCase() !== "internet explorer") {
    ...
    let testCentreMapMarkers = createSelector(
      chooseATestCentrePage.getMapMarkersSelector()
    );
    ...
    await click(chooseATestCentrePage.mapOfCentresTab);
    await verifyIsVisible(chooseATestCentrePage.googleMap);
    testCentreMapMarkers = createSelector(
      chooseATestCentrePage.getMapMarkersSelector()
    );
...
test("Verify clicking on the map markers in the map tab shows the test centre details correctly", async (t) => {
  // Google Maps does not work in IE11 due to lack of JavaScript support
  if (
    t.browser.name.toLowerCase() !== "firefox" &&
    t.browser.name.toLowerCase() !== "internet explorer"
  ) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      await verifyIsVisible(
        chooseATestCentrePage.testCentreInfoCallout,
        0,
        10000
      );
      const testCentreAddress = await getText(
        chooseATestCentrePage.testCentreInfoCallout
      );

      let hasAddressLine2 = false;
      const selectedCentreName = await getText(
        chooseATestCentrePage.mapSelectedCentreName
      );
      const selectedCentreAddressLine1 = await getText(
        chooseATestCentrePage.mapSelectedCentreAddressLine1
      );
      let selectedCentreAddressLine2;
      if (
        await isVisible(chooseATestCentrePage.mapSelectedCentreAddressLine2)
      ) {
        hasAddressLine2 = true;
        selectedCentreAddressLine2 = await getText(
          chooseATestCentrePage.mapSelectedCentreAddressLine2
        );
      }
      const selectedCentreAddressCity = await getText(
        chooseATestCentrePage.mapSelectedCentreAddressCity
      );
      const selectedCentrePostcode = await getText(
        chooseATestCentrePage.mapSelectedCentreAddressPostalCode
      );
...
test("Verify the 'View on Google Maps' link on a selected test centre is constructed correctly", async (t) => {
  ...
  if (
    ...
    await clickLinkContainsURL(
      chooseATestCentrePage.viewOnGoogleMapsLinkText,
      selectedCentreAddressLine1
    );
    await clickLinkContainsURL(
      chooseATestCentrePage.viewOnGoogleMapsLinkText,
      selectedCentrePostcode
    );
  } else {
    console.log("Test skipped for Firefox & IE11");
  }
...
test("Verify you can select a test centre from the map tab to proceed with the booking", async (t) => {
  ...
  if (
    ...
    const preferredDatePage: PreferredDatePage =
      await chooseATestCentrePage.selectATestCentreFromMapTab();
    await verifyTitleContainsText(
      `${preferredDatePage.pageHeading} ${generalTitle}`
    );
    await verifyExactText(
...
async function checkAscendingOrderByDistance(
  t: TestController,
  centreDistances: string,
  numOfTestCentres: number,
  unit: string
) {

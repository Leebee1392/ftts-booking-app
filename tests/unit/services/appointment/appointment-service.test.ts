...
import { ChooseAppointmentBody } from "@controllers/choose-appointment/choose-appointment";
...
import {
  AppointmentSlot,
  KPIIdentifiers,
} from "../../../../src/services/scheduling/types";
...
describe("Appointment service tests", () => {
  ...
  describe("getSlots", () => {
    ...
    describe("no appointment slots returned for the week", () => {
      test("No slots returned", async () => {
        ...
        MockDate.set(globalDate);
        [selectedCentre] = mockCentres;
        const eligibility = req.session.candidate.eligibilities.find(
          (elig) => elig.testType === TestType.CAR
        );

        const { slotsByDate } = await appointmentService.getSlots(
          selectedDate,
          selectedCentre,
          TestType.CAR,
          eligibility
    ...
    describe("test for kpiIdentifiers", () => {
      ...
      beforeEach(async () => {
        ...
        mockSchedulingGateway.availableSlots.mockResolvedValue([
          {
            ...
            dateAvailableOnOrAfterToday:
              "dummyDateAvailableOnOrAfterAfterToday",
            dateAvailableOnOrBeforePreferredDate:
              "dummydateAvailableOnOrBeforePreferredDate",
            dateAvailableOnOrAfterPreferredDate:
              "dummyDateAvailableOnOrAfterPreferredDate",
          },
          {
            testCentreId: "0001:SITE-0207",
            testTypes: ["CAR"],
        ...
        const { slotsByDate, kpiIdentifiers } =
          await appointmentService.getSlots(
            selectedDate,
            selectedCentre,
            TestType.CAR,
            eligibility,
    ...
    describe("morning and evening slot tests", () => {
      test("morning slot", async () => {
        ...
        expect(
          slotsByDate[toISODateString("2021-06-28T09:00:00.000Z")]
        ).toEqual([
          {
            quantity: 1,
            startDateTime: "2021-06-28T09:00:00.000Z",
      ...
      test("afternoon slot", async () => {
        ...
        expect(
          slotsByDate[toISODateString("2021-06-29T13:00:00.000Z")]
        ).toEqual([
          {
            quantity: 1,
            startDateTime: "2021-06-29T13:00:00.000Z",
  ...
  describe("getWeekViewDatesMobile", () => {
    test("When selected date is not at the end or start of the week", () => {
      ...
      expect(
        appointmentService.getWeekViewDatesMobile(
          selectedDate,
          startOfWeek,
          endOfWeek
        )
      ).toEqual(["2020-11-10", "2020-11-11", "2020-11-12"]);
    });
    ...
    test("When selected date is at the start of the week", () => {
      ...
      ).toEqual(["2020-11-09", "2020-11-10", "2020-11-11"]);
    });
    ...
    test("When selected date is at the end of the week", () => {
      ...
      ).toEqual(["2020-11-13", "2020-11-14", "2020-11-15"]);
    });
  });
  ...
  describe("getWeekViewDatesDesktop", () => {
    ...
    test("Returns a string array of dates for the given week when selectedDate is not at the start or end of week", () => {
      const selectedDate = dayjs().tz();
      expect(appointmentService.getWeekViewDatesDesktop(selectedDate)).toEqual(
        expectedReturnDates
      );
    });

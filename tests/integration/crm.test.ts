...
import { Target, SupportType, TestType } from "../../src/domain/enums";
...
describe("Booking app -> CRM integration tests", () => {
  ...
  beforeAll(async () => {
    ...
    await crmGateway.createLicence(
      mockLicenceNumber,
      candidateData.address as ELIG.Address,
      contact.contactid
    );
  });
  ...
  describe("getLicenceNumberRecordsByCandidateId", () => {
    ...
    beforeEach(async () => {
      mockCandidates = [
        await crmTestClient.createNewCandidate(mockLicenceNumber, {
          telephone2: "123",
        ...
        await crmTestClient.createNewCandidate(mockLicenceNumber, {
          telephone2: "456",
          ftts_title: CRMPeopleTitle.MS,
          gendercode: CRMGenderCode.Female,
        }),
        await crmTestClient.createNewCandidate(mockLicenceNumber, {
          telephone2: "789",
          ftts_title: CRMPeopleTitle.DR,
          gendercode: CRMGenderCode.Unknown,
        }),
      ];
    ...
    test("retrieves the correct existing candidate and licence record given a candidate ID and licence number", async () => {
      const result = await crmGateway.getLicenceNumberRecordsByCandidateId(
        mockCandidates[0].candidateId,
        mockLicenceNumber
      );

      const returnedCandidate = await crmTestClient.retrieveCandidate(
        mockCandidates[0].candidateId
      );

      expect(result?.candidateId).toEqual(mockCandidates[0].candidateId);
      expect(result?.licenceNumber).toEqual(mockLicenceNumber);
      expect(result?.personReference).toEqual(
        returnedCandidate.ftts_personreference
      );
      expect(result?.telephone).toEqual(returnedCandidate.telephone2);
      expect(mapPeopleTitleStringToCRMPeopleTitle(result?.title)).toEqual(
        returnedCandidate.ftts_title
      );
      expect(mapGenderEnumToCRMGenderCode(result?.gender)).toEqual(
        returnedCandidate.gendercode
      );
    ...
    test("returns undefined if licence doesn't already exist", async () => {
      ...
      const mockCandidate = await crmTestClient.createNewCandidate(
        mockCorrectLicenceNumber
      );

      const result = await crmGateway.getLicenceNumberRecordsByCandidateId(
        mockCandidate.candidateId,
        mockIncorrectLicenceNumber
      );
  ...
  describe("createLicence", () => {
    test("creates a licence record for a given candidate", async () => {
      ...
      const result = await crmGateway.createLicence(
        mockLicenceNumber,
        candidateData.address as ELIG.Address,
        contact.contactid
      );
      ...
      expect(newLicence.ftts_address1_street1).toBe(
        candidateData.address?.line1
      );
      expect(newLicence.ftts_address1_street2).toBe(
        candidateData.address?.line2
      );
      expect(newLicence.ftts_address1street3).toBe(
        candidateData.address?.line3
      );
      expect(newLicence.ftts_address1street4).toBe(
        candidateData.address?.line4
      );
      expect(newLicence.ftts_address1_city).toBe(candidateData.address?.line5);
      expect(newLicence.ftts_address1_postalcode).toBe(
        candidateData.address?.postcode
      );
  ...
  describe("createCandidate", () => {
    test("creates a candidate record", async () => {
      ...
      expect(candidate.ftts_title).toStrictEqual(
        mapPeopleTitleStringToCRMPeopleTitle(candidateData.title)
      );
      expect(candidate.ftts_firstandmiddlenames).toStrictEqual(
        candidateData.firstnames
      );
      expect(candidate.lastname).toStrictEqual(candidateData.surname);
      expect(candidate.address1_line1).toStrictEqual(
        candidateData.address?.line1
      );
      expect(candidate.address1_line2).toStrictEqual(
        candidateData.address?.line2
      );
      expect(candidate.address1_line3).toStrictEqual(
        candidateData.address?.line3
      );
      expect(candidate.ftts_address1_line4).toStrictEqual(
        candidateData.address?.line4
      );
      expect(candidate.address1_city).toStrictEqual(
        candidateData.address?.line5
      );
      expect(candidate.address1_postalcode).toStrictEqual(
        candidateData.address?.postcode
      );
      expect(candidate.birthdate).toStrictEqual(candidateData.dateOfBirth);
      expect(candidate.gendercode).toStrictEqual(
        mapGenderEnumToCRMGenderCode(candidateData.gender)
      );
      expect(candidate.ftts_personreference).toStrictEqual(
        candidateData.personReference
      );
  ...
  describe("updateCandidate", () => {
    test("successfully updates the candidate details to CRM given a new telephone number and email from the candidate", async () => {
      ...
      await crmGateway.updateCandidate(
        candidateData.candidateId as string,
        updatedCandidateDetails
      );
      const updatedCandidate = await crmTestClient.retrieveCandidate(
        mockCandidate.candidateId
      );

      expect(updatedCandidate.telephone2).toBe(
        updatedCandidateDetails.telephone
      );
      expect(updatedCandidate.emailaddress1).toBe(
        updatedCandidateDetails.email
      );
    });
    ...
    test("successfully updates candidate details including name, address, title and gender", async () => {
      ...
      expect(updatedCandidate.ftts_title).toBe(
        mapPeopleTitleStringToCRMPeopleTitle(updatedCandidateDetails.title)
      );
      expect(updatedCandidate.ftts_firstandmiddlenames).toBe(
        updatedCandidateDetails.firstnames
      );
      expect(updatedCandidate.lastname).toBe(updatedCandidateDetails.surname);
      expect(updatedCandidate.gendercode).toBe(
        mapGenderEnumToCRMGenderCode(updatedCandidateDetails.gender)
      );
      expect(updatedCandidate.address1_line1).toBe(
        updatedCandidateDetails.address?.line1
      );
      expect(updatedCandidate.address1_line2).toBe(
        updatedCandidateDetails.address?.line2
      );
      expect(updatedCandidate.address1_line3).toBe(
        updatedCandidateDetails.address?.line3
      );
      expect(updatedCandidate.ftts_address1_line4).toBe(
        updatedCandidateDetails.address?.line4
      );
      expect(updatedCandidate.address1_city).toBe(
        updatedCandidateDetails.address?.line5
      );
      expect(updatedCandidate.address1_postalcode).toBe(
        updatedCandidateDetails.address?.postcode
      );
    });
    ...
    test("title gets moved to the other title field if it's out of scope", async () => {
      ...
      expect(updatedCandidate.ftts_othertitle).toBe(
        updatedCandidateDetails.title
      );
    });
  });
  ...
  describe("updateCandidateAndCreateBooking", () => {
    test("successfully updates candidate with new telephone number and email address and creates a standard booking", async () => {
      ...
      const priceLists = await crmGateway.getPriceList(Target.GB, [
        bookingData.testType as TestType,
      ]);
      // eslint-disable-next-line prefer-destructuring
      bookingData.priceList = priceLists[0];

      const oldCandidate = await crmTestClient.retrieveCandidate(
        mockCandidate.candidateId
      );

      const updatedCandidateAndBookingResponse =
        await crmGateway.updateCandidateAndCreateBooking(
          candidateData,
          bookingData,
      ...
      const bookingProductId = await crmGateway.createBookingProduct(
        candidateData,
        bookingData,
        updatedCandidateAndBookingResponse.booking,
        true,
        CRMAdditionalSupport.None
      );

      const newCandidate = await crmTestClient.retrieveCandidate(
        updatedCandidateAndBookingResponse.candidate.candidateId as string
      );
      const booking = await crmTestClient.retrieveBooking(
        updatedCandidateAndBookingResponse.booking.id
      );
      const bookingProduct = await crmTestClient.retrieveBookingProduct(
        bookingProductId
      );
      ...
      expect(
        dayjs(booking.ftts_dob).isSame(candidateData.dateOfBirth)
      ).toStrictEqual(true);
      expect(booking.ftts_drivinglicence).toStrictEqual(
        candidateData.licenceNumber
      );
      expect(
        dayjs(booking.ftts_testdate).isSame(bookingData.dateTime)
      ).toStrictEqual(true);
      expect(booking.ftts_reference).toBeTruthy();
      expect(
        TestLanguage.fromCRMTestLanguage(booking.ftts_language)
      ).toBeTruthy();
      expect(booking.ftts_additionalsupportoptions).toStrictEqual(
        CRMAdditionalSupport.None
      );
      expect(booking.ftts_nivoiceoveroptions).toStrictEqual(
      ...
      expect(booking.ftts_selecteddate).toStrictEqual(
        new Date("2020-12-30T10:00:00.000Z")
      );
      expect(booking.ftts_tcnpreferreddate).toStrictEqual(
        new Date("2020-12-30T10:00:00.000Z")
      );
      expect(booking.ftts_dateavailableonoraftertoday).toStrictEqual(
        new Date("2020-12-29T10:00:00.000Z")
      );
      expect(booking.ftts_dateavailableonorbeforepreferreddate).toStrictEqual(
        new Date("2020-12-28T10:00:00.000Z")
      );
      expect(booking.ftts_dateavailableonorafterpreferreddate).toStrictEqual(
        new Date("2020-12-27T10:00:00.000Z")
      );
      expect(booking.ftts_proxypermitted).toStrictEqual(false);

      expect(bookingProduct.ftts_price).toStrictEqual(priceLists[0].price);
      expect(bookingProduct._ftts_productid_value).toStrictEqual(
        priceLists[0].product.productId
      );
      expect(bookingProduct._ftts_testcategoryid_value).toStrictEqual(
        priceLists[0].product.parentId
      );
      expect(bookingProduct.ftts_personalreferencenumber).toStrictEqual(
        "325825"
      );
      expect(bookingProduct.ftts_paymentreferencenumber).toStrictEqual(
        "5134567890123456"
      );
      expect(bookingProduct.ftts_additionalsupportoptions).toStrictEqual(
        CRMAdditionalSupport.None
      );
      ...
      expect(booking.ftts_governmentagency).toStrictEqual(
        CRMGovernmentAgency.Dvsa
      );
      ...
      expect(booking.ftts_preferredcommunicationmethod).toBe(
    ...
    test("successfully updates candidate with new telephone number and email address and creates a non-standard booking", async () => {
      ...
      expect(updatedCandidateAndBookingResponse.candidate.telephone).toBe(
        candidateData.telephone
      );
      expect(updatedCandidateAndBookingResponse.candidate.email).toBe(
        candidateData.email
      );

      expect(booking.ftts_governmentagency).toStrictEqual(
        CRMGovernmentAgency.Dva
      );
      expect(booking.ftts_nonstandardaccommodation).toBeTruthy();
      expect(booking.ftts_nsastatus).toStrictEqual(
        CRMNsaStatus.AwaitingCandidateMedicalEvidence
      );
      expect(booking.ftts_supportrequirements).toBe(
      ...
      expect(booking.ftts_preferreddateandtime).toStrictEqual(
        bookingData.preferredDay
      );
      expect(booking.ftts_preferreddateselected).toBeTruthy();
      expect(booking.ftts_preferredtestcentrelocation).toStrictEqual(
        bookingData.preferredLocation
      );
      expect(booking.ftts_preferredcommunicationmethod).toStrictEqual(
        CRMPreferredCommunicationMethod.Phone
      );
      expect(booking.ftts_proxypermitted).toStrictEqual(false);
      expect(booking.ftts_voicemailmessagespermitted).toBe(
        bookingData.voicemail
      );
    ...
    test("successfully updates candidate with new telephone number and email address and creates a non-standard booking with different test type with nsa status AwaitingCandidateMedicalEvidence", async () => {
      ...
      expect(bookingProduct.ftts_bookingstatus).toStrictEqual(
    ...
    test("successfully updates candidate with new telephone number and email address and creates a non-standard booking with different test type with nsa status AwaitingCSCResponse", async () => {
      ...
      expect(booking.ftts_nonstandardaccommodation).toBeTruthy();
      expect(booking.ftts_nsastatus).toStrictEqual(
        CRMNsaStatus.AwaitingCscResponse
      );
      expect(booking.ftts_supportrequirements).toBeTruthy();
      expect(booking.ftts_additionalsupportoptions).toStrictEqual(
        CRMAdditionalSupport.VoiceoverEnglish
  ...
  describe("updateLicence", () => {
    test("successfully updates licence record with an address and keeping licence number", async () => {
      ...
      await crmGateway.updateLicence(
        mockCandidate.licenceId,
        mockCandidate.candidateId,
        candidateData.address
      );

      const updatedLicenceRecord = await crmTestClient.retrieveLicence(
        mockCandidate.licenceId
      );

      expect(updatedLicenceRecord.ftts_licence).toBe(mockLicence);
      expect(updatedLicenceRecord.ftts_address1_street1).toBe(
        candidateData.address?.line1
      );
      expect(updatedLicenceRecord.ftts_address1_street2).toBe(
        candidateData.address?.line2
      );
      expect(updatedLicenceRecord.ftts_address1street3).toBe(
        candidateData.address?.line3
      );
      expect(updatedLicenceRecord.ftts_address1street4).toBe(
        candidateData.address?.line4
      );
      expect(updatedLicenceRecord.ftts_address1_city).toBe(
        candidateData.address?.line5
      );
      expect(updatedLicenceRecord.ftts_address1_postalcode).toBe(
        candidateData.address?.postcode
      );
    });
  });
  ...
  describe("updateAdditionalSupport", () => {
    test("successfully updates booking and booking product with the bsl option", async () => {
      const mockLicence = `int-test-${uuidv4()}`;
      const mockCandidate = await crmTestClient.createNewCandidate(mockLicence);
      const mockBooking = await crmTestClient.createNewBooking(
        mockCandidate,
        CRMBookingStatus.Confirmed,
        CRMOrigin.CitizenPortal
      );
      await crmGateway.updateAdditionalSupport(
        mockBooking.bookingId,
        mockBooking.bookingProductId,
        CRMAdditionalSupport.BritishSignLanguage
      );

      const updatedBookingProduct = await crmTestClient.retrieveBookingProduct(
        mockBooking.bookingProductId
      );
      const updatedBooking = await crmTestClient.retrieveBooking(
        mockBooking.bookingId
      );

      expect(updatedBookingProduct.ftts_additionalsupportoptions).toBe(
        CRMAdditionalSupport.BritishSignLanguage
      );
      expect(updatedBooking.ftts_additionalsupportoptions).toBe(
        CRMAdditionalSupport.BritishSignLanguage
      );
    });
  });
  ...
  describe("updateVoiceover", () => {
    test("successfully updates booking and booking product with the voiceover option", async () => {
      ...
      await crmGateway.updateVoiceover(
        mockBooking.bookingId,
        mockBooking.bookingProductId,
        CRMVoiceOver.English
      );
      ...
      expect(updatedBookingProduct.ftts_voiceoverlanguage).toBe(
        CRMVoiceOver.English
      );
  ...
  describe("updateBookingStatus", () => {
    test("successfully updates booking status to confirmed", async () => {
      ...
      await crmGateway.updateBookingStatus(
        mockBooking.bookingId,
        CRMBookingStatus.Confirmed,
        false
      );
      ...
      expect(updatedBookingProduct.ftts_bookingstatus).toBe(
        CRMBookingStatus.Confirmed
      );
      expect(updatedBooking.ftts_bookingstatus).toBe(
        CRMBookingStatus.Confirmed
      );
  ...
  describe("markBookingCancelled", () => {
    test("successfully updates booking status to cancelled and cancel date to todays date", async () => {
      ...
      await crmGateway.markBookingCancelled(
        mockBooking.bookingId,
        mockBooking.bookingProductId
      );
      ...
      expect(
        new Date(updatedBookingProduct.ftts_canceldate).toISOString()
      ).toBe(mockToday);
      expect(updatedBooking.ftts_bookingstatus).toBe(
        CRMBookingStatus.Cancelled
      );
  ...
  describe("updateTCNUpdateDate", () => {
    test("successfully updates TCN Update Date", async () => {
      ...
      expect(
        new Date(updatedBookingProduct.ftts_tcn_update_date).toISOString()
      ).toBe(mockToday);
    });
  });
  ...
  describe("createBindBetweenBookingAndPayment", () => {
    test("GIVEN a booking AND a payment WHEN called THEN the booking is binded to the payment", async () => {
      const mockLicence = `int-test-${uuidv4()}`;
      const mockCandidate = await crmTestClient.createNewCandidate(mockLicence);
      const mockBooking = await crmTestClient.createNewBookingWithPayment(
        mockCandidate,
        CRMBookingStatus.Reserved
      );
  ...
  describe("updateBookingStatusPaymentStatusAndTCNUpdateDate", () => {
    ...
    test("successfully updates booking status, booking product, payment status in a batch request when user cancelled", async () => {
      ...
      await crmGateway.updateBookingStatusPaymentStatusAndTCNUpdateDate(
        mockBooking.bookingId,
        mockBooking.bookingProductId,
        CRMBookingStatus.AbandonedNonRecoverable,
        mockBooking.paymentId
      );
    ...
    test("upon a gateway error or a system error from the payments API (cpms code 810 or 828 with status 500), update the booking status and booking product to 'SystemErrorNonRecoverable' and set payment status all in a batch request", async () => {
      ...
      await crmGateway.updateBookingStatusPaymentStatusAndTCNUpdateDate(
        mockBooking.bookingId,
        mockBooking.bookingProductId,
        CRMBookingStatus.SystemErrorNonRecoverable
      );
  ...
  describe("calculateThreeWorkingDays", () => {
    test("should successfully calculate 3 working days for Northern Ireland", async () => {
      const result = await crmGateway.calculateThreeWorkingDays(
        mockToday,
        CRMRemit.NorthernIreland
      );
  ...
  describe("updateCompensationBooking", () => {
    test("successfully creates and updates a compensation booking to recognise the owed compensation", async () => {
      const mockLicence = `int-test-${uuidv4()}`;
      const mockCandidate = await crmTestClient.createNewCandidate(mockLicence);
      const newCompensatedBooking =
        await crmTestClient.createNewCompensatedBooking(
          mockCandidate,
          CRMOrigin.CitizenPortal,
          CRMProductNumber.CAR
        );

      const { bookingId } = newCompensatedBooking;
      const mockOwedCompensationBookingRecognised = dayjs().toISOString();

      await crmGateway.updateCompensationBooking(
        bookingId,
        mockOwedCompensationBookingRecognised
      );

      const expectedBooking = await crmTestClient.retrieveBooking(bookingId);

      expect(expectedBooking.ftts_owedcompbookingrecognised).toStrictEqual(
        new Date(mockOwedCompensationBookingRecognised)
      );
    });
  });
  ...
  describe("getCandidateBookings", () => {
    ...
    test("retrieves a candidate's bookings with expected statuses", async () => {
      const statuses = [
        CRMBookingStatus.Confirmed,
        CRMBookingStatus.CompletePassed,
        CRMBookingStatus.CompleteFailed,
        CRMBookingStatus.ChangeInProgress,
        CRMBookingStatus.CancellationInProgress,
      ];
      const mockBookings = await Promise.all(
        statuses.map((status) =>
          crmTestClient.createNewBooking(mockCandidate, status)
        )
      );

      const result = await crmGateway.getCandidateBookings(
        mockCandidate.candidateId
      );

      expect(result).toHaveLength(5);
      const expectedBookingIds = mockBookings
        .map((booking) => booking.bookingId)
        .sort();
      const actualBookingIds = result
        .map((booking) => booking.bookingId)
        .sort();
    ...
    test("filters out cancelled bookings", async () => {
      await crmTestClient.createNewBooking(
        mockCandidate,
        CRMBookingStatus.Cancelled
      );
    ...
    test("filters out bookings not from the booking app or CSC", async () => {
      await crmTestClient.createNewBooking(
        mockCandidate,
        CRMBookingStatus.Confirmed,
        CRMOrigin.TrainerBookerPortal
      );
      await crmTestClient.createNewBooking(
        mockCandidate,
        CRMBookingStatus.Confirmed,
        CRMOrigin.IHTTCPortal
      );
    ...
    test("gets all the expected booking attributes", async () => {
      ...
      const expectedProps = [
        ...
        "enableEligibilityBypass",
        "origin",
        "testCentre.testCentreId",
        "testCentre.name",
        "testCentre.addressLine1",
        "testCentre.addressLine2",
        "testCentre.addressCity",
        "testCentre.addressPostalCode",
        "testCentre.remit",
        "testCentre.accountId",
        "testCentre.region",
        "testCentre.siteId",
        "bookingProductRef",
        "testSupportNeed",
    ...
    test("retrieves cancelled bookings that are eligible to be rebooked", async () => {
      ...
      const updateBooking = await crmTestClient.updateBooking(
        mockBooking.bookingId,
        {
    ...
    describe("enableViewNsaBookingSlots feature toggle is on", () => {
      ...
      test("filters back all nsa draft bookings that have no NSA bookings slots", async () => {
        const mockDraftBooking = await crmTestClient.createNewBooking(
          mockCandidate,
          CRMBookingStatus.Draft,
          CRMOrigin.CitizenPortal,
          CRMProductNumber.CAR,
        ...
        expect(returnedBooking.nsaStatus).toEqual(
          CRMNsaStatus.AwaitingCscResponse
        );
      ...
      test("filters back all nsa draft bookings with some NSA bookings slots", async () => {
        ...
        for (let i = 0; i < 3; i++) {
          ...
          await crmTestClient.createNSABookingSlot(
            mockDraftBooking.bookingId,
            CRMNsaBookingSlotStatus.Offered,
            "test-reservation-id",
            "2023-04-08T08:45:00Z"
          );
        }
        ...
        const returnedBooking = result[0];
        const returnedNsaBookingSlots =
          returnedBooking.nsaBookingSlots as CRMNsaBookingSlots[];

        expect(result).toHaveLength(1);
        expect(returnedNsaBookingSlots[0]._ftts_bookingid_value).toEqual(
          mockDraftBooking.bookingId
        );
        expect(returnedNsaBookingSlots[1]._ftts_bookingid_value).toEqual(
          mockDraftBooking.bookingId
        );
        expect(returnedNsaBookingSlots[2]._ftts_bookingid_value).toEqual(
          mockDraftBooking.bookingId
        );
      });
      ...
      test("retrieves expected NSA attributes", async () => {
        ...
        expect(returnedNsaBookingSlots[0]).toHaveProperty(
          "_ftts_organisationid_value"
        );
  ...
  describe("getCompensatedBookings", () => {
    ...
    test("retrieves candidates compensated bookings", async () => {
      const compensatedBooking =
        await crmTestClient.createNewCompensatedBooking(mockCandidate);
      const bookingProduct = await crmTestClient.retrieveBookingProduct(
        compensatedBooking.bookingProductId
      );
      const booking = await crmTestClient.retrieveBooking(
        compensatedBooking.bookingId
      );

      const result = await crmGateway.getCandidateCompensatedBookings(
        mockCandidate.candidateId,
        Target.GB
      );
  ...
  describe("updateZeroCostBooking", () => {
    test("successfully updates booking with zero cost fields", async () => {
      const mockLicence = `int-test-${uuidv4()}`;
      const mockCandidate = await crmTestClient.createNewCandidate(mockLicence);
      const mockBooking = await crmTestClient.createNewBookingForInstructor(
        mockCandidate,
        CRMBookingStatus.Confirmed,
        CRMOrigin.CitizenPortal
      );
      ...
      expect(updatedBooking.ftts_zerocostbookingreason).toBe(
        CRMZeroCostBookingReason.EXAMINER
      );
    });
  });
  ...
  describe("getUserDraftNSABookingIfExist", () => {
    ...
    test("returns true if draft NSA booking found", async () => {
      ...
      const result = await crmGateway.getUserDraftNSABookingsIfExist(
        mockCandidate.candidateId,
        TestType.CAR
      );
      ...
      expect(result).toStrictEqual(
        expect.arrayContaining([
          expect.objectContaining({
            bookingId: mockDraftBooking.bookingId,
  ...
  describe("updateNSABookings", () => {
    ...
    test("successfully updates booking status and nsa status to no longer required and standard test booked", async () => {
      ...
      await crmTestClient.updateBooking(mockBooking.bookingId, {
        ftts_nsastatus: CRMNsaStatus.AwaitingCscResponse,
      });

      const mockUpdatedBooking = await crmTestClient.retrieveBooking(
        mockBooking.bookingId
      );
      const crmNsaStatus =
        mockUpdatedBooking.ftts_nsastatus as unknown as CRMNsaStatus;
      ...
      const retrievedBooking = await crmTestClient.retrieveBooking(
        mockBooking.bookingId
      );

      expect(retrievedBooking.ftts_bookingstatus).toBe(
        CRMBookingStatus.NoLongerRequired
      );
      expect(retrievedBooking.ftts_nsastatus).toBe(
        CRMNsaStatus.StandardTestBooked
      );
  ...
  describe("doesCandidateHaveExistingBookingsByTestType", () => {
    ...
    test("returns true if confirmed bookings of existing test type found", async () => {
      await crmTestClient.createNewBookingWithPayment(
        mockCandidate,
        CRMBookingStatus.Confirmed,
        undefined,
        CRMProductNumber.CAR
      ...
      const result =
        await crmGateway.doesCandidateHaveExistingBookingsByTestType(
          mockCandidate.candidateId,
          TestType.CAR
        );
  ...
  describe("rescheduleBookingAndConfirm", () => {
    test("successfully updates booking with new booking details", async () => {
      ...
      await crmGateway.rescheduleBookingAndConfirm(
        mockBooking.bookingId,
        "2022-05-09T09:52:00Z",
        2,
        false
      );

      const changedBooking = await crmTestClient.retrieveBooking(
        mockBooking.bookingId
      );

      expect(changedBooking.ftts_testdate).toStrictEqual(
        new Date("2022-05-09T09:52:00.000Z")
      );

...
export class ChooseAppointmentPage extends BasePage {
  ...
  noSlotsAvailableWarningText =
    "There is no availability on this date. Try another day.";

  pastDaysWarningText = "Days in the past cannot be viewed";

  futureDaysWarningText =
    "This day is not yet available for booking. Check regularly as this may change.";
  ...
  changeBannerText1 =
    "You can change the test time here or keep the time previously selected.";
  ...
  async chooseAppointment(
    ...
    index = 0
  ): Promise<CheckYourDetailsPage> {
    let slotIndex = index;
    const timeValue = await Selector(
      `${this.selectATime} + ${this.slotId}`
    ).nth(slotIndex).value;
    if (timeValue.includes("11:00")) {
      // avoid 11am as TCN stub will throw error
      slotIndex += 1;
    }
    currentBooking.dateTime = await Selector(
      `${this.selectATime} + ${this.slotId}`
    ).nth(slotIndex).value;

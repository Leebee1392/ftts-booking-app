...
import {
  ChooseAppointmentBody,
  ChooseAppointmentController,
} from "@controllers/choose-appointment/choose-appointment";
...
describe("Choose appointment controller", () => {
  const mockedAppointmentService = new AppointmentService(
    {} as SchedulingGateway
  ...
  const mockSlotsByDate: Record<string, AppointmentSlot[]> = {
    ...
    "2022-01-27": [
      { startDateTime: "2022-01-27T09:30:00.000Z" },
      { startDateTime: "2022-01-27T15:15:00.000Z" },
    ] as AppointmentSlot[],
    "2022-01-28": [
      { startDateTime: "2022-01-28T14:15:00.000Z" },
    ] as AppointmentSlot[],
  };
  const mockWeekViewDates = [
    "2022-01-24",
    "2022-01-25",
    "2022-01-26",
    "2022-01-27",
  ...
  beforeAll(() => {
    ...
    mockedAppointmentService.getWeekViewDatesDesktop.mockReturnValue(
      mockWeekViewDates
    );
    mockedAppointmentService.getWeekViewDatesMobile.mockReturnValue(
      mockWeekViewMobileDates
    );
    mockedAppointmentService.getNextDateMobile.mockReturnValue("2022-01-29");
    mockedAppointmentService.getPreviousDateMobile.mockReturnValue(
      "2022-01-25"
    );
  });
  ...
  describe("GET request", () => {
    ...
    test("sets back link to manage-change-location-time page when coming from that page in manage booking journey", async () => {
      ...
      req.session.journey.managedBookingRescheduleChoice =
        "/manage-booking/choose-appointment";
    ...
    test("renders the page with slots view data", async () => {
      ...
      expect(res.render).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          eligibleFromDate:
            req.session.candidate.eligibilities?.[0].eligibleFrom,
    ...
    describe("eligibility dates", () => {
      test("selected date on/after eligibleFrom sets flag isBeforeEligible to false", async () => {
        ...
        req.session.candidate.eligibilities = [
          { eligibleFrom: "2022-07-17", testType: TestType.CAR } as any,
        ];
      ...
      test("selected date on/before eligibleTo sets flag isAfterEligible to false", async () => {
        req.query = { selectedDate: "2022-07-17" };
        req.session.candidate.eligibilities = [
          { eligibleTo: "2022-07-17", testType: TestType.CAR } as any,
        ];
    ...
    describe("KPI indicators", () => {
      test("stores KPI indicators in session when returned from TCN given preferred date", async () => {
        ...
        expect(mockedAppointmentService.getSlots).toHaveBeenCalledWith(
          ...
          req.session.currentBooking.firstSelectedDate
        );
        expect(req.session.currentBooking.dateAvailableOnOrAfterToday).toBe(
          "mock-date-1"
        );
        expect(
  ...
  describe("POST request", () => {
    ...
    test("in edit mode stores the selected slot datetime in the edited location/time session", async () => {
      ...
      expect(req.session.editedLocationTime.dateTime).toBe(
        mockSelectedDatetime
      );
    });
    ...
    test("in manage booking journey stores the selected slot datetime in the manage booking session", async () => {
      ...
      expect(req.session.currentBooking.dateTime).toBeUndefined();
      expect(req.session.manageBookingEdits.dateTime).toBe(
        mockSelectedDatetime
      );
    });

...
export class ManageBookingCheckYourDetailsController {
  ...
  public post = (req: Request): void => {
    logger.debug("check-your-details::post: Post method is called", {
  ...
  private renderPage = (req: Request, res: Response): void => {
    ...
    bookings.forEach((booking: Booking) => {
      if (booking.details.bookingStatus === CRMBookingStatus.Draft) {
        const nsaBookingSlots = booking.details
          .nsaBookingSlots as CRMNsaBookingSlots[];
    ...
    const testType = fromCRMProductNumber(
      product?.productnumber as CRMProductNumber
    );
    ...
    const testLanguage = TestLanguage.fromCRMTestLanguage(
      nsaBooking.testLanguage
    );

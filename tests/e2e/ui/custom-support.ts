...
test("Verify page title, heading and content are displayed correctly", async () => {
  ...
  await verifyExactText(
    customSupportPage.charCountHintText,
    customSupportPage.characterCountText
  );
});
...
test("Verify a validation error message is displayed when entering more than 4000 characters and the text is retained", async () => {
  ...
  await verifyExactText(
    customSupportPage.errorMessageLocator,
    customSupportPage.errorMessageHeader
  );
  await verifyExactText(
    customSupportPage.errorMessageList,
    customSupportPage.errorMessageText
  );
  await verifyContainsText(
    customSupportPage.errorMessageTextAreaLocator,
    customSupportPage.errorMessageText
  );

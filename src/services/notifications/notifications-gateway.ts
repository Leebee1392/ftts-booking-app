...
class NotificationsGateway {
  constructor(
    ...
    private axiosRetryClient = new AxiosRetryClient(
      config.notification.retryPolicy
    ).getClient(),
    ...
    try {
      ...
      switch (true) {
        ...
        case error.response?.status >= 500 && error.response?.status < 600:
          logger.event(
            BusinessTelemetryEvents.NOTIF_ERROR,
            errorMessage,
            errorPayload
          );
          break;
        case error.response?.status >= 400 && error.response?.status < 500:
          logger.event(
            BusinessTelemetryEvents.NOTIF_REQUEST_ISSUE,
            errorMessage,
            errorPayload
          );
          break;
        default:
          logger.warn(
            "NotificationsGateway::sendEmail: Notification API email send failed",
            {
              errorMessage,
              errorPayload,
            }
  ...
  private async getToken(): Promise<AuthHeader> {
    try {
      ...
      logger.event(
        BusinessTelemetryEvents.NOTIF_AUTH_ISSUE,
        "NotificationsGateway::getToken: Token call failed",
        { error }
      );
      throw error;
    }
...
const notificationsGateway = new NotificationsGateway(
  new ManagedIdentityAuth(config.notification.identity)
);

export { notificationsGateway, NotificationsGateway };

...
import { PaymentInitiationController } from "@controllers/payment-initiation/payment-initiation";
import {
...
describe("Payment confirmation controller", () => {
  ...
  beforeEach(() => {
    ...
    mockPaymentGateway.initiateCardPayment.mockResolvedValue(
      cardPaymentResponse
    );
    controller = new PaymentInitiationController(
      mockCrmGateway,
      mockPaymentGateway
    );
  ...
  describe("GET request", () => {
    test("calls payment API to initiate the payment", async () => {
      ...
      expect(mockPaymentGateway.initiateCardPayment).toHaveBeenCalledWith(
        cardPaymentPayload,
        candidateId,
        personReference
      );
    ...
    test("uses instructor redirect uri if instructor journey", async () => {
      ...
      cardPaymentPayload.redirectUri =
        "redirectUri/instructor/payment-confirmation-loading/B-000-000-001";
    ...
    test("adds receiptReference to the session", async () => {
      ...
      expect(req.session.currentBooking.receiptReference).toBe(
    ...
    test("calls CRM to create a bind bewtween a booking and a payment", async () => {
      ...
      expect(
        mockCrmGateway.createBindBetweenBookingAndPayment
  ...
  describe("error handling", () => {
    ...
    test("renders the payment error page if the payment call fails", async () => {
      ...
      expect(res.render).toHaveBeenCalledWith(
        "create/payment-initiation-error"
      );

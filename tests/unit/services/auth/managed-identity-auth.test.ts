...
} from "../../../../src/services/auth/managed-identity-auth";
...
describe("ManagedIdentityAuth", () => {
  ...
  beforeEach(() => {
    mockedChainedTokenCredential.mockReturnValue(
      mockedChainedTokenCredentialInstance
    );
  ...
  test("gets the auth header", async () => {
    mockedChainedTokenCredentialInstance.getToken.mockImplementationOnce(
      () => ({ token: Promise.resolve("mockTestToken") })
    );
  ...
  test("throws an error if unable to get token", async () => {
    mockedChainedTokenCredentialInstance.getToken.mockRejectedValue(
      "Unknown error"
    );

    await expect(managedIdentityAuth.getAuthHeader()).rejects.toBe(
      "Unknown error"
    );
    expect(mockedChainedTokenCredentialInstance.getToken).toHaveBeenCalled();
  });

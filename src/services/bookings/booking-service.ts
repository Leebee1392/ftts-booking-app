...
export class BookingService {
  ...
  public async completeBooking(
    candidate: Pick<
      Candidate,
      "behaviouralMarker" | "behaviouralMarkerExpiryDate"
    >,
    booking: Required<
      Pick<
        Booking,
    ...
    try {
        ...
        [
          {
            ...
            behaviouralMarkers: hasBehaviouralMarkerForTest(
              dateTime,
              candidate.behaviouralMarker,
              candidate.behaviouralMarkerExpiryDate
            )
              ? behaviouralMarkerLabel
              : "",
          },
      ...
      if (error.response?.status === 401 || error.response?.status === 403) {
        logger.event(
          ...
          "BookingService::completeBooking: Failed to authenticate to the scheduling api",
          {
            ...businessIdentifiers,
          }
        );
        ...
        logger.event(
          ...
          `BookingService::completeBooking: Scheduling API did not accept request ${error.response?.status}`,
          {
            ...businessIdentifiers,
          }
        );
        ...
        logger.event(
          ...
          `BookingService::completeBooking: Scheduling API returned error response ${error.response?.status}`,
          {
            ...businessIdentifiers,
          }
        );
      ...
      logger.event(
        BusinessTelemetryEvents.SCHEDULING_FAIL_CONFIRM_NEW,
        "BookingService::completeBooking: Failed to confirm new slot to make the booking with Scheduling API",
        {
          ...businessIdentifiers,
        }
      );
      logger.error(
        error as Error,
        "BookingService::completeBooking: Error confirming booking slot with scheduling",
        {
          bookingRef,
          ...businessIdentifiers,
        }
      );
      await this.unreserveBooking(
        bookingProductRef,
        centre.region,
        bookingProductId,
        bookingId,
    ...
    try {
      ...
      lastRefundDate = await this.crmGateway.calculateThreeWorkingDays(
        dateTime,
        centre.remit
      );
  ...
  public async unreserveBooking(
    ...
    try {
      await this.scheduling.deleteReservation(
        reservationId,
        region,
        bookingProductRef
      ); // We want to still update booking status to draft if this fails
    } catch (error) {
      logger.warn(
        "BookingService::unreserveBooking: Error when deleting reservation",
        {
          error,
          ...businessIdentifiers,
        }

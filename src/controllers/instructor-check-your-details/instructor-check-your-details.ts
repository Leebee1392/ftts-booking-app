...
export class InstructorCheckYourDetailsController {
  ...
  public post = async (req: Request, res: Response): Promise<void> => {
    if (!req.session.currentBooking) {
      throw Error(
        "InstructorCheckYourDetailsController::renderPage: No currentBooking set"
      );
    }
    ...
    if (!booking.bookingRef || !booking.testType || !booking.language) {
      logger.error(
        Error(
          "InstructorCheckYourDetailsController::post::Booking details not set correctly"
        ),
        "Error: Could not send support email",
        {
    ...
    const emailContent = this.buildCorrespondingSupportRequestEmailContent(
      ...
      target,
      lang,
      req.session.candidate as Candidate
    );
    try {
  ...
  private renderPage(req: Request, res: Response): void {
    ...
    if (!candidate || !booking || !booking.testType) {
      throw Error(
        "InstructorCheckYourDetailsController::renderPage: Missing required candidate/booking session data"
      );
    }
  ...
  private getBackLink(req: Request): string {
    if (!req.session.currentBooking) {
      throw Error(
        "InstructorCheckYourDetailsController::getBackLink: No currentBooking set"
      );
    }

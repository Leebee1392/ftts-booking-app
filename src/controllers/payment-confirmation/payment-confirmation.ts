...
import { Locale, Target, TCNRegion, TestType } from "../../domain/enums";
...
import {
  ...
  logger,
  isValidSessionBooking,
  isValidSessionCandidate,
} from "../../helpers/index";
...
export class PaymentConfirmationController {
  ...
  public get = async (req: Request, res: Response): Promise<void> => {
    logger.event(
      BusinessTelemetryEvents.PAYMENT_BACK,
      "PaymentConfirmationController::get: User returned from payment"
    );
    if (
      !isValidSessionBooking(req.session.currentBooking) ||
      !isValidSessionCandidate(req.session.candidate)
    ) {
      ...
      throw new Error(
        "PaymentConfirmationController::get: Missing required session data"
      );
    }

    const { bookingRef, dateTime, testType, centre } =
    ...
    const isTestTypeZeroCost = isZeroCostTest(
      req.session.currentBooking.testType
    );
    const isCompensationBooking =
    ...
    if (
      ...
      throw new Error(
        "PaymentConfirmationController::get: Missing required session data - receipt reference"
      );
    }

    if (isCompensationBooking) {
      await this.compensateBooking(
        req.session.currentBooking,
        req.session.candidate
      );
    } else if (isTestTypeZeroCost) {
      logger.debug(
        "PaymentConfirmationController::get: Updating CRM entity of zero cost booking",
        {
          candidate: req.session.candidate,
          ...getCreatedBookingIdentifiers(req),
        }
      );
      await this.crmGateway.updateZeroCostBooking(
        req.session.currentBooking.bookingId
      );
    } else {
      try {
        paymentId = await this.confirmPayment(
          req,
          res,
          req.session.currentBooking,
          req.session.candidate
    ...
    const bookingCompletionResult = await this.completeBooking(
      req.session.currentBooking,
      req.session.candidate,
      paymentId as string,
      req
    ...
    try {
      ...
      const emailContent = buildBookingConfirmationEmailContent(
        details,
        target,
        locale
      );
      ...
      logger.error(
        error as Error,
        "PaymentConfirmationController::get: Error sending booking confirmation email",
        { ...getCreatedBookingIdentifiers(req) }
      );
    }

    try {
      const nsaBookings = await this.crmGateway.getUserDraftNSABookingsIfExist(
      ...
      logger.error(
        error as Error,
        "PaymentConfirmationController::get: Error executing batch request to update nsa booking status",
        { ...getCreatedBookingIdentifiers(req) }
      );
    }
  ...
  private compensateBooking = async (
    currentBooking: Booking,
    candidate: Candidate
  ): Promise<void> => {
    if (!currentBooking.bookingProductId) {
      throw Error(
        "PaymentConfirmationController::compensateBooking: Unable to create compensated booking - new booking product id does not exist"
      );
    }
    ...
    try {
      await this.payments.compensateBooking(
        {
          compensatedBookingProductId: currentBooking.compensationBooking
            ?.bookingProductId as string,
      ...
      logger.error(
        error as Error,
        "PaymentConfirmationController::compensateBooking: Failed to compensate booking",
        {
          newBookingProductId: currentBooking.bookingProductId,
          originalBookingProductId:
            currentBooking.compensationBooking?.bookingProductId,
  ...
  private confirmPayment = async (
    ...
    if (!currentBooking.receiptReference) {
      throw new Error(
        "PaymentConfirmationController::confirmPayment: Missing receipt reference"
      );
    }

    if (!candidate.personReference) {
      throw new Error(
        "PaymentConfirmationController::confirmPayment: Missing person reference"
      );
    }
    ...
    try {
      cardPaymentCompletionResponse =
        await this.payments.confirmCardPaymentIsComplete(
          currentBooking.receiptReference,
          candidate.candidateId as string,
          candidate.personReference
        );
      logger.debug(
        "PaymentConfirmationController::confirmPayment: confirmCardPayment response for booking",
        {
      ...
      if (cardPaymentCompletionResponse.code !== PaymentStatus.SUCCESS) {
        ...
        logger.event(
          ...
          "PaymentConfirmationController::confirmPayment: Error confirming payment",
          {
            ...getCreatedBookingIdentifiers(req),
            cpmsCode: cardPaymentCompletionResponse?.code,
          }
        );
        await this.bookingService.unreserveBooking(
          bookingProductRef as string,
          centre?.region as TCNRegion,
          bookingProductId as string,
          bookingId as string,
      ...
      logger.error(
        error as Error,
        "PaymentConfirmationController::confirmPayment: Error confirming payment for booking",
        {
      ...
      if (errorCode > 800) {
        ...
        if (errorCode === PaymentStatus.USER_CANCELLED) {
          logger.event(
            BusinessTelemetryEvents.PAYMENT_CANCEL,
            "PaymentConfirmationController::confirmPayment: User cancelled payment",
            {
              error,
              cpmsCode: errorCode,
              ...getCreatedBookingIdentifiers(req),
          ...
          await this.bookingService.unreserveBooking(
            ...
            req.session.currentBooking?.paymentId
          );
          throw new PaymentUserCancelledError();
        } else if (errorCode === PaymentStatus.GATEWAY_ERROR) {
          logger.event(
            BusinessTelemetryEvents.PAYMENT_GATEWAY_ERROR,
            "PaymentConfirmationController::confirmPayment: Payment API gateway error",
            {
              error,
              cpmsCode: errorCode,
              ...getCreatedBookingIdentifiers(req),
          ...
          logger.event(
            BusinessTelemetryEvents.PAYMENT_SYSTEM_ERROR,
            "PaymentConfirmationController::confirmPayment: Payment API system error",
            {
              error,
              cpmsCode: errorCode,
              ...getCreatedBookingIdentifiers(req),
  ...
  private completeBooking = async (
    ...
    let bookingCompletionResult: BookingCompletionResult = {
      isConfirmed: false,
      bookingRef,
    };
    ...
    try {
      ...
      logger.error(
        error as Error,
        "payment-confirmation::get: Error completing booking",
        {
          bookingRef,
          ...getCreatedBookingIdentifiers(req),
        }

...
describe("Test Language helper", () => {
  test("Creating a TestLanguage from an Unknown test language throws error", () => {
    expect(() => TestLanguage.from("unknownLanguage")).toThrow(
      translate("testLanguage.validationError")
    );
  });
  ...
  test("Check if a language is valid - unavailable value", () => {
    expect(() => TestLanguage.isValid("unknownLanguage")).toThrow(
      translate("testLanguage.validationError")
    );
  });
  ...
  describe("Check if a language can be changed", () => {
    test.each([
      ...
      "%s test type booking is able to change language on GB service",
      (testType: TestType) => {
        const response = TestLanguage.canChangeTestLanguage(
          Target.GB,
          testType
        );

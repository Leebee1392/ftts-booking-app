...
export class FindATheoryTestCentrePage extends BasePage {
  ...
  searchLabel =
    "Enter a postcode, town, city or region. You can also use landmarks.";
  ...
  changeBannerWarningText =
    "If you select a new location here, it will replace your previous choice. Date and time slots will still be subject to availability.";

  async enterSearchTerm(searchTerm: string): Promise<void> {
    await enter(this.searchLocationTermTextBox, searchTerm);
  }

  async findATheoryTestCentre(
    searchTerm: string
  ): Promise<ChooseATestCentrePage> {

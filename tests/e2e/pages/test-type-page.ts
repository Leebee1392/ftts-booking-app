...
export class TestTypePage extends BasePage {
  ...
  bookingOtherTypesOfTestsLink =
    'a[href="https://www.gov.uk/driving-licence-categories"]';
  ...
  bookingOtherTypesOfTestsText =
    "You must have a category entitlement on your driving licence for any type of vehicle you want to drive.";

  errorMessageHeader = "There is a problem";

  errorMessageText = "You need to select a theory test";

  updateTestTypeBannerText =
    "You can change the test type or keep your existing choice.";
  ...
  warningText =
    "Important: if you change the test type and continue, you will need to enter the booking details again.";

  compensationText1 =
    "You recently had a theory test cancelled. Select this option to book a replacement test.";
  ...
  async verifyCompensationTextShown(testType: TestType): Promise<void> {
    const selector = this.getTestTypeSelector(testType);
    await t
      .expect(Selector(selector).getAttribute("data-is-compensation"))
      .eql("true");
    await verifyContainsText(
      this.compensationHintTextLocator,
      this.compensationText1
    );

...
test("Verify a validation error message is displayed when entering more than 100 characters and the text is retained", async () => {
  ...
  await verifyContainsText(
    translatorPage.errorMessageLocator,
    translatorPage.errorMessageHeader
  );
  await verifyContainsText(
    translatorPage.errorMessageList,
    translatorPage.overLimitErrorMessageText
  );
  await verifyContainsText(
    translatorPage.errorMessageTextAreaLocator,
    translatorPage.overLimitErrorMessageText
  );
  ...
  await t
    ...
    .eql(Constants.stringWith101Chars);
});
...
test("Verify a validation error message is displayed when not entering any text", async () => {
  ...
  await verifyContainsText(
    translatorPage.errorMessageList,
    translatorPage.emptyErrorMessageText
  );
  await verifyContainsText(
    translatorPage.errorMessageTextAreaLocator,
    translatorPage.emptyErrorMessageText
...
test("Verify the user is able to proceed when entering valid translator details", async () => {
  await translatorPage.enterTranslatorDetails(
    "I would like a translator for Chinese"
  );
  ...
  await verifyContainsText(
    ...
    stayingNSAPage.pageHeadingNoEvidence
  );
});

import { ManageBookingChangeController } from "@controllers/manage-booking-change/manage-booking-change";
...
describe("Manage booking - change controller", () => {
  ...
  describe("GET", () => {
    ...
    test("renders the change booking page", async () => {
      ...
      expect(res.render).toHaveBeenCalledWith("manage-booking/change", {
    ...
    test.each([
      ...
      "when the test is a DVA %s instructor test then it renders the change booking page without the voiceover option present",
      async (_, testProductNumber) => {
    ...
    test.each([
      ...
      "when the test is a DVA %s instructor test then it renders the change booking page with zero cost booking flag set to true",
      async (_, testProductNumber) => {
        req.session.manageBooking.bookings[0].product.productnumber =
          testProductNumber;
    ...
    test("throws an error if no manage booking is set in session", async () => {
      ...
      await expect(controller.get(req, res)).rejects.toEqual(
        new Error("ManageBookingChangeController::get: No manageBooking set")
      );
    });
    ...
    describe("CRM working days calculation", () => {
      describe("if the booking is for today", () => {
        test("does not call CRM as the working days calculation is not needed", async () => {
          ...
          expect(
            mockCrmGateway.calculateThreeWorkingDays
      ...
      describe("if the booking is for a day other than today", () => {
        ...
        test("calls CRM to get the date 3 clear working days prior to the test date and stores it in the session", async () => {
          ...
          req.session.manageBooking.bookings[0].testDateMinus3ClearWorkingDays =
            undefined;

          await controller.get(req, res);

          expect(mockCrmGateway.calculateThreeWorkingDays).toHaveBeenCalledWith(
            mockTestDate,
            mockRemit
          );
          expect(req.session.manageBooking.bookings[0]).toStrictEqual(
            expect.objectContaining({

...
test("Verify correct message appears to inform the candidate the earliest date they can book their test from", async () => {
  ...
  const warningText = `You are not eligible to take tests on the date selected. The earliest date you can book the test for is ${eligibleFromDate.format(
    "D"
  )} ${eligibleFromDate.format("MMMM")} ${eligibleFromDate.format("YYYY")}`;
...
test("Verify correct message appears to inform the candidate when they are no longer eligible to book after a certain date", async () => {
  ...
  const warningText =
    "You are not eligible to take tests on the day selected. For eligibility queries, contact DVSA.";
  await verifyExactText(
    chooseAppointmentPage.warningMessageLocator,
    warningText
  );

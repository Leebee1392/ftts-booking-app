import { ManageBookingCancelController } from "@controllers/manage-booking-cancel/manage-booking-cancel";
import {
  Locale,
  Target,
  TCNRegion,
  ...
  describe("POST", () => {
    ...
    test("renders the cancel booking page", async () => {
      ...
      expect(mockSchedulingGateway.deleteBooking).toHaveBeenCalledWith(
        req.session.manageBooking.bookings[0].bookingProductRef,
        "a"
      );
      ...
      expect(res.render).toHaveBeenCalledWith(
        "manage-booking/cancel-confirmation",
        {
    ...
    test("CSC bookings - renders the cancel booking page", async () => {
      req.session.manageBooking.bookings[0].origin =
    ...
    describe("For zero cost test", () => {
      test("payment API is not called", async () => {
        req.session.manageBooking.bookings[0].product.productnumber =
    ...
    describe("Handle payment/refund", () => {
      describe("if the booking is refundable", () => {
        test("calls payment api to request a refund", async () => {
          ...
          expect(mockPaymentClient.requestRefund).toHaveBeenCalledWith(
            expectedPayload,
            "mockCandidateId",
            mockPersonReference
          );
        ...
        test("requesting refund fails", async () => {
          ...
          expect(logger.event).toHaveBeenCalledWith(
            ...
            "ManageBookingCancelController::handlePayment: Refund attempt failed",
            {
              ...
              personReference:
                req.session.manageBooking.candidate.personReference,
            }
          );
          expect(mockBookingManager.loadCandidateBookings).toHaveBeenCalledWith(
            req,
      ...
      describe("if the booking is not refundable", () => {
        test("calls payment api to recognise the payment as income", async () => {
          ...
          expect(mockPaymentClient.recogniseIncome).toHaveBeenCalledWith(
            expectedRecogniseIncomePayload,
            "mockCandidateId",
            mockPersonReference
          );
        });

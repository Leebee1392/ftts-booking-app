...
export class CRMGateway {
  ...
  public static cdsRetryPolicy: Parameters<typeof cdsRetry>[1] =
    config.crm.retryPolicy;

  public async getLicenceNumberRecordsByCandidateId(
    candidateId: string,
    licenceNumber: string
  ): Promise<Candidate | undefined> {
    try {
      logger.info(
        "CRMGateway::getLicenceNumberRecordsByCandidateId: Attempting to retrieve Candidate and Licence from CRM",
        { candidateId }
      );
      const rawXml = await this.readFile(
        "getLicenceNumberRecordsByCandidateId.xml"
      );
      const xml = rawXml
      ...
      logger.debug(
        "CRMGateway::getLicenceNumberRecordsByCandidateId: Raw Request",
        { xml, entity: Collection.LICENCES }
      );
      const response: FetchXmlResponse<CRMLicenceCandidateResponse> =
        await this.dynamicsWebApi.fetch(Collection.LICENCES, xml);
      logger.debug(
        "CRMGateway::getLicenceNumberRecordsByCandidateId: Raw Response",
        { response, entity: Collection.LICENCES }
      );
      if (!response.value?.length) {
        logger.info(
          "CRMGateway::getLicenceNumberRecordsByCandidateId: No licence/candidate found for the given candidate id",
          { candidateId }
        );
        return undefined;
      }
      ...
      logger.info(
        "CRMGateway::getLicenceNumberRecordsByCandidateId: Candidate was mapped from CRM",
        { candidateId }
      );
      return candidate;
    } catch (error) {
  ...
  public async createLicence(
    ...
    try {
      logger.info("CRMGateway::createLicence: Creating licence record in CRM", {
        candidateId,
      });
      ...
      logger.debug("CRMGateway::createLicence: Raw Request", {
        request,
        candidateId,
      });
      const response =
        await this.dynamicsWebApi.createRequest<CreateLicenceResponse>(request);
      logger.debug("CRMGateway::createLicence: Response from CRM", {
        response,
      });
      ...
      logger.info(
        "CRMGateway::createLicence: Successfully created licence in CRM",
        { candidateId, licenceId }
      );
      return licenceId;
    } catch (error) {
      this.logGeneralError(
  ...
  public async createCandidate(candidate: Candidate): Promise<string> {
    try {
      ...
      logger.debug("CRMGateway::createCandidate: Raw Request", {
        createCandidateRequest,
      });
      const candidateResponse =
        await this.dynamicsWebApi.createRequest<CreateCandidateResponse>(
          createCandidateRequest
        );
      logger.debug(
        "CRMGateway::createCandidate: Create Candidate Raw Response",
        { candidateResponse }
      );
      ...
      logger.info(
        "CRMGateway::createCandidate: Successfully created candidate in CRM",
        { candidateId }
      );
      return candidateId;
    } catch (error) {
  ...
  public async updateCandidate(
    candidateId: string,
    candidateDetails: Partial<Candidate>
  ): Promise<Candidate> {
    try {
      logger.info(
        "CRMGateway::updateCandidate: Attempting to update candidate details",
        { candidateId }
      );

      const updatedCandidateDetails: Partial<CRMContact> =
        mapToCRMContact(candidateDetails);
      const candidateResponse = await this.updateCandidateRequest(
        candidateId,
        updatedCandidateDetails
      );

      logger.debug(
        "CRMGateway::updateCandidate: Successfully updated candidate in CRM",
        { candidateResponse }
      );
  ...
  public async updateLicence(
    licenceId: string,
    candidateId: string,
    address?: ELIG.Address
  ): Promise<CRMLicenceResponse> {
    try {
      logger.info(
        "CRMGateway::updateLicence: Attempting to update Licence details",
        {
          licenceId,
          candidateId,
        }
      );
      const updatedLicenceDetails = mapToCRMLicence(candidateId, undefined, {
        address,
      });
      const request = {
        key: licenceId,
      ...
      logger.debug("CRMGateway::updateLicence: Raw Request", {
        request,
        licenceId,
        candidateId,
      });
      const response =
        await this.dynamicsWebApi.updateRequest<CRMLicenceResponse>(request);
      logger.debug("CRMGateway::updateLicence: Response from CRM", {
        response,
        licenceId,
        candidateId,
      });
      logger.info(
        "CRMGateway::updateLicence: Successfully updated Licence in CRM",
        { licenceId, candidateId }
      );
      return response;
    } catch (error) {
      this.logGeneralError(
  ...
  public async updateCandidateAndCreateBooking(
    candidate: Candidate,
    booking: Booking,
    additionalSupport: CRMAdditionalSupport,
    isStandardAccommodation: boolean,
    priceListId: string
  ): Promise<CandidateAndBookingResponse> {
    ...
    try {
      ...
      if (!candidateId || !licenceId) {
        throw new Error(
          "CRMGateway::updateCandidateAndCreateBooking: Missing required candidate data"
        );
      }

      const updatedCandidateDetails: Partial<CRMContact> = mapToCRMContact({
      ...
      logger.debug(
        "CRMGateway::updateCandidateAndCreateBooking: Response from CRM",
        { response }
      );
      const candidateResponse: CandidateResponse = mapToCandidateResponse(
        response[0] as CRMContact
      );
      let bookingResponse: BookingResponse =
        mapCRMBookingResponseToBookingResponse(
          response[1] as CRMBookingResponse
        );
      ...
      logger.info("CRMGateway::updateCandidateAndCreateBooking: Successful", {
        candidateId,
        licenceId,
  ...
  private async validateBookingAndRetryIfNeeded(
    ...
    while (retries > 0) {
      ...
      const response = await this.createBookingRequest(
        candidate,
        booking,
        additionalSupport,
        isStandardAccommodation,
      ...
      const convertedResponse =
        mapCRMBookingResponseToBookingResponse(response);
    ...
    throw new CrmCreateBookingDataError(
      "CRMGateway::validateBookingAndRetryIfNeeded: crm returned data from different booking"
    );
  }

  private isCreateBookingDataValid(
    bookingResponse: BookingResponse,
    candidate: Candidate
  ): boolean {
    return (
      bookingResponse.firstName === candidate.firstnames &&
  ...
  public async createBookingProduct(
    ...
    try {
      logger.info(
        "CRMGateway::createBookingProduct: Creating Booking Product in CRM",
        {
          candidateId,
          licenceId,
      ...
      logger.debug("CRMGateway::createBookingProduct: Raw Request", {
        request,
      });
      const response =
        await this.dynamicsWebApi.createRequest<CreateBookingProductResponse>(
          request
        );
      logger.debug("CRMGateway::createBookingProduct: Raw Response", {
        response,
      });
      ...
      logger.info(
        "CRMGateway::createBookingProduct: Successfully created Booking Product in CRM",
        {
          candidateId,
          licenceId,
          bookingId: bookingResponse.id,
      ...
      logger.error(
        error,
        "CRMGateway::createBookingProduct: Could not create booking product entity in CRM",
        {
          candidateId,
          licenceId,
          bookingId,
  ...
  public async rescheduleBookingAndConfirm(
    bookingId: string,
    dateTime: string,
    currentRescheduleCount: number,
    isCSCBooking?: boolean,
    testCentreAccountId?: string,
    ...
    logger.info(
      "CRMGateway::rescheduleBookingAndConfirm: Attempting to reschedule and confirm booking",
      { bookingId, dateTime, rescheduleCount }
    );
    const entity: RescheduleUpdateRequest = {
      ...
      ftts_dateavailableonoraftertoday:
        kpiIdentifiers?.dateAvailableOnOrAfterToday,
      ftts_dateavailableonorbeforepreferreddate:
        kpiIdentifiers?.dateAvailableOnOrBeforePreferredDate,
      ftts_dateavailableonorafterpreferreddate:
        kpiIdentifiers?.dateAvailableOnOrAfterPreferredDate,
    ...
    try {
      ...
      logger.debug("CRMGateway::rescheduleBookingAndConfirm: Raw Request", {
        request,
      });
      await this.dynamicsWebApi.updateRequest(request);
      logger.info(
        "CRMGateway::rescheduleBookingAndConfirm: Booking rescheduled and confirmed",
        { bookingId, dateTime }
      );
    } catch (error) {
      logger.error(
        error,
        "CRMGateway::rescheduleBookingAndConfirm: Could not update booking date time and location in CRM",
        { bookingId, dateTime }
      );
      logger.event(
        BusinessTelemetryEvents.CDS_FAIL_BOOKING_CHANGE_UPDATE,
  ...
  public async updateBookingStatus(
    ...
    logger.info(
      "CRMGateway::updateBookingStatus: Attempting to update booking status",
      { bookingId, crmBookingStatus }
    );
    const request: DynamicsWebApi.UpdateRequest = {
      key: bookingId,
      collection: "ftts_bookings",
    ...
    try {
      await this.dynamicsWebApi.updateRequest(request);
      logger.info("CRMGateway::updateBookingStatus: Booking status updated", {
        bookingId,
        crmBookingStatus,
      });
    } catch (error) {
  ...
  public async updateVoiceover(
    bookingId: string,
    bookingProductId: string,
    voiceover: CRMVoiceOver,
    isCSCBooking?: boolean
  ): Promise<void> {
    logger.info("CRMGateway::updateVoiceover: Attempting to update voicover", {
      bookingId,
      bookingProductId,
      voiceover,
    });
    try {
      ...
      logger.info(
        "CRMGateway::updateVoiceover: Voiceover updated successfully",
        { bookingId, bookingProductId, voiceover }
      );
    } catch (error) {
      logger.error(
        error,
  ...
  public async updateAdditionalSupport(
    bookingId: string,
    bookingProductId: string,
    additionalSupport: CRMAdditionalSupport,
    isCSCBooking?: boolean
    ...
    logger.info(
      "CRMGateway::updateAdditionalSupport: Attempting to update additional support",
      { bookingId, bookingProductId, additionalSupport }
    );
    try {
      this.dynamicsWebApi.startBatch();
      this.updateProductBookingAdditionalSupport(
        bookingProductId,
        additionalSupport
      );
      this.updateBookingAdditionalSupport(
        bookingId,
        additionalSupport,
        isCSCBooking
      );
      await this.dynamicsWebApi.executeBatch();
      logger.info(
        "CRMGateway::updateAdditionalSupport: Additional support updated",
        { bookingId, bookingProductId, additionalSupport }
      );
    } catch (error) {
      logger.error(
  ...
  public async updateLanguage(
    ...
    logger.info("CRMGateway::updateLanguage: Attempting to update language", {
      bookingId,
      bookingProductId,
      language,
    });
    try {
      ...
      logger.debug("CRMGateway::updateLanguage: Raw Request", {
      ...
      await this.dynamicsWebApi.executeBatch();
      logger.info("CRMGateway::updateLanguage: Language updated successfully", {
        bookingId,
        bookingProductId,
        language,
      });
      ...
      logger.error(
        ...
        { bookingId, bookingProductId, language }
      );
      logger.event(
        BusinessTelemetryEvents.CDS_FAIL_BOOKING_CHANGE_UPDATE,
        "CRMGateway::updateLanguage: Could not update language in CRM",
  ...
  public async updateCompensationBooking(
    bookingId: string,
    owedCompensationBookingRecognised: string
  ): Promise<void> {
    const identifiers = {
      bookingId,
    ...
    logger.info(
      "CRMGateway::updateCompensationBooking: Attempting to update compensation booking",
      identifiers
    );

    try {
      await this.updateOwedCompensationBookingRecognised(
        bookingId,
        owedCompensationBookingRecognised
      );
      logger.info(
        "CRMGateway::updateCompensationBooking: Compensation booking has been updated in CRM",
        identifiers
      );
    } catch (error) {
      logger.error(
        error as Error,
        "CRMGateway::updateCompensationBooking: Could not update compensation booking in CRM",
        identifiers
      );
      throw error;
    }
   ...
   */
  public async getCandidateBookings(
    candidateId: string
    ...
    try {
      ...
      if (config.featureToggles.enableViewNsaBookingSlots) {
        ...
        const standardBookingsQuery = rawStandardXml.replace(
          "${candidateId}",
          candidateId
        );
        const nsaBookingsQuery = rawNsaXml.replace(
          "${candidateId}",
          candidateId
        );
        ...
        this.dynamicsWebApi.startBatch();
        await this.dynamicsWebApi.fetch(
          Collection.BOOKING_PRODUCT,
          standardBookingsQuery
        );
        await this.dynamicsWebApi.fetch(
          Collection.BOOKING_PRODUCT,
        ...
        const [standardBookings, nsaBookings] = [
          response[0].value as CRMXmlBookingDetails[],
          response[1].value as CRMXmlBookingDetails[],
        ];
        ...
        if (nsaBookingSlots) {
          ...
          nsaBookings.forEach((nsaBooking: CRMXmlBookingDetails) => {
            ...
            nsaBookingSlots.forEach((nsaBookingSlot: CRMNsaBookingSlots) => {
              if (
                nsaBookingSlot._ftts_bookingid_value ===
                nsaBooking._ftts_bookingid_value
              ) {
                const bookingSlot =
                  nsaBooking.ftts_nsabookingslots as CRMNsaBookingSlots[];
        ...
        logger.debug("CRMGateway::getCandidateBookings: Raw Request", {
          request,
        });
        const response =
          await this.dynamicsWebApi.retrieveMultipleRequest<CRMBookingDetails>(
            request
          );
        logger.debug("CRMGateway::getCandidateBookings: Raw Response", {
          response,
        });
      ...
      if (!normalisedRecords.length) {
        logger.debug(
          "CRMGateway::getCandidateBookings: No bookings found for candidate",
          { candidateId }
        );
        return [];
      }
      ...
      const result = normalisedRecords
        .filter((booking: CRMBookingDetails) => {
          ...
          if (booking.ftts_bookingid.ftts_governmentagency === null) {
            if (
              booking.ftts_bookingid.ftts_origin === CRMOrigin.CitizenPortal ||
              (booking.ftts_bookingid.ftts_origin ===
                CRMOrigin.CustomerServiceCentre &&
                booking.ftts_paymentstatus === CRMPaymentStatus.Success)
            ) {
              faultyBookingRefs.push(
                booking?.ftts_bookingid?.ftts_reference as string
              );
            }
            return false;
          }
          ...
          if (booking.ftts_bookingstatus === CRMBookingStatus.Cancelled) {
            return (
              booking.ftts_bookingid.ftts_owedcompbookingassigned !== null &&
      ...
      if (faultyBookingRefs.length > 0) {
        logger.debug(
          "CRMGateway::getCandidateBookings: The following bookings are filtered out because of missing data",
          { faultyBookingRefs, candidateId }
        );
      }

      // We need to retrieve the payment information for any bookings which have the status Cancellation in Progress.
      const resultsThatContainBookingsWithPaymentInformation =
        await this.getPaymentInformationForBookingDetails(result);
      logger.debug(
        "CRMGateway::getCandidateBookings: result of fetched bookings with bookings also containing payment information",
        { resultsThatContainBookingsWithPaymentInformation, candidateId }
      );
  ...
  public async getCandidateCompensatedBookings(
    candidateId: string,
    target: Target
  ): Promise<CompensatedBooking[]> {
    try {
      ...
      const xml = rawXml
        ...
        .replace(
          "${governmentAgency}",
          target === Target.NI
            ? String(CRMGovernmentAgency.Dva)
            : String(CRMGovernmentAgency.Dvsa)
        );
      logger.debug("CRMGateway::getCandidateCompensatedBookings: Raw Request", {
        xml,
        entity: Collection.BOOKING_PRODUCT,
      });
      const response: FetchXmlResponse<CompensatedBooking> =
        await this.dynamicsWebApi.fetch(Collection.BOOKING_PRODUCT, xml);
      logger.debug(
        "CRMGateway::getCandidateCompensatedBookings: Raw Response",
        { response, entity: Collection.BOOKING_PRODUCT }
      );
      ...
      this.logGeneralError(
        error,
        "CRMGateway::getCandidateCompensatedBookings: Could not retrieve compensated booking from CRM",
        { candidateId }
      );
      throw error;
    }
  ...
  public async calculateThreeWorkingDays(
    testDate: string,
    remit: CRMRemit
  ): Promise<string> {
    try {
      ...
      logger.debug("CRMGateway::calculateThreeWorkingDays: Raw Request", {
        request,
      });
      const response =
        await this.dynamicsWebApi.executeUnboundAction<GetWorkingDaysResponse>(
          "ftts_GetClearWorkingDay",
          request
        );
      logger.debug("CRMGateway::calculateThreeWorkingDays: Raw Response", {
        response,
      });
      const dueDate =
        response.DueDate instanceof Date
          ? response.DueDate
  ...
  public async updateTCNUpdateDate(bookingProductId: string): Promise<void> {
    logger.info(
      "CRMGateway::updateTCNUpdateDate: Attempting to update ftts_tcn_update_date",
      { bookingProductId }
    );
    try {
      const request: DynamicsWebApi.UpdateRequest = {
      ...
      logger.debug("CRMGateway::updateTCNUpdateDate: Raw Request", {
        request,
        bookingProductId,
      });
      await this.dynamicsWebApi.updateRequest(request);
      logger.info(
        "CRMGateway::updateTCNUpdateDate: ftts_tcn_update_date updated successfully",
        { bookingProductId }
      );
    } catch (error) {
      logger.error(
        error,
        "CRMGateway::updateTCNUpdateDate: Could not set the TCN Update Date on Booking Product ID",
        { bookingProductId }
      );
      logger.event(
        BusinessTelemetryEvents.CDS_FAIL_BOOKING_CHANGE_UPDATE,
        "CRMGateway::updateTCNUpdateDate: Could not set the TCN Update Date",
        { error, bookingProductId }
      );
      throw error;
    }
  }

  public async updateBookingStatusPaymentStatusAndTCNUpdateDate(
    bookingId: string,
    bookingProductId: string,
    ...
    try {
      ...
      if (paymentId) {
        updateBookingRequest.entity[
          "ftts_payment@odata.bind"
        ] = `/ftts_payments(${paymentId})`;
      }
      const updateBookingProductRequest: DynamicsWebApi.UpdateRequest = {
        key: bookingProductId,
        collection: "ftts_bookingproducts",
      ...
      await this.dynamicsWebApi.executeBatch();
      logger.info(
        "CRMGateway::updateBookingStatusPaymentStatusAndTCNUpdateDate: Update finished successfully",
        {
          bookingId,
          bookingProductId,
          bookingStatus,
  ...
  public async updateNSABookings(
    nsaBookings: NsaBookingDetail[]
  ): Promise<void> {
    if (nsaBookings.length >= 1000) {
      const error = Error(
        "CRMGateway::updateNSABookings: The number of NSA bookings to update exceeds a thousand"
      );
    ...
    try {
      ...
      for (const nsaBooking of nsaBookings) {
        if (
          nsaBooking.crmNsaStatus === CRMNsaStatus.AwaitingCscResponse ||
          nsaBooking.crmNsaStatus === CRMNsaStatus.DuplicationsClosed
        ) {
          const request: DynamicsWebApi.UpdateRequest = {
            ...
            entity: {
              ...
              ftts_callamend:
                nsaBooking.origin === CRMOrigin.CustomerServiceCentre
                  ? "true"
                  : undefined,
      ...
      await this.dynamicsWebApi.executeBatch();
      logger.info(
        "CRMGateway::updateNSABookings: Update finished successfully",
        {
          nsaBookings,
        }
      );
  ...
  public async getPriceList(
    ...
    const priceListId =
      target === Target.NI
        ? config.crm.priceListId.dva
        : config.crm.priceListId.dvsa;
    try {
      ...
      const response =
        await this.dynamicsWebApi.retrieveMultipleRequest<CRMProductPriceLevel>(
          request
        );
      ...
      const priceList: PriceListItem[] = response.value.map(
        (productPriceLevel) => ({
      ...
      logger.error(
        ...
        { priceListId, testTypes, target }
      );
      throw error;
    }
  }

  public async createBindBetweenBookingAndPayment(
    bookingId?: string,
    paymentId?: string,
    ...
    try {
      ...
      logger.info(
        "CRMGateway::createBindBetweenBookingAndPayment: Booking updated successfully",
        {
          bookingId,
          paymentId,
          receiptReference,
      ...
      this.logGeneralError(
        error,
        "CRMGateway::createBindBetweenBookingAndPayment: Could not create a bind",
        {
          bookingId,
          paymentId,
          receiptReference,
  ...
  public async updateZeroCostBooking(bookingId: string): Promise<void> {
    logger.info("CRMGateway::updateZeroCostBooking: Attempting to update", {
      bookingId,
    });
    try {
      const request: DynamicsWebApi.UpdateRequest = {
      ...
      logger.debug("CRMGateway::updateZeroCostBooking: Raw Request", {
        request,
        bookingId,
      });
      await this.dynamicsWebApi.updateRequest(request);
      logger.info(
        "CRMGateway::updateZeroCostBooking: Update finished successfully",
        { bookingId }
      );
    } catch (error) {
      logger.error(
  ...
  public async markBookingCancelled(
    bookingId: string,
    bookingProductId: string,
    isCSCBooking?: boolean
  ): Promise<void> {
    logger.info(
      "CRMGateway::markBookingCancelled: Attempting to update booking status and cancel date",
    ...
    try {
      ...
      const response = await this.dynamicsWebApi.executeBatch();
      logger.info(
        "CRMGateway::markBookingCancelled: Booking status and Booking product cancel date updated",
        { bookingId, bookingProductId }
      );
      logger.debug("CRMGateway::markBookingCancelled:: Raw response", {
        response,
        bookingId,
      });
    } catch (error) {
  ...
  private async updateProductBookingAdditionalSupport(
    bookingProductId: string,
    additionalSupport: CRMAdditionalSupport
  ): Promise<CRMBookingProduct> {
    const request: DynamicsWebApi.UpdateRequest = {
      key: bookingProductId,
      collection: "ftts_bookingproducts",
      entity: {
    ...
    logger.debug(
      "CRMGateway::updateProductBookingAdditionalSupport: Raw Request",
      { request, bookingProductId }
    );
    return this.dynamicsWebApi.updateRequest(request);
  }

  private async updateBookingAdditionalSupport(
    bookingId: string,
    additionalSupport: CRMAdditionalSupport,
    isCSCBooking?: boolean
  ): Promise<CRMBookingResponse> {
    const request: DynamicsWebApi.UpdateRequest = {
      key: bookingId,
      collection: "ftts_bookings",
      entity: {
    ...
    logger.debug("CRMGateway::updateBookingAdditionalSupport: Raw Request", {
      request,
      bookingId,
    });
    return this.dynamicsWebApi.updateRequest(request);
  ...
  private async updateBookingProductVoiceover(
    bookingProductId: string,
    voiceover: CRMVoiceOver
  ): Promise<CRMBookingProduct> {
    const request: DynamicsWebApi.UpdateRequest = {
    ...
    logger.debug("CRMGateway::updateBookingProductVoiceover: Raw Request", {
      request,
      bookingProductId,
    });
    return this.dynamicsWebApi.updateRequest(request);
  ...
  private async updateCandidateRequest(
    candidateId: string,
    updatedCandidateDetails: Partial<CRMContact>
  ): Promise<CRMContact> {
    const request: DynamicsWebApi.UpdateRequest = {
      key: candidateId,
      collection: "contacts",
    ...
    logger.debug("CRMGateway::updateCandidateRequest: Raw Request", {
      request,
      candidateId,
    });
    return this.dynamicsWebApi.updateRequest(request);
  ...
  private async updateBookingVoiceover(
    bookingId: string,
    voiceover: CRMVoiceOver,
    isCSCBooking?: boolean
  ): Promise<CRMBookingResponse> {
    ...
    logger.debug("CRMGateway::updateBookingVoiceover: Raw Request", {
      request,
      bookingId,
    });
    return this.dynamicsWebApi.updateRequest(request);
  ...
  private async updateOwedCompensationBookingRecognised(
    bookingId: string,
    owedCompensationBookingRecognised: string
  ): Promise<void> {
    const request: DynamicsWebApi.UpdateRequest = {
    ...
    logger.debug(
      "CRMGateway:updateOwedCompensationBookingRecognised: Raw Request",
      { request, bookingId }
    );
    return this.dynamicsWebApi.updateRequest(request);
  }

  private async getPaymentInformationForBookingDetails(
    ...
    const filteredBookings = bookingDetails.filter(
      (booking) =>
        booking.bookingStatus === CRMBookingStatus.CancellationInProgress
    );
    ...
    try {
      ...
      logger.debug(
        "CRMGateway::getPaymentInformationForBookingDetails: Raw Response",
        { results }
      );
      ...
      results.forEach((result) => {
        ...
        updatedBookingDetails = updatedBookingDetails.map((bookingDetail) => {
          ...
          if (
            bookingDetail.bookingProductId ===
            result.value[0]._ftts_bookingproduct_value
          ) {
            return mapPaymentInformationToBookingDetails(
              result.value[0],
              bookingDetail
            );
          }
      ...
      logger.error(
        error,
        `CRMGateway::getPaymentInformationForBookingDetails: Could not retrieve payment information for bookings that have the status Cancellation in Progress- ${error.message}`
      );
      throw error;
    }
  }
  ...
  private async createBookingRequest(
    ...
    const request: DynamicsWebApi.CreateRequest = {
      collection: "ftts_bookings",
      entity: mapToCRMBooking(
        candidate,
        booking,
        candidate.candidateId as string,
        candidate.licenceId as string,
        additionalSupport,
        isStandardAccommodation,
        priceListId
      ),
  ...
  private async getNsaBookingSlots(
    nsaBookings: CRMXmlBookingDetails[]
  ): Promise<CRMNsaBookingSlots[] | undefined> {
    ...
    nsaBookings.forEach((nsaBooking: CRMXmlBookingDetails) => {
      ...
      filterQuery = filterQuery.concat(
        `<condition attribute="ftts_bookingid" operator="eq" value="${bookingId}" />\n`
      );
    });

    const nsaBookingSlotQuery = rawNsaBookingSlotXml.replace(
      "${bookingidConditions}",
      filterQuery
    );
    ...
    const response: FetchXmlResponse<CRMNsaBookingSlots> =
      await this.dynamicsWebApi.fetch(
        Collection.NSA_BOOKING_SLOTS,
        nsaBookingSlotQuery
      );
  ...
  public async getUserDraftNSABookingsIfExist(
    candidateId: string,
    testType: TestType
  ): Promise<NsaBookingDetail[] | undefined> {
    ...
    try {
      ...
      logger.debug("CRMGateway::getUserDraftNSABookingsIfExist: Raw Request", {
        xml,
        entity: Collection.BOOKING_PRODUCT,
      });
      const response = await this.dynamicsWebApi.fetch(
        Collection.BOOKING_PRODUCT,
        xml
      );
      logger.debug("CRMGateway::getUserDraftNSABookingsIfExist: Raw Response", {
        response,
        entity: Collection.BOOKING_PRODUCT,
        candidateId,
      });
      if (!response.value?.length) {
        logger.debug(
          "CRMGateway::getUserDraftNSABookingsIfExist: No draft bookings found for candidate",
          { candidateId }
        );
        return undefined;
      }
      ...
      this.logGeneralError(
        error,
        "CRMGateway::getUserDraftNSABookingsIfExist: Could not retrieve NSA draft booking from CRM",
        { candidateId }
      );
      throw error;
    }
  ...
  public async doesCandidateHaveExistingBookingsByTestType(
    candidateId: string,
    testType: TestType
  ): Promise<boolean> {
    try {
      const rawXml = await this.readFile(
        "getCandidateBookingsByCurrentTestType.xml"
      );
      const xml = rawXml
        .replace("${candidateId}", candidateId)
        .replace("${testType}", toCRMProductNumber(testType))
        .replace(
          ...
          CRMBookingStatus.Confirmed.toString()
        )
        .replace("${DATE_NOW}", new Date().toISOString());

      logger.debug(
        "CRMGateway::doesCandidateHaveExistingBookingsByTestType: Raw Request",
        {
          xml,
          entity: Collection.BOOKING_PRODUCT,
          candidateId,
      ...
      const response: FetchXmlResponse<string> =
        await this.dynamicsWebApi.fetch(Collection.BOOKING_PRODUCT, xml);
      logger.debug(
        "CRMGateway::doesCandidateHaveExistingBookingsByTestType: Raw Response",
        {
          response,
          entity: Collection.BOOKING_PRODUCT,
          candidateId,
      ...
      if (!response.value?.length) {
        logger.debug(
          "CRMGateway::doesCandidateHaveExistingBookingsByTestType: No bookings found with existing test type for candidate",
      ...
      this.logGeneralError(
        error,
        "CRMGateway::doesCandidateHaveExistingBookingsByTestType: Could not retrieve candidate bookings from CRM",
        { candidateId, testType }
      );
      throw error;
    }
  ...
  public async getRescheduleCount(bookingId: string): Promise<number> {
    try {
      ...
      const response =
        await this.dynamicsWebApi.retrieveRequest<CRMBookingRescheduleCountResponse>(
          request
        );
      logger.debug("CRMGateway::getRescheduleCount: Raw Response", {
        response,
      });

      if (!response) {
        throw new Error(
          "CRMGateway::getRescheduleCount: Empty response from CRM"
        );
      }
      ...
      logger.error(
        error as Error,
        "CRMGateway::getRescheduleCount: Failed to get reschdule count from CRM",
        { bookingId }
      );
      throw error;
    }
  ...
  private async readFile(fileName: string): Promise<string> {
    ...
    return fs.promises.readFile(
      `./src/services/crm-gateway/data/${fileName}`,
      "utf-8"
    );
  }

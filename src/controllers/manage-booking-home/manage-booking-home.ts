...
import {
  CRMNsaBookingSlots,
  NsaBookingDetail,
} from "../../services/crm-gateway/interfaces";
...
export class ManageBookingHomeController {
  ...
  public get = async (req: Request, res: Response): Promise<void> => {
    if (!req.session.manageBooking) {
      throw new Error(
        "ManageBookingHomeController::get: No Manage Booking Session"
      );
    }
    ...
    const bookings = store.manageBooking
      .getBookings(req)
      .filter((booking) => booking.isViewable())
      .sort(byTestDateSoonestFirst);

    const { validBookings, bookingsWithErrors } =
      await this.findInvalidChangeInProgressBookings(bookings);

    const nsaBookingDetails: NsaBookingDetail[] =
      this.retrieveNsaBookingDetails(validBookings);
    const compensationEligibleNotificationLink =
      this.retrieveCompensationEligibleLink(
        validBookings,
    ...
    return res.render("manage-booking/home", {
      ...
      heading: bookings.length
        ? translate("manageBookingHome.header")
        : translate("manageBookingHome.noBookings"),
  ...
  private findInvalidChangeInProgressBookings = async (
    bookings: Booking[]
  ): Promise<{ validBookings: Booking[]; bookingsWithErrors: Booking[] }> => {
    const changeInProgressBookings = bookings.filter((booking) =>
      booking.isChangeInProgress()
    );
    const statusOfChangeInProgressBookings = await Promise.all(
      changeInProgressBookings.map(this.validateChangeInProgressBooking)
    );
    ...
    const validBookings = bookings.filter(
      (booking) => !bookingRefsWithErrors.includes(booking.details.reference)
    );
    const bookingsWithErrors = bookings.filter((booking) =>
      bookingRefsWithErrors.includes(booking.details.reference)
    );
    return {
      validBookings,
      bookingsWithErrors,
  ...
  private validateChangeInProgressBooking = async (
    changeInProgressBooking: Booking
  ): Promise<ChangeInProgressBookingTCNStatus> => {
    const {
      reference: bookingRef,
      bookingProductRef,
      testCentre,
    } = changeInProgressBooking.details;
    try {
      const booking = await this.scheduling.getBooking(
        bookingProductRef,
        testCentre.region
      );
      if (booking) {
  ...
  private retrieveCompensationEligibleLink(
    bookings: Booking[],
    target: Target
  ): string {
    const compensationEligibleBookings = bookings.filter((booking) =>
    ...
    if (
      compensationEligibleBookings.length > 1 &&
    ...
    const isInstructorBooking = INSTRUCTOR_TEST_TYPES.includes(
      fromCRMProductNumber(
        compensationEligibleBookings[0].details.product
          ?.productnumber as CRMProductNumber
      )
    );
  ...
  private isBookingTypeMixed(bookings: Booking[]): boolean {
    const instructorBookings = bookings.filter((booking) =>
      INSTRUCTOR_TEST_TYPES.includes(
        fromCRMProductNumber(
    ...
    return !(
      instructorBookings.length === bookings.length ||
  ...
  private retrieveNsaBookingDetails(bookings: Booking[]): NsaBookingDetail[] {
    ...
    const checkValidNsaBookingSlots = (
      nsaBookingSlots: CRMNsaBookingSlots
    ): boolean =>
      nsaBookingSlots.ftts_status === CRMNsaBookingSlotStatus.Offered;
    ...
    nsaBookingDetails = bookings
      ...
      .map((booking: Booking) => {
        const bookingDetails = booking.details;
        const canViewSlots: boolean =
          bookingDetails.nsaBookingSlots?.filter(checkValidNsaBookingSlots)
            .length === 3 &&
          bookingDetails.nsaStatus ===
            CRMNsaStatus.AwaitingCandidateSlotConfirmation; // NSA status needed to allow viewing of slots
        return {
          bookingId: bookingDetails.bookingId,
          nsaStatus: mapCRMNsaStatusToNSAStatus(
            bookingDetails.nsaStatus as CRMNsaStatus
          ),
          canViewSlots,
        };
      });

  ...
  describe("isValid", () => {
    describe("is true when", () => {
      ...
      validDates.forEach((validInput) => {
        test(`${validInput.dobDay}-${validInput.dobMonth}-${validInput.dobYear}`, () => {
          ...
          expect(DateOfBirth.isValid(validInput.dobDay, { req } as Meta)).toBe(
            true
          );
        });
      });
    ...
    describe("is false when", () => {
      const invalidDates: Array<[any, string]> = [
        ...
        [
          { dobDay: "32", dobMonth: "01", dobYear: "1990" },
          "day does not exist",
        ],
        [
          { dobDay: "29", dobMonth: "02", dobYear: "2021" },
          "day does not exist - not a leap year",
        ],
      ];
      ...
      invalidDates.forEach((invalidInput) => {
        test(`${invalidInput[1]}`, () => {
          ...
          expect(() =>
            DateOfBirth.isValid(invalidInput[0].dobDay, { req } as Meta)
          ).toThrow();
        });
      });
    });

...
function multipartMixedMiddleware(req: any, res: any, next: any) {
  if (
    ...
    req.headers["content-type"].includes("multipart/mixed")
  ) {
    let data = "";
...
(router as any).render = (req: any, res: any) => {
  ...
  if (url.includes("/test-centres") === true && req.method === "GET") {
    const parsedQuery: ParsedUrlQuery = querystring.parse(
    ...
    if (searchTerm === "errorUnknownError") {
      ...
      searchTerm &&
      (searchTerm.includes("<script>") === true ||
        searchTerm.includes("warningZeroResults") === true)
    ) {
      ...
      const testCentreData = testCentresClone;
      testCentreData.testCentres = testCentresClone.testCentres.slice(
        0,
        numOfResults
      );
      res.status(200).jsonp(testCentreData);
    ...
    if (testCentre) {
      while (i < days) {
        ...
        const dayOfTheWeek = slotTime.toLocaleString("en-gb", {
          weekday: "long",
        });
    ...
    const slotTime = new Date(startDateTime).toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
    });
    if (slotTime === "11:00") {
    ...
    url.includes(`/gateway/${receiptReference}/complete`) &&
    req.method === "PUT"
  ) {
    res.status(200).jsonp({
      code: 801,
    ...
    url.includes("/crm/ftts_bookingproducts") &&
    req.method === "PATCH"
  ) {
    res.status(204).jsonp({});
    ...
    url.includes("/crm/ftts_GetClearWorkingDay") &&
    req.method === "POST"
  ) {
    ...
    if (req.body.includes("/crm/ftts_bookings")) {
      ...
      req.body.includes("ftts_licences") &&
      req.body.includes("contacts")
    ) {
      res.status(200).jsonp([
        {
          value: [
      ...
      req.body.includes("ftts_bookingproducts") &&
      req.body.includes("ftts_testlanguage")
    ) {
      console.log("req.body", req.body);
      res.status(204).jsonp({});
    } else if (
      req.body.includes("ftts_bookingproducts") &&
      req.body.includes("ftts_additionalsupportoptions")
    ) {
      console.log("req.body", req.body);
      res.status(204).jsonp({});
    } else if (
      req.body.includes("ftts_bookingproducts") &&
      req.body.includes("ftts_voiceoverlanguage")
    ) {
      console.log("req.body", req.body);
      res.status(204).jsonp({});
    } else {
    ...
    if (drivingLicence && drivingLicence[1] === "ZZZZZ061102W97YT") {
      // no existing licence
      res.status(200).jsonp({
        value: [],
      });
      ...
      drivingLicence &&
      (drivingLicence[1] === "AAAAA061102W97YT" ||
        drivingLicence[1].startsWith("TESTR252244N9"))
    ) {
      // existing licence retrieved
      res.status(200).jsonp({
        value: [
          {
            ftts_licenceid: "11111111-75ca-ea11-a812-00224801cecd",
    ...
    } else if (drivingLicence && drivingLicence[1] === "BBBBB061102W97YT") {
      // existing licence retrieved
      res.status(200).jsonp({
        value: [
          {
    ...
    } else if (drivingLicence && drivingLicence[1] === "JONES061102W97YT") {
      // existing licence retrieved
      res.status(200).jsonp({
        value: [
          {
    ...
    } else if (drivingLicence && drivingLicence[1] === "06159200") {
      // existing licence retrieved
      res.status(200).jsonp({
        value: [
          {
    ...
    } else if (drivingLicence && drivingLicence[1] === "CCCCC061102W97YT") {
      // existing licence retrieved
      res.status(200).jsonp({
        value: [
          {
    ...
    } else if (drivingLicence && drivingLicence[1] === "DDDDD061102W97YT") {
      // existing licence retrieved
      res.status(200).jsonp({
        value: [
          {
    ...
    } else if (drivingLicence && drivingLicence[1] === "EEEEE061102W97YT") {
      // existing licence retrieved
      res.status(200).jsonp({
        value: [
          {
    ...
    } else if (drivingLicence && drivingLicence[1] === "FFFFF061102W97YT") {
      // existing licence retrieved
      res.status(200).jsonp({
        value: [
          {
    ...
    } else if (drivingLicence && drivingLicence[1] === "GGGGG061102W97YT") {
      // existing licence retrieved
      res.status(200).jsonp({
        value: [
          {
    ...
    } else if (drivingLicence && drivingLicence[1].startsWith("9463719")) {
      // existing NI licence retrieved
      res.status(200).jsonp({
        value: [
          {
            ftts_licenceid: "11111111-75ca-ea11-a812-00224801cecd",
    ...
    } else if (drivingLicence && drivingLicence[1] === "55667788") {
      // existing NI licence retrieved
      res.status(200).jsonp({
        value: [
          {
    ...
    } else if (drivingLicence && drivingLicence[1] === "74563326") {
      // instructor NI driving licence
      res.status(200).jsonp({
        value: [
          {
    ...
    } else if (drivingLicence && drivingLicence[1] === "PAULF152143RS8IV") {
      // instructor GB driving licence
      res.status(200).jsonp({
        value: [
          {
    ...
    const filter = String(parsedQuery.$filter);
    const candidateId = RegExp("_ftts_candidateid_value eq (.*) and").exec(
      filter
    );

    if (candidateId && candidateId[1].startsWith("11111111")) {
      const singleFutureGbBooking = mockData.bookingsSingleFutureGB
        .value as CRMBookingDetails[];
      singleFutureGbBooking[0].ftts_testdate = getFutureDate(
        "month",
        4
      ).toISOString();
      res.status(200).jsonp(mockData.bookingsSingleFutureGB);
    } else if (candidateId && candidateId[1].startsWith("88888888")) {
      const singleFutureNiBooking = mockData.bookingsSingleFutureNI
        .value as CRMBookingDetails[];
      singleFutureNiBooking[0].ftts_testdate = getFutureDate(
        "month",
        4
      ).toISOString();
      res.status(200).jsonp(mockData.bookingsSingleFutureNI);
    } else if (candidateId && candidateId[1].startsWith("22222222")) {
      const multipleFutureGbBookings = mockData.bookingsMultipleFutureGB
        .value as CRMBookingDetails[];
      multipleFutureGbBookings[0].ftts_testdate = getFutureDate(
        "day",
        1
      ).toISOString();
      multipleFutureGbBookings[1].ftts_testdate = getFutureDate(
        "day",
        3
      ).toISOString();
      multipleFutureGbBookings[2].ftts_testdate = getFutureDate(
        "hour",
        1
      ).toISOString();
      multipleFutureGbBookings[3].ftts_testdate = getFutureDate(
        "month",
        4
      ).toISOString();
      res.status(200).jsonp(mockData.bookingsMultipleFutureGB);
    } else if (candidateId && candidateId[1].startsWith("99999999")) {
      const multipleFutureBookingsNi = mockData.bookingsMultipleFutureNI
        .value as CRMBookingDetails[];
      multipleFutureBookingsNi[0].ftts_testdate = getFutureDate(
        "day",
        1
      ).toISOString();
      multipleFutureBookingsNi[1].ftts_testdate = getFutureDate(
        "day",
        3
      ).toISOString();
      multipleFutureBookingsNi[2].ftts_testdate = getFutureDate(
        "hour",
        1
      ).toISOString();
      multipleFutureBookingsNi[3].ftts_testdate = getFutureDate(
        "month",
        4
      ).toISOString();
      res.status(200).jsonp(mockData.bookingsMultipleFutureNI);
    } else if (candidateId && candidateId[1].startsWith("33333333")) {
      const cscPaymentSuccessBooking = mockData.bookingsCSCPaymentSuccess
        .value as CRMBookingDetails[];
      cscPaymentSuccessBooking[0].ftts_testdate = getFutureDate(
        "month",
        4
      ).toISOString();
      res.status(200).jsonp(mockData.bookingsCSCPaymentSuccess);
    } else if (candidateId && candidateId[1].startsWith("44444444")) {
      const cscPaymentFailureBooking = mockData.bookingsCSCPaymentFailure
        .value as CRMBookingDetails[];
      cscPaymentFailureBooking[0].ftts_testdate = getFutureDate(
        "month",
        4
      ).toISOString();
      ...
      const errorHandlingBooking = mockData.bookingsErrorHandling
        .value as CRMBookingDetails[];
      errorHandlingBooking[0].ftts_testdate = getFutureDate(
        "month",
        4
      ).toISOString();
      res.status(200).jsonp(mockData.bookingsErrorHandling);
    } else if (candidateId && candidateId[1].startsWith("11112222")) {
      const multipleFutureGbInstructorBooking = mockData
        .bookingsMultipleFutureInstructorGB.value as CRMBookingDetails[];
      multipleFutureGbInstructorBooking[0].ftts_testdate = getFutureDate(
        "month",
        4
      ).toISOString();
      multipleFutureGbInstructorBooking[1].ftts_testdate = getFutureDate(
        "month",
        4
      ).toISOString();
      multipleFutureGbInstructorBooking[2].ftts_testdate = getFutureDate(
        "month",
        4
      ).toISOString();
      res.status(200).jsonp(mockData.bookingsMultipleFutureInstructorGB);
    } else if (candidateId && candidateId[1].startsWith("33334444")) {
      const multipleFutureNiInstructorBooking = mockData
        .bookingsMultipleFutureInstructorNI.value as CRMBookingDetails[];
      multipleFutureNiInstructorBooking[0].ftts_testdate = getFutureDate(
        "month",
        4
      ).toISOString();
      multipleFutureNiInstructorBooking[1].ftts_testdate = getFutureDate(
        "month",
        4
      ).toISOString();
    ...
    if (drivingLicenceNumber.includes("JONES061102W97YT")) {
      ...
      mockData.eligGlenWilliamNi.eligibilities[0].eligibleTo = getFutureDate(
        "month",
        2,
        false
      ).format("YYYY-MM-DD");
      ...
      mockData.eligAbdurRahmanBentonGb.eligibilities[0].eligibleFrom =
        getFutureDate("month", 3, false).format("YYYY-MM-DD");
      mockData.eligAbdurRahmanBentonGb.eligibilities[0].eligibleTo =
        getFutureDate("year", 5, false).format("YYYY-MM-DD");

...
class SchedulingGateway {
  private static instance: SchedulingGateway = new SchedulingGateway(
    new ManagedIdentityAuth(config.scheduling.identity)
  );
  ...
  constructor(
    ...
    private axiosRetryClient_Retrieve = new AxiosRetryClient({
      ...defaultRetryPolicy,
      maxRetries: maxRetriesRetrieve,
    }).getClient(),
    private axiosRetryClient_Reserve = new AxiosRetryClient({
      ...defaultRetryPolicy,
      maxRetries: maxRetriesReserve,
    }).getClient(),
    private axiosRetryClient_Confirm = new AxiosRetryClient({
      ...defaultRetryPolicy,
  ...
  public async getBooking(
    ...
    testCentreRegion: string
  ): Promise<BookingFullResponse> {
    try {
      const authHeader = await this.auth.getAuthHeader();
      const requestUrl = `${this.apiUrl}/tcn/${testCentreRegion}/bookings/${bookingProductRef}`;
      logger.debug(
        "SchedulingGateway::getBooking: Attempting to retrieve booking",
        {
          requestUrl,
          bookingProductRef,
        }
      );
      const response = await this.axiosRetryClient_Retrieve.get(
        requestUrl,
        authHeader
      );
  ...
  public async availableSlots(
    ...
    try {
      logger.debug(
        `SchedulingGateway::availableSlots: Attempting to get available slots for ${centre.name}`,
        {
      ...
      const slots = await this.axiosRetryClient_Retrieve.get<AppointmentSlot[]>(
        requestUrl,
        authHeader
      );
      ...
      logger.error(
        error,
        "SchedulingGateway::availableSlots: Call to scheduling api failed",
        {
  ...
  public async reserveSlot(
    ...
    try {
      ...
      const response = await this.axiosRetryClient_Reserve.post<
        ReservationResponse[]
      >(
        `${this.apiUrl}/tcn/${centre.region}/reservations`,
        reservationsRequest,
        authHeader
      );
      ...
      logger.error(
        error,
        "SchedulingGateway::reserveSlot: reserve slot failed",
        {
          ...businessIdentifiers,
        }
      );
      if (error.response?.status === 409) {
        logger.event(
          BusinessTelemetryEvents.SCHEDULING_SLOT_EXP,
          "SchedulingGateway::reserveSlot: Received an expired response from Scheduling API",
          {
            ...businessIdentifiers,
          }
        );
  ...
  public async confirmBooking(
    ...
    const authHeader = await this.auth.getAuthHeader();
    const reservationId = bookingRequest.map(
      (booking) => booking.reservationId
    );
    const bookingRefId = bookingRequest.map(
      (booking) => booking.bookingReferenceId
    );

    logger.debug(
      "SchedulingGateway::confirmBooking: Attempting to confirm the following bookings",
      {
    ...
    const bookingResponse = await this.axiosRetryClient_Confirm.post<
      BookingResponse[]
    >(`${this.apiUrl}/tcn/${region}/bookings`, bookingRequest, authHeader);
  ...
  public async deleteBooking(
    ...
    try {
      ...
      const requestUrl = `${this.apiUrl}/tcn/${testCentreRegion}/bookings/${bookingProductRef}`;
      logger.debug(
        "SchedulingGateway::deleteBooking: Attempting to delete booking",
        {
          requestUrl,
          bookingProductRef,
          testCentreRegion,
      ...
      logger.debug(
        "SchedulingGateway::deleteBooking: Successfully deleted booking",
        {
          bookingProductRef,
        }
      );
  ...
  public async deleteReservation(
    ...
    try {
      ...
      logger.debug(
        "SchedulingGateway::deleteReservation: Attempting to delete reservation",
        {
          requestUrl,
          reservationId,
          testCentreRegion,
      ...
      await this.axiosRetryClient.delete(requestUrl, authHeader);
      logger.debug(
        "SchedulingGateway::deleteReservation: Successfully deleted reservation",
        {
          reservationId,
        }
      );
      ...
      if (error.response?.status === 404) {
        logger.warn(
          "SchedulingGateway::deleteReservation: failed to delete non-existing reservation",
          {
            reservationId,
            bookingProductRef,
          }
        ...
        logger.error(
          error,
          "SchedulingGateway::deleteReservation: Delete failed",
          {
            reservationId,
            bookingProductRef,
          }
  ...
  private toTestTypeCode(testType: TestType): string {
    ...
    if (!testTypeCode) {
      throw new Error(
        `SchedulingGateway::toTestTypeCode: No scheduling mapping for TestType '${testType}'`
      );
    }
    return testTypeCode;
  }

  logEventByProductRef(
    error: AxiosError,
    bookingProductRef: string,
    func: string
  ): void {
    ...
    if (!status) {
      logger.error(
        error as Error,
        `SchedulingGateway::logEventByProductRef::${func}: Not an axios error`
      );
    } else if (status === 401 || status === 403) {
      logger.event(
        BusinessTelemetryEvents.SCHEDULING_AUTH_ISSUE,
        `SchedulingGateway::${func}: Failed to authenticate to the scheduling api`,
        {
          bookingProductRef,
        }
      );
      ...
      logger.event(
        ...
        `SchedulingGateway::${func}: Failed to get request from Scheduling api`,
        {
          bookingProductRef,
        }
      );
      ...
      logger.event(
        ...
        `SchedulingGateway::${func}: Failed to communicate with the scheduling API server`,
        {
          bookingProductRef,
        }
      );
  ...
  logEventByIdentifiers(
    error: AxiosError,
    businessIdentifiers: Record<string, unknown>,
    func: string
  ): void {
    const status = error.response?.status;
    if (!status) {
      logger.error(
        error as Error,
        `SchedulingGateway::logEventByIdentifiers::${func}: Not an axios error`
      );
    } else if (status === 401 || status === 403) {
      logger.event(
        BusinessTelemetryEvents.SCHEDULING_AUTH_ISSUE,
...
export { SchedulingGateway, ReservationResponse, SlotUnavailableError };

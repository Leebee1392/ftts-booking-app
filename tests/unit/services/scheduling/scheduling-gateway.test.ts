...
const mockedAxiosRetrieve =
  jest.createMockFromModule<jest.Mocked<AxiosStatic>>("axios");
const mockedAxiosReserve =
  jest.createMockFromModule<jest.Mocked<AxiosStatic>>("axios");
const mockedAxiosConfirm =
  jest.createMockFromModule<jest.Mocked<AxiosStatic>>("axios");
...
describe("SchedulingGateway", () => {
  ...
  beforeEach(() => {
    scheduling = new SchedulingGateway(
      mockSchedulingAuth as any,
      mockedAxios,
      mockedAxiosRetrieve,
      mockedAxiosReserve,
      mockedAxiosConfirm,
  ...
  describe("availableSlots", () => {
    ...
    test("will request slots between given dateFrom and dateTo", async () => {
      const result = await scheduling.availableSlots(
        "2020-01-04",
        "2020-01-10",
        mockCentre,
        mockTestType
    ...
    test("will return kpi data if preferred date is provided", async () => {
      mockedAxiosRetrieve.get.mockImplementationOnce(() =>
        Promise.resolve({
          data: [
            {
    ...
    test.each([
      ...
      "will send test type %s when given test type %s",
      async (expectedTestType: string, givenTestType: TestType) => {
        const result = await scheduling.availableSlots(
          "2020-01-04",
          "2020-01-10",
          mockCentre,
  ...
  describe("reserveSlot", () => {
    test("successfully reserves a slot", async () => {
      ...
      const result = await scheduling.reserveSlot(
        mockCentre,
        mockTestType,
        mockStartDateTime
      );
    ...
    test("throws a SlotInvalidError and does not query scheduling API if slot is invalid", async () => {
      await expect(
        scheduling.reserveSlot(mockCentre, mockTestType, "invalid")
      ).rejects.toThrow(SlotInvalidError);
    ...
    test("throws a SlotUnavailableError if the response is 409 Conflict", async () => {
      ...
      await expect(
        scheduling.reserveSlot(mockCentre, mockTestType, mockStartDateTime)
  ...
  describe("confirmBooking", () => {
    test("successfully confirms a booking", async () => {
      ...
      const result = await scheduling.confirmBooking(
        BookingRequestMock,
        mockRegion
      );
  ...
  describe("deleteBooking", () => {
    ...
    test("throws an error if something goes wrong", async () => {
      ...
      await expect(
        scheduling.deleteBooking(mockBookingProductRef, mockRegion)
      ).rejects.toEqual(mockedError);

      expect(logger.error).toHaveBeenCalledTimes(1);
      expect(logger.error).toHaveBeenCalledWith(
        mockedError,
        "SchedulingGateway::deleteBooking: Delete failed",
        {
  ...
  describe("deleteReservation", () => {
    test("successfully deletes a reservation", async () => {
      await scheduling.deleteReservation(
        mockReservationId,
        mockRegion,
        mockBookingProductRef
      );
  ...
  describe("getBooking", () => {
    test("successfully gets a booking", async () => {
      mockedAxiosRetrieve.get.mockResolvedValue({
        data: BookingResponseFullMock,
      });

      const result = await scheduling.getBooking(
        mockBookingProductRef,
        mockRegion
      );
    ...
    test("throws an error if something goes wrong", async () => {
      ...
      await expect(
        scheduling.getBooking(mockBookingProductRef, mockRegion)
      ).rejects.toEqual(mockedError);
      expect(logger.error).toHaveBeenCalledWith(
        mockedError,
  ...
  describe("logEventByProductRef", () => {
    ...
    test("handles a 401 error", async () => {
      ...
      scheduling.logEventByProductRef(
        error as CustomAxiosError,
        bookingProductRef,
        "fakeFunction"
      );
  ...
  describe("logEventByIdentifiers", () => {
    ...
    test("handles a 401 error", async () => {
      ...
      scheduling.logEventByIdentifiers(
        error as CustomAxiosError,
        identifiers,
        "fakeFunction"
      );

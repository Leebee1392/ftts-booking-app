  ...
  describe("Get test centres", () => {
    test("fetches GB test centres", async () => {
      ...
      expect(mockedAxios.get).toHaveBeenCalledWith(
        "v1/test-centres?region=gb&term=testterm&numberOfResults=6",
        {
          headers: {
    ...
    test("fetches NI test centres", async () => {
      ...
      expect(mockedAxios.get).toHaveBeenCalledWith(
        "v1/test-centres?region=ni&term=testterm&numberOfResults=7",
        {
          headers: {
            Authorization: `Bearer ${mockAccessToken}`,
          },
  ...
  describe("error handling", () => {
    test("400 errors", async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        error,
        "LocationsGateway::fetchCentres: Error retrieving test centres from locations api. Term: testterm Region: gb"
      );
    });
    ...
    test("401 errors", async () => {
      ...
      await expect(locations.fetchCentres("testterm", "gb", 5)).rejects.toEqual(
        error
      );
      expect(logger.event).toHaveBeenCalledWith(
    ...
    test("non-400 errors", async () => {
      ...
      await expect(
        locations.fetchCentres("testterm", "gb", 5)
      ).rejects.toThrow();
    });
  });
});

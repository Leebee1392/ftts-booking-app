...
const invertMap = <Key, Value>(map: Map<Key, Value>): Map<Value, Key> =>
  new Map(Array.from(map, (e) => [e[1], e[0]]));

export const toCRMTestType = (testType: TestType): CRMTestType =>
  TEST_TYPE_TO_CRM_TEST_TYPE.get(testType) as CRMTestType;

export const fromCRMTestType = (crmTestType: CRMTestType): TestType =>
  CRM_TEST_TYPE_TO_TEST_TYPE.get(crmTestType) as TestType;

export const toTestEngineTestType = (
  testType: TestType
): TestEngineTestType => {
  const crmTestType = toCRMTestType(testType);
  return CRM_TEST_TYPE_TO_TEST_ENGINE_TEST_TYPE.get(
    crmTestType
  ) as TestEngineTestType;
};

export const toCRMProductNumber = (testType: TestType): CRMProductNumber =>
  TEST_TYPE_TO_CRM_PRODUCT_NUMBER.get(testType) as CRMProductNumber;

export const fromCRMProductNumber = (
  crmProductNumber: CRMProductNumber
): TestType =>
  CRM_PRODUCT_NUMBER_TO_TEST_TYPE.get(crmProductNumber) as TestType;

export const fromCRMOrigin = (crmOrigin: CRMOrigin): Origin =>
  CRM_ORIGIN_TO_ORIGIN.get(crmOrigin) as Origin;
...
const CRM_TEST_TYPE_TO_TEST_ENGINE_TEST_TYPE: Map<
  CRMTestType,
  TestEngineTestType
> = new Map([
...
const TEST_TYPE_TO_CRM_PRODUCT_NUMBER: Map<TestType, CRMProductNumber> =
  new Map([
...
const CRM_PRODUCT_NUMBER_TO_TEST_TYPE = invertMap(
  TEST_TYPE_TO_CRM_PRODUCT_NUMBER
);

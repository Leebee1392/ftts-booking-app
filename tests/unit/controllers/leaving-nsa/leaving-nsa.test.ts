import { LeavingNSAController } from "@controllers/leaving-nsa/leaving-nsa";
...
describe("LeavingNSAController", () => {
  ...
  describe("get", () => {
    test("it should render the correct page", () => {
      ...
      expect(res.render).toHaveBeenCalledWith("supported/leaving-nsa", {
  ...
  describe("post", () => {
    ...
    test("when continuing with instructor standard accomodations and the user already has NSA request", async () => {
      ...
      expect(res.redirect).toHaveBeenCalledWith(
        "/instructor/received-support-request"
      );
    });
  });
});

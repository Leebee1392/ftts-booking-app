...
test("Verify page title and heading are displayed correctly", async () => {
  ...
  await verifyTitleContainsText(
    `${chooseAppointmentPage.pageHeading} ${generalTitle}`
  );
...
test("Verify page of appointment times is rendered correctly on the browser", async (t) => {
  ...
  await preferredDatePage.selectPreferredDateWithAppointments(
    mySessionData.testCentreSearch
  );
  await verifyTitleContainsText(
    `${chooseAppointmentPage.pageHeading} ${generalTitle}`
  );
  ...
  await verifyExactText(
    chooseAppointmentPage.changeTestCenterLocator,
    chooseAppointmentPage.changeTestCentreText
  );
  await verifyExactText(
    chooseAppointmentPage.testCentreNameLocator,
    mySessionData.currentBooking.centre.name
  );

  // check preferred date is the selected date
  const currentSelectedDate = await getText(chooseAppointmentPage.selectedDate);

  const month = dayjs(mySessionData.testCentreSearch.selectedDate).format(
    "MMMM"
  );
  const dayOfWeek = dayjs(mySessionData.testCentreSearch.selectedDate).format(
    "dddd"
  );
  const dayOfMonth = dayjs(mySessionData.testCentreSearch.selectedDate).format(
    "D"
  );
  ...
  const slotHeader = await getText(
    chooseAppointmentPage.morningAfternoonSlotsHeader,
    0
  );
  ...
  const appointmentTabCount = await createSelector(
    chooseAppointmentPage.appointmentDayTabs
  ).filterVisible().count;
...
test("Verify a warning message is displayed if my preferred date has no appointment times available", async () => {
  ...
  await verifyExactText(
    chooseAppointmentPage.warningMessageLocator,
    chooseAppointmentPage.noSlotsAvailableWarningText
  );
});
...
test("Verify clicking the 'previous' link shows the previous set of available appointment times", async (t) => {
  ...
  const currentSelectedDate: string = await createSelector(
    chooseAppointmentPage.selectedDate
  ).getAttribute("href");
  const currentSelectedDateObj = new Date(
    dateRegex.exec(currentSelectedDate)[1]
  );

  if (t.browser.platform.toLowerCase() === "mobile") {
    // check we are moving back by 1-3 days (because on mobile if it hits the start of the week it will stop on Monday)
    const prevLinkDate: string = await Selector("a")
      .withExactText(chooseAppointmentPage.prevDayLinkText)
      .getAttribute("href");
    const prevLinkDateObj = new Date(dateRegex.exec(prevLinkDate)[1]);
    const diff =
      (currentSelectedDateObj.getTime() - prevLinkDateObj.getTime()) /
      (1000 * 3600 * 24);
    ...
    const prevLinkDate: string = await Selector("a")
      .withExactText(chooseAppointmentPage.prevWeekLinkText)
      .getAttribute("href");
    const prevLinkDateObj = new Date(dateRegex.exec(prevLinkDate)[1]);
  ...
  await t
    .expect(
      await createSelector(chooseAppointmentPage.selectedDate).getAttribute(
        "href"
      )
    )
    .notEql(currentSelectedDate, { timeout: MEDIUM_TIMEOUT });

  // check that the newly selected date is now before the old selected date
  const newlySelectedDate: string = await createSelector(
    chooseAppointmentPage.selectedDate
  ).getAttribute("href");
  const newlySelectedDateObj = new Date(dateRegex.exec(newlySelectedDate)[1]);

  await t
    .expect(newlySelectedDateObj.valueOf())
    .lt(currentSelectedDateObj.valueOf());
});
...
test("Verify clicking the 'next' link shows the next available set of appointment times", async (t) => {
  ...
  if (t.browser.platform.toLowerCase() === "mobile") {
    ...
    const nextLinkDate: string = await Selector("a")
      .withExactText(chooseAppointmentPage.nextDayLinkText)
      .getAttribute("href");
    const nextLinkDateObj = new Date(dateRegex.exec(nextLinkDate)[1]);
    const diff =
      (nextLinkDateObj.getTime() - currentSelectedDateObj.getTime()) /
      (1000 * 3600 * 24);
    await t.expect(diff).within(1, 3);
    ...
    const nextLinkDate: string = await Selector("a")
      .withExactText(chooseAppointmentPage.nextWeekLinkText)
      .getAttribute("href");
    const nextLinkDateObj = new Date(dateRegex.exec(nextLinkDate)[1]);
    const diff = Math.round(
      (nextLinkDateObj.getTime() - currentSelectedDateObj.getTime()) /
  ...
  await t
    .expect(newlySelectedDateObj.valueOf())
    .gt(currentSelectedDateObj.valueOf());
});
...
test("Verify a warning message is displayed if trying to view appointments times for a date in the past", async (t) => {
  ...
  await verifyExactText(
    chooseAppointmentPage.warningMessageLocator,
    chooseAppointmentPage.pastDaysWarningText
  );
});
...
test("Verify a warning message is displayed if trying to view appointments times for a date in the future", async (t) => {
  ...
  await verifyExactText(
    chooseAppointmentPage.warningMessageLocator,
    chooseAppointmentPage.futureDaysWarningText
  );
});

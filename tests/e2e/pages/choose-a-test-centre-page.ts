...
import { click, link, getText } from "../utils/helpers";
...
export class ChooseATestCentrePage extends BasePage {
  ...
  bannerText =
    "You can change the test location here or keep your existing choice.";
  ...
  async selectATestCentreWithName(
    testCentreName: string,
    centre: Centre
  ): Promise<PreferredDatePage> {
    const selectTestCentreButton = this.selectButtonWithTestCentreName.replace(
      "<>",
      testCentreName
    );
    const testCentreButton = Selector(selectTestCentreButton).nth(0);
    const testCentreIdValue = await testCentreButton.value;
    const testCentrePositionSelector = Selector(
      this.testCentrePosition.replace("<>", testCentreIdValue)
    );
  ...
  async getTestCentreAddress(centre: Centre, index: number): Promise<void> {
    ...
    const addressLineCount = await Selector(this.centreAddressLocator).nth(
      index
    ).childNodeCount;
    ...
    centre.addressPostalCode = await getText(
      this.centreAddressPostalCode,
      index
    );
  }
  ...
  async selectATestCentreFromMapTab(): Promise<PreferredDatePage> {
    if (t.browser.platform.toLowerCase() === "mobile") {
      await click(
        `${this.mapSelectedCentreDetails} ${this.mapSelectedCentreSelectButton}[class*=${this.mapSelectedCenterSelectButtonMobileClass}]`
      );
    } else {
      await click(
        `${this.mapSelectedCentreDetails} ${this.mapSelectedCentreSelectButton}[class*=${this.mapSelectedCenterSelectButtonDesktopClass}]`
      );
    }
    return new PreferredDatePage();
  }
  ...
  async see5MoreTestCentres(): Promise<void> {
    ...
    await t
      .expect(await getText(this.testCentreCount))
      .notEql(prevCount, { timeout: MEDIUM_TIMEOUT });
  }

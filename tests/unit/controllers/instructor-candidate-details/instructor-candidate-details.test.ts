...
import { InstructorCandidateDetailsController } from "@controllers/instructor-candidate-details/instructor-candidate-details";
...
describe("Instructor Candidate Details controller", () => {
  ...
  beforeEach(() => {
    ...
    eligibilityGateway.getEligibility = jest
      .fn()
      .mockResolvedValue(mockEligibilityResponse);
    ...
    candidateDetailsController = new InstructorCandidateDetailsController(
      new CandidateService(crmGateway as any, eligibilityGateway)
    );
  ...
  describe("POST", () => {
    describe("candidate exists in CRM", () => {
      test("corrects licence capitalisation and trims spaces before sending to eligibility", async () => {
        eligibilityGateway.getEligibility = jest
          .fn()
          .mockResolvedValue(mockEligibilityResponse);
        crmGateway.getLicenceNumberRecordsByCandidateId = jest
          .fn()
          .mockResolvedValue({
            ...mockCandidateResponse,
          });
        ...
        expect(eligibilityGateway.getEligibility).toHaveBeenCalledWith(
          "JONES061102W97YT",
          false,
          Target.GB,
          Locale.GB
      ...
      test("renders the test type page if in support mode, call to Eligibility is successful and candidate has a licence record", async () => {
        ...
        expect(
          ...
          mockEligibilityResponse.candidateId,
          mockLicence
        );
        expect(req.session.candidate).toStrictEqual({
          ...mockEligibilityResponse,
    ...
    describe("candidate doesn't exist in CRM", () => {
      test("renders the email page if call to Eligibility is successful", async () => {
        ...
        eligibilityGateway.getEligibility = jest
          .fn()
          .mockResolvedValue(mockEligibilityResponse);
        crmGateway.createCandidate = jest
          .fn()
          .mockResolvedValue(mockCandidateId);
    ...
    test("throws error if a call to CRM fails when creating candidate", async () => {
      ...
      await expect(
        candidateDetailsController.post(req, res)
      ).rejects.toStrictEqual(new CrmCreateUpdateCandidateError());
    });
    ...
    test("renders candidate details with an error if the eligibility call fails", async () => {
      eligibilityGateway.getEligibility = jest
        .fn()
        .mockRejectedValue(new EligibilityRetrieveError());
    ...
    test("handles 401 response from eligibility api", async () => {
      eligibilityGateway.getEligibility = jest
        .fn()
        .mockRejectedValue(new EligibilityAuthError());

      await expect(
        candidateDetailsController.post(req, res)
      ).rejects.toStrictEqual(new EligibilityAuthError());
    });

    test("handles 403 response from eligibility api", async () => {
      eligibilityGateway.getEligibility = jest
        .fn()
        .mockRejectedValue(new EligibilityAuthError("403"));

      await expect(
        candidateDetailsController.post(req, res)
      ).rejects.toStrictEqual(new EligibilityAuthError("403"));
    });

    test("handles 429 response from eligibility api", async () => {
      eligibilityGateway.getEligibility = jest
        .fn()
        .mockRejectedValue(new EligibilityTooManyRequestsError());

      await candidateDetailsController.post(req, res);

      expect(res.render).toHaveBeenCalledWith(
        "instructor/error-eligibility-retry"
      );
    });
    ...
    test("candidate books with an incorrect PRN", async () => {
      ...
      expect(res.render).toHaveBeenCalledWith("instructor/candidate-details", {
        backLink: "/instructor/choose-support",
        details: mockBody,
        support: false,
        errors: [
    ...
    test.each([
      ...
      "redirects correctly to the next page if the candidate's driving licence number is entered in %s",
      async (_, licenceNumber) => {
        const mockBodyWithNonUppercaseLicenceNmber = {
          ...mockBody,
          licenceNumber,
        };

        await candidateDetailsController.post(
          { ...req, body: mockBodyWithNonUppercaseLicenceNmber },
          res
        );
    ...
    test("redirects correctly to the next page if the candidate's dob is entered with no leading zeros and matches the eligibility dob", async () => {
      ...
      await candidateDetailsController.post(
        { ...req, body: mockBodyDobNoLeadingZeros },
        res
      );
    ...
    test("missing journey is caught", async () => {
      ...
      await expect(candidateDetailsController.post(req, res)).rejects.toThrow(
        Error("InstructorCandidateDetailsController::post: No journey set")
      );
    });
    ...
    describe("errors", () => {
      test("candidate has no eligible tests", async () => {
        ...
        expect(res.render).toHaveBeenCalledWith("no-eligibility");
      });
      ...
      test("eligibility api returns 500", async () => {
        eligibilityGateway.getEligibility = jest
          .fn()
          .mockRejectedValue(new EligibilityServerError("500"));
      ...
      test("eligibility api returns 599", async () => {
        eligibilityGateway.getEligibility = jest
          .fn()
          .mockRejectedValue(new EligibilityServerError("599"));
      ...
      test("eligibility api returns 409 (not most recent licence) - shows candidate details page with an error", async () => {
        eligibilityGateway.getEligibility = jest
          .fn()
          .mockRejectedValue(new EligibilityNotLatestLicenceError());

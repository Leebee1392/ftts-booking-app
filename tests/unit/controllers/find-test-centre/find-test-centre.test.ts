import { FindTestCentreController } from "@controllers/find-test-centre/find-test-centre";
...
describe("Find Test Centre controller", () => {
  ...
  describe("GET", () => {
    test("should render find-test-centre, first time", () => {
      ...
      expect(res.render).toHaveBeenCalledWith("common/find-test-centre", {
    ...
    describe("Back button link is generated", () => {
      ...
      test("back button takes user to the voiceover page if the user was shown voiceover screen", () => {
        ...
        req.session.currentBooking.selectStandardSupportType =
    ...
    test("missing journey on session", () => {
      ...
      expect(() => findTestCentreController.get(req, res)).toThrow(
        Error("FindTestCentreController::get: No journey set")
      );
    });
    ...
    test("missing booking on session", () => {
      ...
      expect(() => findTestCentreController.get(req, res)).toThrow(
        Error("FindTestCentreController::get: No currentBooking set")
      );
    });
  });
  ...
  describe("POST", () => {
    ...
    test("missing journey on session", () => {
      ...
      expect(() => findTestCentreController.post(req, res)).toThrow(
        Error("FindTestCentreController::post: No journey set")
      );
    });
    ...
    test("missing booking on session", () => {
      ...
      expect(() => findTestCentreController.post(req, res)).toThrow(
        Error("FindTestCentreController::post: No currentBooking set")
      );
    });
  });
});

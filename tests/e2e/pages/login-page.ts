...
import { enter, click, verifyContainsText } from "../utils/helpers";
...
export class LoginPage extends BasePage {
  ...
  // Content
  pageHeading =
    "Enter the booking reference and driving licence number of the person taking the theory test";

  errorMessageMissing =
    "Enter both the booking reference and candidate's driving licence number to access this booking";

  errorMessageValidation =
    "You must use the most recent driving licence number and the correct booking reference. Please check the details you have entered, then try again.";
  ...
  async enterLoginDetailsWithSpaces(
    bookingReference: string,
    drivingLicence: string
  ): Promise<void> {
    await enter(this.bookingReferenceTextBox, `     ${bookingReference}      `);

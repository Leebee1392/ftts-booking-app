...
describe("Booking handler", () => {
  ...
  describe("create booking", () => {
    test("successfully reserves a slot and creates a booking and booking product in CRM", async () => {
      ...
      scheduling.reserveSlot = jest.fn().mockImplementationOnce(
        (centre: Centre, startDateTime: string): Promise<ReservationResponse> =>
          Promise.resolve({
      ...
      crmGateway.updateCandidateAndCreateBooking = jest
        .fn()
        .mockResolvedValue(mockCandidateAndBookingResponse);
      crmGateway.createBookingProduct = jest
        .fn()
        .mockResolvedValue("booking-product-id");

      const bookingHandler: BookingHandler = new BookingHandler(
        crmGateway,
        req,
        scheduling
      );
      ...
      expect(scheduling.reserveSlot).toHaveBeenCalledWith(
      ...
      expect(req.session.currentBooking.bookingRef).toEqual(
        mockCandidateAndBookingResponse.booking.reference
      );
      expect(req.session.currentBooking.bookingProductRef).toEqual(
        `${mockCandidateAndBookingResponse.booking.reference}-01`
      );
      expect(req.session.currentBooking.bookingId).toEqual(
        mockCandidateAndBookingResponse.booking.id
      );
      expect(req.session.currentBooking.bookingProductId).toEqual(
        "booking-product-id"
      );
    ...
    test("NSA booking gets created with an updated telephone number supplied", async () => {
      ...
      crmGateway.updateCandidateAndCreateBooking = jest
        .fn()
        .mockResolvedValue(mockBookingAndCandidateResponse);
      crmGateway.createBookingProduct = jest
        .fn()
        .mockResolvedValue(mockBookingID);
    ...
    test("errors when reserving a booking with scheduling API fails", async () => {
      ...
      const bookingHandler: BookingHandler = new BookingHandler(
        mockedCrmGateway as unknown as CRMGateway,
        req,
        scheduling
      );

      await expect(bookingHandler.createBooking()).rejects.toThrow(
        "expected error"
      );
      expect(
        mockedCrmGateway.updateCandidateAndCreateBooking
  ...
  describe("error handling", () => {
    test("missing candidate on init", () => {
      ...
      expect(() => new BookingHandler(crmGateway, req, scheduling)).toThrow(
        Error("BookingHandler::constructor: No candidate set")
      );
    });
    ...
    test("missing booking on init", () => {
      ...
      expect(() => new BookingHandler(crmGateway, req, scheduling)).toThrow(
        Error("BookingHandler::constructor: No currentBooking set")
      );
    });
    ...
    test("missing journey on init", () => {
      ...
      expect(() => new BookingHandler(crmGateway, req, scheduling)).toThrow(
        Error("BookingHandler::constructor: No journey set")
      );
    });
    ...
    test("reservation catches booking missing data", async () => {
      ...
      await expect(bookingHandler.createBooking()).rejects.toThrow(
        Error("BookingHandler::reserveBooking: No booking params set")
      );
    });
    ...
    test("reservation catches candidate missing data", async () => {
      ...
      await expect(bookingHandler.createBooking()).rejects.toThrow(
        Error("BookingHandler::createEntities: No booking params set")
      );
    });
  });
});

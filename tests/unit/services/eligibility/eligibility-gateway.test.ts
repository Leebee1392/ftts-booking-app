...
describe("EligibilityGateway", () => {
  ...
  beforeEach(() => {
    ...
    eligibilityGateway = new EligibilityGateway(
      mockAuth as unknown as ManagedIdentityAuth,
      mockedAxios
    );
  });
  ...
  describe("getEligibility", () => {
    test("should return the candidates eligibility information", async () => {
      ...
      const result = await eligibilityGateway.getEligibility(
        "mock-licence",
        isManageBooking
      );
    ...
    test("should throw an error when eligibility fails", async () => {
      ...
      await expect(
        eligibilityGateway.getEligibility("mock-licence", isManageBooking)
    ...
    test("should throw a TooManyRequestsError when eligibility returns http code 429", async () => {
      ...
      mockedAxios.post.mockRejectedValue(error);
      await expect(
        eligibilityGateway.getEligibility("mock-licence")
      ).rejects.toStrictEqual(new EligibilityTooManyRequestsError());
      expect(logger.error).toHaveBeenCalledWith(
        error,
    ...
    test("should throw a ServerError when eligibility returns http code 5**", async () => {
      ...
      await expect(
        eligibilityGateway.getEligibility("mock-licence")
      ).rejects.toStrictEqual(new EligibilityServerError("500"));
      expect(logger.error).toHaveBeenCalledWith(
        error,
    ...
    test("should throw a AuthError when eligibility returns http code 401", async () => {
      ...
      await expect(
        eligibilityGateway.getEligibility("mock-licence")
      ).rejects.toStrictEqual(new EligibilityAuthError("401"));
      expect(logger.error).toHaveBeenCalledWith(
        error,
    ...
    test("should throw a LicenceNotFound error when eligibility returns http code 404", async () => {
      ...
      await expect(
        eligibilityGateway.getEligibility(
          "mock-licence",
          false,
          Target.GB,
          Locale.GB
        ...
        new EligibilityLicenceNotFoundError(
    ...
    test("logs if a test type cannot be mapped", async () => {
      ...
      expect(result).toStrictEqual(expectedResult);
      expect(logger.warn).toHaveBeenCalledWith(
        "EligibilityGateway::mapEligibilityResponseToCandidate Unknown Test Type: ABC123"
      );
    });
  });
});

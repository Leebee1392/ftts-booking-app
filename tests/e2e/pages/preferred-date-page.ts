...
import { link, click, enter, verifyIsVisible } from "../utils/helpers";
...
export class PreferredDatePage extends BasePage {
  ...
  datePickerFirstAvailableDay =
    "button.date__button:not(.date__button--disabled)";
  ...
  changeBannerText =
    "You can change the test slot here or keep the date and time previously selected.";

  changeBannerWarningText =
    "If you select a new date and time here they will replace the details previously entered. Date and time slots are both subject to availability.";

  pageContent =
    "If no test slots are available on this date, this service can help you find one on another day.";
  ...
  async enterPreferredDateAndSubmit(
    day: string,
    month: string,
    year: string
  ): Promise<ChooseAppointmentPage> {
    await this.enterPreferredDate(day, month, year);
    await click(this.continueButton);
    return new ChooseAppointmentPage();
  }

  async selectPreferredDateWithAppointments(
  ...
  async selectPreferredDateLessThan3Days(
    testCentreSearch: TestCentreSearch
  ): Promise<ChooseAppointmentPage> {
  ...
  async selectPreferredDateWithNoAppointments(
    testCentreSearch: TestCentreSearch
  ): Promise<ChooseAppointmentPage> {

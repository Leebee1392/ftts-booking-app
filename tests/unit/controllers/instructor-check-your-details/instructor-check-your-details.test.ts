...
import {
  Language, PreferredDay, PreferredLocation, SupportType, Target, TestType, Voiceover,
...
describe('Instructor Check Your Details controller', () => {
  ...
  describe('GET request', () => {
    ...
    test('DVA context presents translator option', () => {
      ...
      expect(res.render).toHaveBeenCalledWith('instructor/check-your-details', expect.objectContaining({
  ...
  describe('POST request', () => {
    ...
    test('renders the custom error page when the user is able to retry the request', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith({ message: 'booking creation failed', status: 500 }, 'InstructorCheckYourDetailsController::post: Error creating booking entity in CRM');
    });
    ...
    test('throws a fatal error when the user is not able to retry the request', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith({ message: 'booking creation failed', status: 400 }, 'InstructorCheckYourDetailsController::post: Error creating booking entity in CRM');
    });
    ...
    test('redirects to technical error page when CrmCreateBookingDataError', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(error, 'InstructorCheckYourDetailsController::post: Error creating booking entity in CRM');
    });

    test('redirects to the booking confirmation page', async () => {
      await checkYourDetails.post(req, res);

      expect(res.redirect).toHaveBeenCalledWith('/instructor/booking-confirmation');
    });

...
fixture`Block duplicate Standard Accommodation bookings`
  ...
  .afterEach(async () => {
    ...
    if (!runningTestsLocally()) {
      await crmGateway.cleanUpBookingProducts(
...
dataSet.forEach((data) => {
  ...
  test(`Verify user is blocked from booking same test type - ${data.testName}`, async () => {
    ...
    const sessionDataExistingBooking = new SessionData(
      data.target,
      data.locale,
      false,
    ...
    if (data.nonStandardAccommodations) {
      ...
      sessionDataExistingBooking.currentBooking.selectSupportType = [
      ...
      sessionDataExistingBooking.currentBooking.preferredDayOption =
        PreferredDay.ParticularDay;
      sessionDataExistingBooking.currentBooking.preferredDay =
        "I only want to have tests on Mondays";
      sessionDataExistingBooking.currentBooking.preferredLocationOption =
        PreferredLocation.ParticularLocation;
      sessionDataExistingBooking.currentBooking.preferredLocation =
    ...
    if (data.target === Target.NI && data.instructorBooking) {
      await verifyTitleContainsText(
        checkYourDetailsPage.pageHeadingNiInstructor
      );
    } else {
    ...
    await clickLinkContainsURL(
      bookingExistsPage.manageBookingLinkText,
      "manage-booking/login"
    );
    await verifyContainsText(
      bookingExistsPage.manageBookingLink,
      bookingExistsPage.manageBookingLinkText
    );
...
test("Verify user can proceed to book a new test of the same type if an existing confirmed booking is in the past", async () => {
  ...
  sessionDataExistingBooking.currentBooking.dateTime = dayjs()
    .subtract(1, "day")
    .toISOString();
  t.ctx.sessionData = sessionDataExistingBooking;
  if (!runningTestsLocally()) {
  ...
  // now attempt to book a test of the same type
  const sessionDataNewBooking = sessionDataExistingBooking.snapshot();
  sessionDataNewBooking.currentBooking.dateTime = dayjs()
    .add(3, "day")
    .toISOString();
  await setCookie(headerLogger, sessionDataNewBooking);
  await t.navigateTo(
    `${process.env.BOOKING_APP_URL}/${checkYourDetailsPage.pathUrl}`
  );
  await verifyTitleContainsText(checkYourDetailsPage.pageHeading);
  await checkYourDetailsPage.checkDataMatchesSession(sessionDataNewBooking);
  await checkYourDetailsPage.continueBooking();
  await paymentsPage.makePayment(
    Constants.testPayment,
    sessionDataNewBooking.candidate.email
  );
  ...
  sessionDataNewBooking.currentBooking.bookingRef = await getText(

...
import selectDateController from "@controllers/select-date/select-date";
...
describe("Select Date controller", () => {
  ...
  describe("GET", () => {
    test("renders the view with correct data", () => {
      ...
      expect(res.render).toHaveBeenCalledWith("select-date", {
    ...
    test("throws error if journey is not set", () => {
      ...
      expect(() => selectDateController.get(req, res)).toThrow(
        new Error("SelectDateController::get: No journey set")
      );
    });
  });
  ...
  describe("POST", () => {
    ...
    test("sets the selected date in the session", () => {
      ...
      expect(req.session.testCentreSearch.selectedDate).toEqual(
        expectedDateString
      );
    });
    ...
    test("sets the first selected test date in session if not already set", () => {
      ...
      expect(req.session.currentBooking.firstSelectedDate).toEqual(
        expectedDateString
      );
    });
    ...
    test("when in managed booking mode sets the selected date in the session", () => {
      ...
      expect(req.session.manageBookingEdits.dateTime).toEqual(
        expectedDateString
      );
      expect(res.redirect).toHaveBeenCalledWith(
        "/manage-booking/choose-appointment?selectedDate=2020-02-01"
      );
    });
    ...
    test("selecting todays date throws error", () => {
      ...
      expect(() => selectDateController.post(req, res)).toThrow(
        new Error("dateIsTodayOrTomorrow")
      );
    });
    ...
    test("throws error if no current booking is set", () => {
      ...
      expect(() => selectDateController.post(req, res)).toThrow(
        new Error("SelectDateController::post: No currentBooking set")
      );
    });
  });
});

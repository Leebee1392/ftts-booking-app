...
import { TechnicalErrorController } from "@controllers/error-technical/error-technical";
...
describe("Error technical controller", () => {
  ...
  beforeEach(() => {
    ...
    setCorrectLanguage.mockImplementation(async () =>
      Promise.resolve(Locale.GB)
    );
  ...
  describe("GET", () => {
    test.each([
      ...
      "should render technical error page",
      async (givenTarget: Target | undefined, expectedTarget: Target) => {
        req.query.target = givenTarget;
        await technicalErrorController.get(req, res);

        expect(res.render).toHaveBeenCalledWith("common/error-technical", {
          startAgainLink: START_AGAIN_LINK,
        });
        expect(setCorrectLanguage).toHaveBeenCalledWith(
          req,
          res,
          expectedTarget
        );

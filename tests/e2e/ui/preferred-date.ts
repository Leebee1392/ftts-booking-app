...
test("Verify page title, heading and UI contents are displayed correctly", async () => {
  await verifyTitleContainsText(
    `${preferredDatePage.pageHeading} ${Constants.generalTitle}`
  );
  ...
  await verifyExactText(
    preferredDatePage.pageContentLocator,
    preferredDatePage.pageContent
  );
...
test("Verify that an error is displayed when entering no date and submitting", async () => {
  ...
  await verifyExactText(
    preferredDatePage.errorMessageLocator,
    preferredDatePage.errorMessageHeader
  );
  await verifyExactText(
    preferredDatePage.errorMessageList,
    "Enter a valid date"
  );
...
test("Verify that an error is displayed when entering a date more than 6 months in the future", async () => {
  ...
  const overSixMonthsDate = new Date(
  ...
  await verifyExactText(
    preferredDatePage.errorMessageList,
    "Date must be within 6 months of today"
  );
  await click(preferredDatePage.errorLinkDate);
});
...
test("Verify that we cannot search for slots for today", async () => {
  ...
  await verifyExactText(
    preferredDatePage.errorMessageList,
    "There are no slots available today or tomorrow. Try another date."
  );
  await click(preferredDatePage.errorLinkDate);
});

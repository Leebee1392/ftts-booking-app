import { InstructorCheckYourDetailsController } from "@controllers/instructor-check-your-details/instructor-check-your-details";
import {
  Language,
  PreferredDay,
  PreferredLocation,
...
const mockEmailContentEvidenceRequired = {
  subject: "subject",
  body: "body evidence required",
};
const mockEmailContentEvidenceNotRequired = {
  subject: "subject",
  body: "body evidence not required",
};
const mockEmailContentEvidenceMayBeRequired = {
  subject: "subject",
  body: "body evidence may be required",
};
const mockEmailContentReturningCandidate = {
  subject: "subject",
  body: "body returning candidate",
};
jest.mock("../../../../src/services/notifications/content/builders", () => ({
  ...
  buildEvidenceNotRequiredEmailContent: () =>
    mockEmailContentEvidenceNotRequired,
  buildEvidenceMayBeRequiredEmailContent: () =>
    mockEmailContentEvidenceMayBeRequired,
  buildReturningCandidateEmailContent: () => mockEmailContentReturningCandidate,
}));

describe("Instructor Check Your Details controller", () => {
  let res: any;
  let req: any;
  let checkYourDetails: InstructorCheckYourDetailsController;
  ...
  beforeEach(() => {
    ...
    checkYourDetails = new InstructorCheckYourDetailsController(
      notifications,
      crmGateway
    );
  });
  ...
  describe("GET request", () => {
    test("renders check-your-details page", () => {
      ...
      expect(res.render).toHaveBeenCalledWith("instructor/check-your-details", {
        firstNames: "Test",
        surname: "User",
        dateOfBirth: "1990-01-01",
        licenceNumber: "JONES061102W97YT",
  ...
  describe("POST request", () => {
    test.each([
      ...
      "calls notification API to send a support request confirmation email. Support type: %, email content: %",
      async (
        supportType: SupportType,
        expectedEmailContent: { subject: string; body: string }
      ) => {
    ...
    test("handles email for returning candidate", async () => {
      ...
      expect(notifications.sendEmail).toHaveBeenCalledWith(
        mockEmail,
        mockEmailContentReturningCandidate,
        req.session.currentBooking.bookingRef,
        Target.GB
      );
    });
    ...
    test("logs and swallows any error from calling notification API", async () => {
      notifications.sendEmail = jest
        .fn()
        .mockRejectedValueOnce(new Error("ntf api failed"));
    ...
    test("renders the custom error page when the user is able to retry the request", async () => {
      ...
      await checkYourDetails.post(req, res);
      expect(res.render).toHaveBeenCalledWith("supported/check-details-error");
      expect(logger.error).toHaveBeenCalledWith(
        { message: "booking creation failed", status: 500 },
        "InstructorCheckYourDetailsController::post: Error creating booking entity in CRM"
      );
    });
    ...
    test("throws a fatal error when the user is not able to retry the request", async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        { message: "booking creation failed", status: 400 },
        "InstructorCheckYourDetailsController::post: Error creating booking entity in CRM"
      );
    });
    ...
    test("redirects to technical error page when CrmCreateBookingDataError", async () => {
      ...
      expect(res.redirect).toHaveBeenCalledWith(
        "/error-technical?source=/&target=gb&lang=gb"
      );
      expect(logger.error).toHaveBeenCalledWith(
        error,
        "InstructorCheckYourDetailsController::post: Error creating booking entity in CRM"
    ...
    test("redirects to the booking confirmation page", async () => {
      ...
      expect(res.redirect).toHaveBeenCalledWith(
        "/instructor/booking-confirmation"
      );
    });

...
export class InstructorVoiceoverController {
  ...
  public post = (req: Request, res: Response): void => {
    ...
    if (inEditMode) {
      return isNonStandardJourney(req)
  ...
  private renderPage(req: Request, res: Response): void {
    if (!req.session.journey) {
      throw new Error(
        "InstructorVoiceoverController::renderPage: No Journey object found in session"
      );
    }
    ...
    if (!testType) {
      throw new Error(
        "InstructorVoiceoverController::renderPage: Missing test type in session current booking"
      );
    }
    ...
    availableOptions.forEach((option) => {
      ...
      if (
        (inEditMode ||
          inManagedBookingEditMode ||
          receivedSupportRequestPageFlag ||
          !isNonStandardJourney(req)) &&
        target === Target.GB
      ) {
        optionText = `${translate(
          "voiceover.editMode.yesOptionPrefix"
        )} ${translate(`generalContent.language.${option}`)}`;
      }
      radioItems.push({
        value: option,
        text: optionText,
  ...
  private getBackLink = (req: Request): string => {
    if (!req.session.journey) {
      throw new Error(
        "InstructorVoiceoverController::getBackLink: No Journey object found in session"
      );
    }

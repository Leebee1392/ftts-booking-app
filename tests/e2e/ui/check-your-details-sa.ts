...
const pageUrl = `${process.env.BOOKING_APP_URL}/${checkYourDetailsPage.pathUrl}`;
...
fixture`Check your details - Standard Accommodations`
...
test("Verify page UI contents are displayed correctly", async (t) => {
  ...
  await verifyTitleContainsText(
    `${checkYourDetailsPage.pageHeading} ${generalTitle}`
  );
  await verifyContainsText(
    checkYourDetailsPage.pageHeadingLocator,
    checkYourDetailsPage.pageHeading
  );

  await verifyExactText(
    checkYourDetailsPage.personalDetailsHeadingLocator,
    "Personal details"
  );
  await verifyExactText(checkYourDetailsPage.personalDetailsKey, "Name", 0);
  await verifyExactText(
    checkYourDetailsPage.personalDetailsKey,
    "Date of birth",
    1
  );
  await verifyExactText(
  ...
  await verifyExactText(
    checkYourDetailsPage.testDetailsHeadingLocator,
    "Test details"
  );
  ...
  await verifyExactText(
    checkYourDetailsPage.testDetailsKey,
    "Test time and date",
    2
  );
  await verifyExactText(
  ...
  await verifyExactText(
    checkYourDetailsPage.testDetailsKey,
    "On-screen British Sign Language",
    6
  );
  await verifyExactText(
    checkYourDetailsPage.testDetailsKey,
  ...
  await verifyContainsText(
    checkYourDetailsPage.paymentAlertMessageLocator,
    checkYourDetailsPage.paymentAlertMessage
  );
...
test("Verify the Back link takes you to the choose appointment page", async () => {
  ...
  await verifyExactText(
    chooseAppointmentPage.pageHeadingLocator,
    chooseAppointmentPage.pageHeading
  );
});
...
test("Change email - the correct content is displayed", async (t) => {
  ...
  await verifyExactText(
    contactDetailsPage.updateEmailBanner,
    contactDetailsPage.updateEmailBannerText
  );
  await t
    .expect(Selector(contactDetailsPage.emailAddressTextBox).value)
    .eql(sessionData.candidate.email);
  await t
    .expect(Selector(contactDetailsPage.confirmEmailAddressTextBox).value)
    .eql(sessionData.candidate.email);
  await verifyExactText(
    contactDetailsPage.continueButton,
    contactDetailsPage.updateEmailButtonText
  );
  await verifyExactText(
    contactDetailsPage.cancelButton,
    contactDetailsPage.cancelEmailButtonText
  );
});
...
test("Change email - cancel using the link in the banner, the original email is kept", async (t) => {
  ...
  await verifyExactText(
    checkYourDetailsPage.personalDetailsValue,
...
test("Change test type - the correct content is displayed", async (t) => {
  const testTypePage = await checkYourDetailsPage.changeTestType();
  await verifyExactText(
  ...
  await t
    .expect(
      Selector(capitalizeFirstLetter(testTypePage.testCategoryButtonCar))
        .checked
    )
    .ok();

  await verifyContainsText(
    testTypePage.updateTestTypeBanner,
    testTypePage.updateTestTypeBannerText
  );
  await verifyExactText(testTypePage.warningIcon, "!");
  await verifyContainsText(
    testTypePage.warningMessageLocator,
    testTypePage.warningText
  );

  await verifyExactText(
    testTypePage.continueButton,
    testTypePage.updateTestTypeButtonText
  );
  await verifyExactText(
    testTypePage.cancelButton,
    testTypePage.cancelTestTypeButtonText
  );
});

test("Change test type - cancel using the link in the banner, the original test type is kept", async (t) => {
  const { sessionData } = t.ctx;
  const testTypePage = await checkYourDetailsPage.changeTestType();
  await link(testTypePage.cancelTestTypeLinkText);
  await verifyExactText(
    checkYourDetailsPage.testDetailsValue,
    capitalizeFirstLetter(sessionData.currentBooking.testType),
    0
  );
});

test("Change test type - cancel using the button at the bottom, the original test type is kept", async (t) => {
  const { sessionData } = t.ctx;
  const testTypePage = await checkYourDetailsPage.changeTestType();
...
test("Change test type - save and we have to re-enter all other details", async (t) => {
  ...
  await t
    .expect(Selector(languagePage.testLanguageButtonEnglish).checked)
    .notOk();
  await t
    .expect(Selector(languagePage.testLanguageButtonWelsh).checked)
    .notOk();
  await languagePage.selectTestLanguage(
    Languages.get(sessionData.currentBooking.language)
  );

  const selectStandardSupportPage = new SelectStandardSupportPage();
  await t
    .expect(Selector(selectStandardSupportPage.onScreenBslRadioButton).checked)
    .notOk();
  await t
    .expect(Selector(selectStandardSupportPage.voiceoverRadioButton).checked)
    .notOk();
  await t
    .expect(Selector(selectStandardSupportPage.noSupportRadioButton).checked)
    .notOk();
  await selectStandardSupportPage.selectSupportTypeRequired(
    SupportType.NO_SUPPORT_WANTED
  );

  const findTheoryTestCentrePage = new FindATheoryTestCentrePage();
  ...
  const preferredDatePage = await chooseTheoryTestCentrePage.selectATestCentre(
...
test("Change language - the correct content is displayed", async (t) => {
  ...
  await verifyExactText(
    languagePage.pageHeadingLocator,
    languagePage.pageHeading
  );
  await verifyIsNotVisible(languagePage.backLink);
  await t.expect(Selector(languagePage.testLanguageButtonEnglish).checked).ok();

  await verifyExactText(
    languagePage.updateLanguageBanner,
    languagePage.updateLanguageBannerText
  );
  await verifyExactText(
    languagePage.continueButton,
    languagePage.updateLanguageButtonText
  );
  await verifyExactText(
    languagePage.cancelButton,
    languagePage.cancelLanguageButtonText
  );
});
...
test("Change language for NI - change button is hidden for a NI user", async (t) => {
  const sessionData = new SessionData(Target.NI, Locale.NI);
  await setCookie(headerLogger, sessionData);
  await t.navigateTo(pageUrl);

  await verifyTitleContainsText(
    `${checkYourDetailsPage.pageHeading} ${generalTitleNI}`
  );
  await verifyContainsText(
    checkYourDetailsPage.pageHeadingLocator,
    checkYourDetailsPage.pageHeading
  );
  await verifyIsNotVisible(checkYourDetailsPage.changeTestLanguageLink);
});

test("Change time date location - What do you want to change page - the correct content is displayed", async (t) => {
  const whatDoYouWantToChangePage =
    await checkYourDetailsPage.changeTestTimeDateLocation();
  await verifyTitleContainsText(
    `${whatDoYouWantToChangePage.pageHeading} ${generalTitle}`
  );
  await verifyExactText(
    whatDoYouWantToChangePage.pageHeadingLocator,
    whatDoYouWantToChangePage.pageHeading
  );

  await verifyIsNotVisible(whatDoYouWantToChangePage.backLink);

  await verifyExactText(
    whatDoYouWantToChangePage.bannerTextLocator,
    whatDoYouWantToChangePage.bannerText
  );
  await verifyExactText(
    whatDoYouWantToChangePage.hintLocator,
    whatDoYouWantToChangePage.hintText
  );
  await verifyExactText(
    whatDoYouWantToChangePage.timeOnlyHintLocator,
    whatDoYouWantToChangePage.timeOnlyHintText
  );
  await verifyExactText(
    whatDoYouWantToChangePage.timeAndDateOnlyHintLocator,
    whatDoYouWantToChangePage.timeAndDateOnlyHintText
  );
  await verifyExactText(
    whatDoYouWantToChangePage.locationOnlyHintLocator,
    whatDoYouWantToChangePage.locationOnlyHintText
  );
  await verifyExactText(
    whatDoYouWantToChangePage.changeAndContinueButton,
    whatDoYouWantToChangePage.changeAndContinueButtonText
  );
  await verifyExactText(
    whatDoYouWantToChangePage.cancelAndKeepYourSelectionButton,
    whatDoYouWantToChangePage.cancelAndKeepYourSelectionButtonText
  );

  await t
    .expect(Selector(whatDoYouWantToChangePage.changeTimeOnlyOption).checked)
    .notOk();
  await t
    .expect(Selector(whatDoYouWantToChangePage.changeTimeAndDateOption).checked)
    .notOk();
  await t
    .expect(Selector(whatDoYouWantToChangePage.changeLocationOption).checked)
    .notOk();
});
...
test("Change time date location - What do you want to change page - Verify the user is unable to proceed without selecting a test time date or location and an error message is displayed", async () => {
  ...
  await verifyExactText(
    whatDoYouWantToChangePage.errorMessageLocator,
    whatDoYouWantToChangePage.errorMessageHeader
  );
  await verifyExactText(
    whatDoYouWantToChangePage.errorMessageList,
    whatDoYouWantToChangePage.errorMessageText
  );
  await verifyContainsText(
    whatDoYouWantToChangePage.errorMessageRadioLocator,
    whatDoYouWantToChangePage.errorMessageText
  );
...
test("Change time date location - change time and verify the new time and old date and location is displayed", async (t) => {
  ...
  const whatDoYouWantToChangePage =
    await checkYourDetailsPage.changeTestTimeDateLocation();
  const chooseAppointmentPage =
    await whatDoYouWantToChangePage.selectTimeOnly();
  await chooseAppointmentPage.chooseAppointment(sessionData.currentBooking);
...
test("Change time date location - go to change the time slot page and verify the banner text but later decide to keep the old time slot", async (t) => {
  ...
  const chooseAppointmentPage =
    await whatDoYouWantToChangePage.selectTimeOnly();
  await verifyContainsText(
    chooseAppointmentPage.bannerLocator,
    chooseAppointmentPage.changeBannerText1
  );
  await verifyContainsText(
    chooseAppointmentPage.bannerLocator,
    chooseAppointmentPage.changeBannerText2
  );
...
test("Change time date location - go to change the time slot page but later decide to change location, date and time and verify location banner text in location page", async (t) => {
  ...
  await verifyContainsText(
    findTheoryTestCentrePage.bannerLocator,
    findTheoryTestCentrePage.changeBannerText
  );
  await verifyContainsText(
    findTheoryTestCentrePage.bannerLocator,
    findTheoryTestCentrePage.changeBannerWarningText
  );
  await verifyExactText(
    findTheoryTestCentrePage.cancelButton,
    findTheoryTestCentrePage.cancelButtonText
  );
  await verifyExactText(
    findTheoryTestCentrePage.findButton,
    findTheoryTestCentrePage.findButtonText
  );
  await verifyExactText(findTheoryTestCentrePage.searchLocationTermTextBox, "");
  const chooseTheoryTestCentrePage =
    await findTheoryTestCentrePage.findATheoryTestCentre(
...
test("Change time date location - go to change the time slot but later decide to change location but later to decide to keep the same test location from search results and verify the banner text in search results page", async (t) => {
  ...
  await verifyExactText(
    chooseTheoryTestCentrePage.bannerLocator,
    chooseTheoryTestCentrePage.bannerText
  );
...
test("Change time date location - change date and time and verify the banner text and new date and time and old location is displayed", async (t) => {
  ...
  const whatDoYouWantToChangePage =
    await checkYourDetailsPage.changeTestTimeDateLocation();
  const preferredDatePage =
    await whatDoYouWantToChangePage.selectTimeAndDateOnly();
  await verifyExactText(preferredDatePage.dayTextBox, "");
  await verifyExactText(preferredDatePage.monthTextBox, "");
  await verifyExactText(preferredDatePage.yearTextBox, "");
  await verifyContainsText(
    preferredDatePage.bannerLocator,
    preferredDatePage.changeBannerText
  );
  await verifyContainsText(
    preferredDatePage.bannerLocator,
    preferredDatePage.changeBannerWarningText
  );
  await verifyExactText(
    preferredDatePage.cancelButton,
    preferredDatePage.cancelButtonText
  );
  const testDate = preferredDatePage.adjustForDaysWithNoSlots(
    getFutureDate("month", 4)
  );
  const chooseAppointmentPage =
    await preferredDatePage.enterPreferredDateAndSubmit(
      testDate.date().toString(),
      (testDate.month() + 1).toString(),
...
test("Change BSL - the correct content is displayed", async (t) => {
  const { sessionData } = t.ctx;
  const bslPage = await checkYourDetailsPage.changeBsl();
  await verifyExactText(bslPage.pageHeadingLocator, bslPage.pageHeading);
  await verifyIsNotVisible(bslPage.backLink);
  const bslSelectedOption = bslPage.bslYesNo.replace(
    "<>",
  ...
  await verifyExactText(
    bslPage.continueButton,
    bslPage.confirmChangeButtonText
  );
  await verifyExactText(bslPage.cancelButton, bslPage.cancelChangeButtonText);
});

test("Change BSL - cancel using the link in the banner, the original BSL option is kept", async (t) => {
  const { sessionData } = t.ctx;
  const bslPage = await checkYourDetailsPage.changeBsl();
...
test("Change BSL - cancel using the button at the bottom, the original BSL option is kept", async (t) => {
  const { sessionData } = t.ctx;
  const bslPage = await checkYourDetailsPage.changeBsl();
...
test("Change BSL for GB - save and the updated BSL option is shown", async (t) => {
  const { sessionData } = t.ctx;
  const bslPage = await checkYourDetailsPage.changeBsl();
...
test("Change Voiceover GB - the correct content is displayed", async (t) => {
  ...
  await verifyExactText(
    voiceoverPage.pageHeadingLocator,
    voiceoverPage.pageHeading
  );
  await verifyIsNotVisible(voiceoverPage.backLink);
  const voiceoverSelector = voiceoverPage.voiceoverButton.replace(
    "<>",
  ...
  await verifyContainsText(
    voiceoverPage.updateLanguageBanner,
    voiceoverPage.updateBannerText
  );
  await verifyContainsText(
    voiceoverPage.updateLanguageBanner,
    voiceoverPage.cancelLinkText
  );
  await verifyExactText(
    voiceoverPage.continueButton,
    voiceoverPage.updateButtonText
  );
  await verifyExactText(
    voiceoverPage.cancelButton,
    voiceoverPage.cancelButtonText
  );
});
...
test("Change Voiceover for NI - save and the updated Voiceover option is shown", async (t) => {
  const sessionData = new SessionData(Target.NI, Locale.NI);
  await setCookie(headerLogger, sessionData);
  await t.navigateTo(pageUrl);
...
test("Change Support Requested for GB from no to yes - save and the updated option is shown", async (t) => {
  const { sessionData } = t.ctx;
  const supportRequestedPage =
    await checkYourDetailsPage.changeSupportRequested();
  ...
  await navigationHelper.navigateToCheckYourDetailsPage();
});

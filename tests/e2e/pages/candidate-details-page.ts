...
export class CandidateDetailsPage extends BasePage {
  ...
  firstnamesExampleLink =
    'details[data-automation-id="firstnames-example-link"]';

  surnameExampleLink = 'details[data-automation-id="surname-example-link"]';

  licenceNumberExampleLink =
    'details[data-automation-id="licenceNumber-example-link"]';

  exampleImageFirstNamesAltText =
    'img[alt="Image showing where to find first and middle names on a driving licence"]';

  exampleImageSurnameAltText =
    'img[alt="Image showing where to find last name on a driving licence"]';

  exampleImageLicenceNumnberAltText =
    'img[alt="Image showing where to find driving licence number on a driving licence"]';

  instructorPersonalReferenceTextBox =
    'input[data-automation-id="personalReference"]';
  ...
  errorMessage =
    "You must enter these details as they appear on the driving licence. Please check them carefully and try again.";

  seeExampleText = "See example";

  licenceNumberHintTextGB =
    "Enter the 16-digit driving licence number of the person taking the test.";

  licenceNumberHintTextNI =
    "Enter the 8-digit driving licence number of the person taking the test.";
  ...
  async enterCandidateDetails(candidate: Candidate): Promise<void> {
    ...
    await enter(
      this.dobMonthTextBox,
      dayjs(candidate.dateOfBirth).format("MM")
    );
    await enter(
      this.dobYearTextBox,
      dayjs(candidate.dateOfBirth).format("YYYY")
    );
  }
  ...
  async enterInstructorCandidateDetails(
    candidate: Candidate,
    target: Target
  ): Promise<void> {
    ...
    if (target === Target.GB) {
      await enter(
        this.instructorPersonalReferenceTextBox,
        candidate.personalReferenceNumber
      );
    } else if (target === Target.NI) {
      await enter(
        this.instructorPersonalReferenceTextBox,
        candidate.paymentReceiptNumber
      );
    }
  }

  async enterInstructorCandidateDetailsWithSpaces(
    candidate: Candidate,
    target: Target
  ): Promise<void> {
  ...
  async enterInstructorCandidateDetailsAndSubmit(
    candidate: Candidate,
    target: Target
  ): Promise<void> {
  ...
  async checkExampleImagesAreVisible(): Promise<void> {
    const image1 = Selector(this.exampleImageFirstNamesAltText)
      .nth(0)
      .with({ visibilityCheck: true });
    await t.expect(image1.exists).ok("", { timeout: 20000 });

    const image2 = Selector(this.exampleImageSurnameAltText)
      .nth(0)
      .with({ visibilityCheck: true });
    await t.expect(image2.exists).ok("", { timeout: 20000 });

    const image3 = Selector(this.exampleImageLicenceNumnberAltText)
      .nth(0)
      .with({ visibilityCheck: true });

...
export class NavigationHelper {
  ...
  async navigateToFindTestCentrePage(): Promise<FindATheoryTestCentrePage> {
    ...
    if (!this.sessionData.hasComeFromNsaJourney) {
      // check the test type
      if (
        this.sessionData.currentBooking.testType === TestType.CAR ||
        this.sessionData.currentBooking.testType === TestType.MOTORCYCLE
      ) {
        // if we have BSL, make sure we select it now
        if (this.sessionData.currentBooking.bsl) {
          await this.selectStandardSupportPage.selectSupportTypeRequired(
            SupportType.ON_SCREEN_BSL
          );
        } else if (
          this.sessionData.currentBooking.voiceover !== Voiceover.NONE
        ) {
          await this.selectStandardSupportPage.selectSupportTypeRequired(
        ...
        // other test types will only have Voiceover

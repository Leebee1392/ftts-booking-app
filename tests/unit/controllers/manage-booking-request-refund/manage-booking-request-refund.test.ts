...
import { ManageBookingRequestRefundController } from "@controllers/manage-booking-request-refund/manage-booking-request-refund";
...
describe("manage booking - request refund controller", () => {
  ...
  beforeEach(() => {
    ...
    requestRefundController = new ManageBookingRequestRefundController(
      paymentsGateway,
  ...
  describe("GET", () => {
    test("renders the page successfully", () => {
      ...
      expect(res.render).toHaveBeenCalledWith("manage-booking/request-refund", {
        backLink: "../home",
  ...
  describe("POST", () => {
    test("successfully calls to request the refund and updates the compensation booking", async () => {
      ...
      expect(paymentsGateway.requestRefund).toHaveBeenCalledWith(
        mockPayload,
        mockCandidate.candidateId,
        mockCandidate.personReference
      );
      expect(crmGateway.updateCompensationBooking).toHaveBeenCalledWith(
        firstBooking.bookingId,
        mockToday
      );
      expect(res.render).toHaveBeenCalledWith(
        "manage-booking/refund-confirmation",
        {
    ...
    test("given that the call to payments api fails, error gets thrown and refund email is not sent", async () => {
      ...
      await expect(requestRefundController.post(req, res)).rejects.toEqual(
        error
      );

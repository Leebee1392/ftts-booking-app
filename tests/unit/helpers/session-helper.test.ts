...
import {
  ...
  getTimeoutErrorPath,
  isCandidateSet,
  mapBookingEntityToSessionBooking,
  mapCentreEntityToSessionCentre,
  mapSessionCentreToCentreEntity,
...
describe("Session Helper", () => {
  ...
  test("maps booking entity to session booking", () => {
    ...
    const mappedBooking = mapBookingEntityToSessionBooking(
      bookingEntity as any
    );
  ...
  describe("getTimeoutErrorPath - go to timeout page with option to go back to service they came from", () => {
    test("timeout page link", () => {
      ...
      expect(getTimeoutErrorPath(req)).toStrictEqual(
        expect.stringContaining("/timeout?source=/")
      );
    });
    ...
    test("timeout page link - manage booking", () => {
      ...
      expect(getTimeoutErrorPath(req)).toStrictEqual(
        expect.stringContaining("/timeout?source=/manage-booking")
      );
    });
    ...
    test("timeout page link - instructor", () => {
      ...
      expect(getTimeoutErrorPath(req)).toStrictEqual(
        expect.stringContaining("/timeout?source=/instructor")
      );
    });
  });

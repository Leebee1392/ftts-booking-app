...
export class BookingHandler {
  ...
  private async createEntities(): Promise<void> {
    logger.debug(
      "BookingHandler::createEntities: Attempting to create entities",
      { bookingId: this.booking.bookingId }
    );
    ...
    const priceListId =
      this.req.session.target === Target.NI
        ? config.crm.priceListId.dva
        : config.crm.priceListId.dvsa;
    const candidateAndBookingResponse =
      await this.crmGateway.updateCandidateAndCreateBooking(
        this.candidate,
        this.booking,
        this.getAdditionalSupport(),
        this.isStandardAccommodation,
        priceListId
      );
    ...
    const bookingProductId = await this.crmGateway.createBookingProduct(
      this.candidate,
      this.booking,
      candidateAndBookingResponse.booking,
      this.isStandardAccommodation,
      this.getAdditionalSupport()
    );
  ...
  private getAdditionalSupport(): CRMAdditionalSupport {
    ...
    if (
      ...
      currentBooking?.voiceover === Voiceover.WELSH
    ) {
      return CRMAdditionalSupport.VoiceoverWelsh;
    }
    if (
      currentBooking?.voiceover &&
      currentBooking?.voiceover === Voiceover.ENGLISH
    ) {

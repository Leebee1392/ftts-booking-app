...
test("Verify page title and heading are displayed correctly", async () => {
  ...
  await verifyContainsText(
    candidateDetailsPage.licenceNumberHint,
    candidateDetailsPage.licenceNumberHintTextGB
  );
});
...
test("Verify the user is able enter valid drivers details with white spaces and still navigate to the contact details page as they are trimmed", async () => {
  const candidateDetailsPage =
    await navigationHelper.navigateToCandidateDetailsPage();
  await candidateDetailsPage.enterCandidateDetailsWithSpaces(
    sessionData.candidate
  );
...
test("Verify the user is able enter licence number in lower case and navigate to the contact details page", async () => {
  ...
  await enter(
    candidateDetailsPage.drivingLicenceTextBox,
    Constants.drivingLicenceGB.toLowerCase()
  );
  await candidateDetailsPage.submitDetails();
  const contactDetailsPage: ContactDetailsPage = new ContactDetailsPage();
  await verifyExactText(
...
test("Verify the user gets an error when entering an invalid first name", async () => {
  ...
  await candidateDetailsPage.submitDetails();
  await verifyExactText(
    candidateDetailsPage.errorMessageLocator,
    candidateDetailsPage.errorMessage
  );
});
...
test("Verify error prefix appears in the title when there is an error", async () => {
  ...
  await verifyTitleContainsText(
    `${candidateDetailsPage.pageHeading} ${Constants.generalTitle}`
  );
});
...
test("Verify the user is able to open all See example links", async () => {
  ...
  await verifyContainsText(
    candidateDetailsPage.candidateDetailsContentLocator3,
    candidateDetailsPage.seeExampleText,
    0
  );
  await verifyContainsText(
    candidateDetailsPage.candidateDetailsContentLocator3,
...
test("Verify the user is able enter valid NI drivers details and navigate to the contact details page", async () => {
  ...
  const candidateDetailsPage =
    await navigationHelperNI.navigateToCandidateDetailsPage();
  await candidateDetailsPage.enterCandidateDetailsAndSubmit(
    sessionDataNI.candidate
  );
  const contactDetailsPage: ContactDetailsPage = new ContactDetailsPage();
  await verifyTitleContainsText(
    `${contactDetailsPage.pageHeading} ${Constants.generalTitle}`
  );
  await verifyExactText(
    contactDetailsPage.pageHeadingLocator,
    contactDetailsPage.pageHeading
...
test("Verify if the candidate has no eligibilties on their licence an error page is displayed", async () => {
  ...
  const candidateDetailsPage =
    await navigationHelperNoElig.navigateToCandidateDetailsPage();
  await candidateDetailsPage.enterCandidateDetailsAndSubmit(
    sessionDataNoElig.candidate
  );
  ...
  await verifyExactText(
    eligibilityErrorPage.pageHeadingLocator,
    eligibilityErrorPage.pageHeading
  );
  await verifyExactText(
    eligibilityErrorPage.messageLocator,
    eligibilityErrorPage.text1DVSA,
    0
  );
});
...
test("Verify if the candidate has behavioural markers but is still eligible to book online they can continue to book", async () => {
  ...
  const candidateDetailsPage =
    await navigationHelperNoElig.navigateToCandidateDetailsPage();
  await candidateDetailsPage.enterCandidateDetailsAndSubmit(
    sessionDataEligMarkers.candidate
  );
  const contactDetailsPage: ContactDetailsPage = new ContactDetailsPage();
  await verifyTitleContainsText(
    `${contactDetailsPage.pageHeading} ${Constants.generalTitle}`

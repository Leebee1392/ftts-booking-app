...
test("Verify the user is able enter valid drivers details with white spaces and still navigate to the contact details page as they are trimmed", async () => {
  const sessionData = new SessionData(Target.GB, Locale.GB, false, true);
  const candidateDetailsPage = await new NavigationHelperInstructor(
    sessionData
  ).navigateToCandidateDetailsPage();
  sessionData.candidate.personalReferenceNumber = "321971";
  await candidateDetailsPage.enterInstructorCandidateDetailsWithSpaces(
    sessionData.candidate,
    sessionData.target as Target
  );
...
test("Verify the user is able enter licence number in lower case and navigate to the contact details page", async () => {
  ...
  await candidateDetailsPage.enterInstructorCandidateDetails(
    sessionData.candidate,
    sessionData.target as Target
  );
  await enter(
    candidateDetailsPage.drivingLicenceTextBox,
    sessionData.candidate.licenceNumber.toLowerCase()
  );
  await candidateDetailsPage.submitDetails();
  await verifyExactText(
    contactDetailsPage.pageHeadingLocator,

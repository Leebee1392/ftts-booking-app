...
dataSet.forEach((data) => {
  ...
  test(`Verify when payment is cancelled, booking can be started again - ${data.testName}`, async () => {
    ...
    await verifyTitleContainsText(
      `${data.bookingCancelledHeading} ${data.title}`
    );
    await verifyContainsText(
      bookingCancelledPage.pageHeadingLocator,
      data.bookingCancelledHeading
    );
    ...
    const startBookingAgainURL = data.instructorBooking
      ? urlInstructor
      : urlCandidate;

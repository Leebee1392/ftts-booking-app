...
export class StayingNSAController {
  ...
  public get = async (req: Request, res: Response): Promise<void> => {
    ...
    const userDraftNsaBookings = await this.crm.getUserDraftNSABookingsIfExist(
    ...
    const hasSupportNeedsInCRM = hasCRMSupportNeeds(
    ...
    return res.render(`supported/${evidenceRoute}`, {
      showExtraContent:
        isDeafCandidate(selectedSupportOptions) &&
        selectedSupportOptions.includes(SupportType.EXTRA_TIME),
...
export default new StayingNSAController(CRMGateway.getInstance());

...
export class BookingManager {
  ...
  public async loadCandidateBookings(
    req: Request,
    candidateId: string
  ): Promise<BookingDetails[]> {
    const allBookings = await this.crm.getCandidateBookings(candidateId);
    const target = req.session.target ?? Target.GB;
    const bookings = allBookings.filter((booking: BookingDetails) =>
      this.agencyMatchesTarget(booking.governmentAgency, target)
    );
  ...
  private agencyMatchesTarget = (
    agency: CRMGovernmentAgency,
    target: Target
  ): boolean =>
    agency in CRMGovernmentAgency &&

...
test("GB Candidate Standard Accommodations (Car) - Verify that session data is persisted when using the back links - from start to choose appointment", async (t) => {
  ...
  await verifyExactText(
    selectStandardSupportTypePage.pageHeadingLocator,
    selectStandardSupportTypePage.pageHeading
  );
  await t
    .expect(
      Selector(selectStandardSupportTypePage.noSupportRadioButton).checked
    )
    .ok();

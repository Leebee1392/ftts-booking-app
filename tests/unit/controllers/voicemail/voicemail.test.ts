import { VoicemailController } from '@controllers/voicemail/voicemail';
...
describe('VoicemailController', () => {
  ...
  describe('get', () => {
    test('it should render the correct page', () => {
      ...
      expect(res.render).toHaveBeenCalledWith('supported/voicemail', expect.any(Object));
    });
    ...
    test('checks voicemail \'yes\' option if voicemail set to true in session', () => {
      ...
      expect(res.render).toHaveBeenCalledWith('supported/voicemail', expect.objectContaining({
        voicemailYes: true,
        voicemailNo: false,
      }));
    });

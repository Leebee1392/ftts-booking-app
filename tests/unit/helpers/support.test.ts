...
describe("Support Helper", () => {
  ...
  describe("removeInvalidOptionsFrom", () => {
    ...
    test.each(invalidOptionMatches)(
      "for target: %s and selected test type is: %s removes the following options from the list: %s",
      (
        target: Target,
        testType: TestType,
    ...
    test.each([Target.GB, Target.NI])(
      "when On-screen BSL not available, removes On-screen BSL support option from the list",
      (target: Target) => {
        ...
        expect(supportTypeOptions.keys()).not.toContain(
          SupportType.ON_SCREEN_BSL
        );
      }
    );
  ...
  describe("getInvalidOptions", () => {
    test.each(invalidOptionMatches)(
      "for target: %s and selected test type is: %s returns: %s",
      (
        target: Target,
        testType: TestType,
        expectedSupportTypesList: SupportType[]
      ) => {
        expect(getInvalidOptions(testType, target)).toEqual(
          expectedSupportTypesList
        );
      }
    );
  });

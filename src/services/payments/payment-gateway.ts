...
class PaymentGateway {
  constructor(
    private auth: ManagedIdentityAuth = new ManagedIdentityAuth(
      config.payment.identity
    ),
    private axiosRetryClient = new AxiosRetryClient(
      config.payment.retryPolicy
    ).getClient(),
    private userCancelledAxiosRetryClient = new AxiosRetryClientForPaymentErrors(
      config.payment.retryPolicy
    ).getClient(),
  ...
  public async initiateCardPayment(
    ...
    logger.debug("PaymentApiClient::initiateCardPayment: sending payload", {
      cardPaymentPayload,
    });
    logger.event(
      BusinessTelemetryEvents.PAYMENT_STARTED,
      "PaymentApiClient::initiateCardPayment: Start",
      {
        personReference,
        userId,
      }
    ...
    const response =
      await this.axiosRetryClient.post<Candidate.CardPaymentResponse>(
        requestUrl,
        Candidate.ObjectSerializer.serialize(
          cardPaymentPayload,
          "CardPaymentPayload"
        ),
        await this.headers(`${UserPrefix.Person}:${userId}`, personReference)
      );
    const responseData: Candidate.CardPaymentResponse =
      Candidate.ObjectSerializer.deserialize(
        response.data,
        "CardPaymentResponse"
      );
    logger.debug("PaymentApiClient::initiateCardPayment: received response", {
      responseData,
    });
    return responseData;
  }
  ...
  public async confirmCardPaymentIsComplete(
    ...
    logger.debug(
      "PaymentApiClient::confirmCardPaymentIsComplete: sending payload",
      { receiptReference }
    );
    const requestUrl = `${this.apiUrl}/api/v1/candidate/gateway/${receiptReference}/complete`;
    const response =
      await this.userCancelledAxiosRetryClient.put<Candidate.CardPaymentCompletionResponse>(
        requestUrl,
        null,
        await this.headers(`${UserPrefix.Person}:${userId}`, personReference)
      );
    const responseData: Candidate.CardPaymentCompletionResponse =
      Candidate.ObjectSerializer.deserialize(
        response.data,
        "CardPaymentCompletionResponse"
      );
    logger.debug(
      "PaymentApiClient::confirmCardPaymentIsComplete: received response",
      { responseData }
    );
    if (response.data?.code === PaymentStatus.SUCCESS) {
      logger.event(
        BusinessTelemetryEvents.PAYMENT_SUCCESS,
        "PaymentApiClient::confirmCardPaymentIsComplete: Finished",
        {
          userId,
          personReference,
        }
  ...
  public async requestRefund(
    ...
    logger.debug("PaymentApiClient::requestRefund: sending payload", {
      batchRefundPayload,
    });
    logger.event(
      BusinessTelemetryEvents.PAYMENT_REFUND_INITIATED,
      "PaymentApiClient::requestRefund: Started",
      {
        userId,
        personReference,
      }
    ...
    const response =
      await this.axiosRetryClient.post<Candidate.BatchPaymentRefundResponse>(
        requestUrl,
        Candidate.ObjectSerializer.serialize(
          batchRefundPayload,
          "BatchRefundPayload"
    ...
    const responseData: Candidate.BatchPaymentRefundResponse =
      Candidate.ObjectSerializer.deserialize(
        response.data,
        "BatchPaymentRefundResponse"
      );
    logger.debug("PaymentApiClient::requestRefund: received response", {
      responseData,
    });
    if (
      responseData?.code &&
      Number(responseData?.code) === PaymentStatus.REFUND_SUCCESS
    ) {
      logger.event(
        BusinessTelemetryEvents.PAYMENT_REFUND_SUCCESS,
        "PaymentApiClient::requestRefund: Finished",
        {
          userId,
          personReference,
        }
  ...
  public async recogniseIncome(
    ...
    logger.debug("PaymentApiClient::recogniseIncome: sending payload", {
    ...
    try {
      ...
      logger.event(
        ...
        "PaymentApiClient::recogniseIncome: Finished",
        {
          userId,
          personReference,
        }
  ...
  public async compensateBooking(
    ...
    logger.debug("PaymentApiClient::compensateBooking: sending payload", {
  ...
  private async headers(
    ...
    try {
      ...
      logger.event(
        ...
        "PaymentApiClient::headers: Failed to get token",
        {
          error,
          paymentUserId,
          personReference,
...
export { PaymentGateway, paymentGateway };

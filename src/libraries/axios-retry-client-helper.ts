...
export function isUserCancelledError(err: CustomAxiosError): boolean {
  return (
    ...
    err.response?.data.code === PaymentStatus.USER_CANCELLED
  );
}
...
export function isGatewayError(err: CustomAxiosError): boolean {
  return (
    err.response?.status === 500 &&
    err.response?.data.code === PaymentStatus.GATEWAY_ERROR
  );
}
...
export function isSystemError(err: CustomAxiosError): boolean {
  return (
    err.response?.status === 500 &&
    err.response?.data.code === PaymentStatus.SYSTEM_ERROR
  );
}
...
export function calculateRetryDelay(
  ...
  if (is429Error(err)) {
    ...
    if (retryAfterHeader) {
      delay =
        parseRetryAfter(retryAfterHeader) ?? retryPolicy.defaultRetryDelay;
  ...
  logger.warn(
    `axiosRetryClientHelper::calculateRetryDelay:Retrying failed ${err.config?.method} request to ${err.config?.url} - attempt ${retryCount} of ${retryPolicy.maxRetries}`,
    {
      error: err.toString(),

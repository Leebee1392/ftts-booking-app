...
describe("UtcDate", () => {
  describe("subtractBusinessDays returns the correct date", () => {
    describe("3 days before", () => {
      const threeBusinessDaysBefore: Array<[string, string, string]> = [
        [
          "February 16, 2020 23:15:30 GMT+01:00",
          "2020-02-11",
          "given a date on Sunday",
        ],
        [
          "May 03, 2020",
          "28 April 2020",
          "given a date on Sunday at the beginning of a month",
        ],
        ["2020-02-29", "2020-02-25", "given a date on Saturday"],
        [
          "2020-02-01",
          "2020-01-28",
          "given a date on Saturday at the beginning of a month",
        ],
      ...
      threeBusinessDaysBefore.forEach((entry) => {
        test(`${entry[2]}`, () => {
          expect(
            new UtcDate(new Date(entry[0])).subtractBusinessDays(3)
          ).toStrictEqual(UtcDate.of(entry[1]));
        });
      });
    });
  });
  ...
  describe("isValidIsoTimeStamp", () => {
    test("returns true if the date is valid", () => {
      expect(UtcDate.isValidIsoTimeStamp("2020-07-30T09:00:00.000Z")).toEqual(
        true
      );
    });

    test("returns false if the date is invalid", () => {
      expect(UtcDate.isValidIsoTimeStamp("2020-07-32T09:00:00.000Z")).toEqual(
        false
      );
    });

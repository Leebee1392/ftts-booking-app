...
export const canChangeBsl = (voiceover?: Voiceover): boolean =>
  voiceover === undefined || voiceover === Voiceover.NONE;

export const canChangeVoiceover = (bsl?: boolean): boolean => !bsl;

export const canShowBslChangeButton = (
  bsl?: boolean,
  voiceover?: Voiceover
): boolean => {
  ...
  return (
    canChangeBslOption || (!canChangeBslOption && !canChangeVoiceoverOption)
  );
};

export const canShowVoiceoverChangeButton = (
  voiceover?: Voiceover,
  bsl?: boolean
): boolean => {
  const canChangeVoiceoverOption = canChangeVoiceover(bsl);
  ...
  return (
    canChangeVoiceoverOption ||
    (!canChangeBslOption && !canChangeVoiceoverOption)
  );
};

export const isNonStandardSupportSelected = (
  selectedSupportTypes: SupportType[]
): boolean =>
  selectedSupportTypes.some((supportType) =>
    nonStandardSupportTypes.includes(supportType)
  );

export const isOnlyStandardSupportSelected = (
  selectedSupportTypes: SupportType[]
): boolean =>
  !selectedSupportTypes.some(
    (supportType) =>
      nonStandardSupportTypes.includes(supportType) ||
      supportType === SupportType.OTHER
  );

export const isOnlyCustomSupportSelected = (
  selectedSupportTypes: SupportType[]
): boolean =>
  selectedSupportTypes.includes(SupportType.OTHER) &&
...
export const getInvalidOptions = (
  ...
  if (target === Target.GB) {
    ...
    if (
      [TestType.LGVHPT, TestType.PCVHPT, TestType.ADIHPT].includes(testType)
    ) {
...
export const removeInvalidOptions = (
  supportTypeOptions: Map<string, SupportTypeOption>,
  req: Request
): void => {
  const testType = req.session.currentBooking?.testType as TestType;
  const { target } = req.session;
  getInvalidOptions(testType, target as Target).forEach(
    (invalidOption: SupportType) => {
      supportTypeOptions.delete(invalidOption);
    }
  );
};

export const toSupportTypeOptions = (
  supportTypes: SupportType[]
): Map<string, SupportTypeOption> => {

...
const pageUrl = `${process.env.BOOKING_APP_URL}/instructor/${contactDetailsPage.pathUrl}`;
...
fixture`Instructor - Contact details`
  ...
  .beforeEach(async (t) => {
    await setCookie(
      headerLogger,
      new SessionData(Target.GB, Locale.GB, false, true)
    );
...
test("Verify page title heading are displayed correctly - Non-Standard Accommodations", async (t) => {
  const sessionData = new SessionData(Target.GB, Locale.GB, true, true);
  await setCookie(headerLogger, sessionData);
  await t.navigateTo(pageUrlNsa);
  await verifyExactText(
    contactDetailsPage.pageHeadingLocator,
    contactDetailsPage.pageHeadingNSA
  );
});
...
test("Verify that when entering no details, an error is displayed", async () => {
  ...
  await verifyExactText(
    contactDetailsPage.errorMessageLocator,
    contactDetailsPage.errorMessageHeader
  );
  await verifyContainsText(
    contactDetailsPage.errorMessageList,
    contactDetailsPage.invalidEmailErrorText
  );
  await verifyContainsText(
    contactDetailsPage.errorMessageList,
    contactDetailsPage.invalidConfirmationEmailErrorText
  );
  await verifyContainsText(
    contactDetailsPage.errorMessageList,
    contactDetailsPage.confirmEmailMatchesErrorText
  );
...
test("Verify that when entering an email address over 100 characters, an error is displayed", async () => {
  ...
  await verifyContainsText(
    contactDetailsPage.errorMessageList,
    contactDetailsPage.over100CharsErrorText
  );

...
const sessionDataNsa = new SessionData(Target.NI, Locale.NI, true);
const navigationHelper: NavigationHelper = new NavigationHelper(sessionData);
const navigationHelperNsa: NavigationHelperNSA = new NavigationHelperNSA(
  sessionDataNsa
);
...
fixture`Online booking app accessibility tests - DVA (NI)`
  ...
  .beforeEach(async () => {
    bookingRef = runningTestsLocally()
      ? Constants.bookingReference1
      : bookingRefForEnvironment;
    drivingLicence = runningTestsLocally()
      ? Constants.drivingLicenceNI
      : drivingLicenceForEnvironment;
    await setCookie(headerLogger, new SessionData(Target.NI));
  })
  .afterEach(async (t) => {
    await runAxeCheck(t);
  })
...
test("Accessibility test - Choose support page", async (t) => {
  ...
  await verifyContainsText(
    chooseSupportPage.pageHeadingLocator,
    chooseSupportPage.pageHeading
  );
});
...
test("Accessibility test - Candidate details page", async (t) => {
  ...
  await verifyContainsText(
    candidateDetailsPage.pageHeadingLocator,
    candidateDetailsPage.pageHeading
  );
});
...
test("Accessibility test - Candidate details with support page", async (t) => {
  ...
  await verifyContainsText(
    candidateDetailsPage.pageHeadingLocator,
    candidateDetailsPage.pageHeadingSupport
  );
});
...
test("Accessibility test - Find a theory test centre page", async (t) => {
  const findATheoryTestCentrePage: FindATheoryTestCentrePage =
    new FindATheoryTestCentrePage();
  t.ctx.page = findATheoryTestCentrePage.pathUrl;
  await t.navigateTo(`${baseUrl}/${t.ctx.page}`);
  await verifyContainsText(
    findATheoryTestCentrePage.pageHeadingLocator,
    findATheoryTestCentrePage.pageHeading
  );
});
...
test("Accessibility test - Find a theory test centre error page", async (t) => {
  const findATheoryTestCentrePage: FindATheoryTestCentrePage =
    new FindATheoryTestCentrePage();
  const findATheoryTestCentreErrorPage: FindATheoryTestCentreErrorPage =
    new FindATheoryTestCentreErrorPage();
  t.ctx.page = `${findATheoryTestCentrePage.pathUrl}-error-page`;
  await t.navigateTo(`${baseUrl}/${findATheoryTestCentrePage.pathUrl}`);
  await findATheoryTestCentrePage.findATheoryTestCentre(
    Constants.searchWithError
  );
  await verifyContainsText(
    findATheoryTestCentreErrorPage.pageHeadingLocator,
    findATheoryTestCentreErrorPage.pageHeading
  );
});

test("Accessibility test - Select a theory test centre page", async (t) => {
  const chooseATestCentrePage: ChooseATestCentrePage =
    new ChooseATestCentrePage();
  t.ctx.page = chooseATestCentrePage.pathUrl;
  await t.navigateTo(`${baseUrl}/${t.ctx.page}`);
  await verifyContainsText(
    chooseATestCentrePage.pageHeadingLocator,
    chooseATestCentrePage.pageHeading
  );
});
...
test("Accessibility test - Enter preferred date page", async (t) => {
  ...
  await t.navigateTo(`${baseUrl}/${t.ctx.page}`);
  await verifyContainsText(
    preferredDatePage.pageHeadingLocator,
    preferredDatePage.pageHeading
  );
});
...
test("Accessibility test - Choose appointment page", async (t) => {
  const preferredDatePage: PreferredDatePage = new PreferredDatePage();
  const chooseAppointmentPage: ChooseAppointmentPage =
    new ChooseAppointmentPage();
...
test("Accessibility test - Choose appointment page - preferred date not available", async (t) => {
  ...
  await t.navigateTo(`${baseUrl}/${preferredDatePage.pathUrl}`);
  await preferredDatePage.selectPreferredDateWithNoAppointments(
    sessionData.testCentreSearch
  );
  await verifyContainsText(
    chooseAppointmentPage.pageHeadingLocator,
...
test("Accessibility test - Check your details page (Non-Standard accommodations)", async (t) => {
  const checkYourDetailsPage: CheckYourDetailsNsaPage =
    new CheckYourDetailsNsaPage();
...
test("Accessibility test - Booking confirmation page", async (t) => {
  ...
  await t.navigateTo(`${baseUrl}/${t.ctx.page}`);
  await verifyContainsText(
    bookingConfirmationPage.pageHeadingLocator,
    bookingConfirmationPage.pageHeadingSA
  );
});
...
test("Accessibility test - NSA Booking confirmation page", async (t) => {
  ...
  await verifyContainsText(
    bookingConfirmationPage.pageHeadingLocator,
    bookingConfirmationPage.pageHeadingNSA
  );
});
...
test("Accessibility test - Login page", async (t) => {
  ...
  await verifyContainsText(loginPage.pageHeadingLocator, loginPage.pageHeading);
});
...
test("Accessibility test - Login page - with error", async (t) => {
  ...
  await verifyContainsText(
    loginPage.errorMessageLocator,
    loginPage.errorMessageMissing
  );
  await verifyContainsText(loginPage.pageHeadingLocator, loginPage.pageHeading);
});
...
test("Accessibility test - Cancel bookings page", async (t) => {
  ...
  await verifyContainsText(
    cancelBookingPage.pageHeadingLocator,
    cancelBookingPage.pageHeading
  );
});
...
test("Accessibility test - Reschedule booking - Check change page", async (t) => {
  t.ctx.page = `${ManageBookingsPage.pathUrl.replace(
    "/",
    "-"
  )}-reschedule-check`;
  await t.navigateTo(`${baseUrl}/${loginPage.pathUrl}`);
...
test("Accessibility test - Manage booking - Confirmed change page", async (t) => {
  ...
  await verifyContainsText(
    changeConfirmedPage.pageHeadingLocator,
    changeConfirmedPage.pageHeading
  );
});
...
test("Accessibility test - Staying NSA page", async (t) => {
  ...
  await verifyContainsText(
    stayingNSAPage.pageHeadingLocator,
    stayingNSAPage.pageHeading
  );
});
...
test("Accessibility test - Leaving NSA page", async (t) => {
  ...
  await verifyContainsText(
    leavingNsaPage.pageHeadingLocator,
    leavingNsaPage.pageHeading
  );
});

test("Accessibility test - NSA Select Support Type page", async (t) => {
  await setCookie(headerLogger, sessionDataNsa);
  const selectSupportTypePage: SelectSupportTypePage =
    new SelectSupportTypePage();
  t.ctx.page = `${selectSupportTypePage.pathUrl.replace("/", "-")}`;

  await t.navigateTo(`${baseUrl}/${selectSupportTypePage.pathUrl}`);
  await verifyContainsText(
    selectSupportTypePage.pageHeadingLocator,
    selectSupportTypePage.pageHeading
  );
});

test("Accessibility test - NSA Select Support Type page - with error", async (t) => {
  const sessionDataSupportType = new SessionData(Target.NI, Locale.NI, true);
...
test("Accessibility test - NSA voiceover page", async (t) => {
  ...
  await verifyContainsText(
    nsaVoiceoverPage.pageHeadingLocator,
    nsaVoiceoverPage.pageHeading
  );
});

test("Accessibility test - NSA Voiceover page - with error", async (t) => {
  const sessionDataVoicover = new SessionData(Target.NI, Locale.NI, true);
...
test("Accessibility test - NSA Voicemail page", async (t) => {
  ...
  await verifyContainsText(
    nsaVoicemailPage.pageHeadingLocator,
    nsaVoicemailPage.pageHeading
  );
});

test("Accessibility test - NSA Voicemail page - with error", async (t) => {
  const sessionDataVoicemail = new SessionData(Target.NI, Locale.NI, true);
...
test("Accessibility test - NSA Telephone contact page", async (t) => {
  await setCookie(headerLogger, sessionDataNsa);
  const nsaTelephoneContactPage: NsaTelephoneContactPage =
    new NsaTelephoneContactPage();
  t.ctx.page = `${nsaTelephoneContactPage.pathUrl.replace("/", "-")}`;

  await t.navigateTo(`${baseUrl}/${nsaTelephoneContactPage.pathUrl}`);
  await verifyContainsText(
    nsaTelephoneContactPage.pageHeadingLocator,
    nsaTelephoneContactPage.pageHeading
  );
});
...
test("Accessibility test - NSA Custom support page", async (t) => {
  ...
  await verifyContainsText(
    customSupportPage.pageHeadingLocator,
    customSupportPage.pageHeading
  );
});

test("Accessibility test - NSA Translator page", async (t) => {
  const sessionDataTranslator = new SessionData(Target.NI, Locale.NI, true);
  await setCookie(headerLogger, sessionDataTranslator);
  const translatorPage: TranslatorPage = new TranslatorPage();
  t.ctx.page = `${translatorPage.pathUrl.replace("/", "-")}`;

  await t.navigateTo(
    `${baseUrl}/${translatorPage.pathUrl}?target=${Target.NI}`
  );
  await verifyContainsText(
    translatorPage.pageHeadingLocator,
    translatorPage.pageHeading
  );
});

test("Accessibility test - NSA Translator page - with error", async (t) => {
  const sessionDataTranslator = new SessionData(Target.NI, Locale.NI, true);
  sessionDataTranslator.currentBooking.translator = undefined;
  await setCookie(headerLogger, sessionDataTranslator);
  const translatorPage: TranslatorPage = new TranslatorPage();
  t.ctx.page = `${translatorPage.pathUrl.replace("/", "-")}-error`;
...
test("Accessibility test - NSA Preferred Day page", async (t) => {
  ...
  await verifyContainsText(
    preferredDayNSAPage.pageHeadingLocator,
    preferredDayNSAPage.pageHeading
  );
});

test("Accessibility test - NSA Preferred Location page", async (t) => {
  await setCookie(headerLogger, sessionDataNsa);
  const preferredLocationNSAPage: PreferredLocationNSAPage =
    new PreferredLocationNSAPage();
  t.ctx.page = `${preferredLocationNSAPage.pathUrl.replace("/", "-")}`;

  await t.navigateTo(`${baseUrl}/${preferredLocationNSAPage.pathUrl}`);
  await verifyContainsText(
    preferredLocationNSAPage.pageHeadingLocator,
    preferredLocationNSAPage.pageHeading
  );
});

async function runAxeCheck(t: TestController) {
  await t.takeScreenshot({ fullPage: true });
  // remove colour contrast check on NI tests due to the DVA colour scheme always flagging this issue
  ...
  createHtmlReport({
    results,
    options: {
      projectKey: "FTTS Online Booking App - DVA (NI)",
      outputDir: "axe-reports-ni",
      reportFileName,
    },
  });
  ...
  if (results.violations.length > 0) {
    ...
    await t
      ...
      .eql(
        0,
        `Axe check failed with violations (see ${reportFileName} for full description) Summary: ${violations}`
      );
  }
}

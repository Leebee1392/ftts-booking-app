...
const multiUserManagedIdentityCredential =
  new MultiUserManagedIdentityCredential(
    config.crm.auth.multipleApplicationUsers.clientIds
  );
...
export const acquireToken = async (
  dynamicsWebApiCallback: OnTokenAcquiredCallback
): Promise<void> => {
  let accessToken;
  try {
    ...
    logger.error(
      e,
      `dynamicsWebApi::acquireToken: Failed to acquire token: ${
        (e as Error).message
      }`,
      { multiUserEnabled }
    );

...
export class VoiceoverController {
  ...
  private renderPage(req: Request, res: Response): void {
    if (!req.session.journey) {
      throw new Error(
        "VoiceoverController::renderPage: No Journey object found in session"
      );
    }
    if (!req.session.currentBooking) {
      throw new Error(
        "VoiceoverController::renderPage: No Current Booking object found in session"
      );
    }
    ...
    if (!testType) {
      throw new Error(
        "VoiceoverController::renderPage: Missing test type in session current booking"
      );
    }
  ...
  private getBackLink = (req: Request): string => {
    if (!req.session.journey) {
      throw new Error(
        "VoiceoverController::getBackLink: No Journey object found in session"
      );
    }

...
import { CandidateDetailsController } from "@controllers/candidate-details/candidate-details";
import {
  Locale,
  Target,
  TestType,
...
describe("Candidate Details controller", () => {
  ...
  beforeEach(() => {
    ...
    candidateDetailsController = new CandidateDetailsController(
      new CandidateService(crmGateway, eligibilityGateway)
    );
  });
  ...
  describe("GET", () => {
    test("renders the candidate details page", () => {
      ...
      expect(res.render).toHaveBeenCalledWith("common/candidate-details", {
        details: {},
        support: false,
      });
    });
  ...
  describe("POST", () => {
    ...
    test("throws error if a call to CRM fails when creating licence", async () => {
      ...
      await expect(candidateDetailsController.post(req, res)).rejects.toEqual(
        new CrmCreateUpdateCandidateError()
      );
    });
    ...
    test("missing journey is caught", async () => {
      ...
      await expect(candidateDetailsController.post(req, res)).rejects.toThrow(
        Error("CandidateDetailsController::post: No journey set")
      );
    });
    ...
    describe("errors", () => {
      ...
      test("renders retry page when eligibility api returns 429 too many requests error", async () => {
        ...
        expect(res.render).toHaveBeenCalledWith("eligibility-retry");
      });

...
test("Verify page title, heading and content are displayed correctly", async () => {
  ...
  await verifyExactText(
    preferredLocationNSAPage.charCountHintText,
    preferredLocationNSAPage.characterCountText
  );
  await t
    .expect(
      Selector(
        preferredLocationNSAPage.getRadioSelector(PreferredLocation.DecideLater)
      ).checked
    )
    .notOk();
  await t
...
test("Verify user can proceed by selecting 'I know which locations I want' and without entering any preferred location details", async () => {
  await preferredLocationNSAPage.selectPreferredLocationOption(
    PreferredLocation.ParticularLocation
  );
...
test("Verify user can proceed by selecting 'I know which locations I want' and entering some valid preferred location details", async () => {
  ...
  await preferredLocationNSAPage.enterPreferredLocationInformation(
    "I prefer Birmingham City Center"
  );
  await verifyTitleContainsText(
    `${contactDetailsPage.pageHeadingNSA} ${generalTitle}`
  );
...
test("Verify a validation error message is displayed when not selecting any option", async () => {
  ...
  await verifyExactText(
    preferredLocationNSAPage.errorMessageLocator,
    preferredLocationNSAPage.errorMessageHeader
  );
  await verifyExactText(
    preferredLocationNSAPage.errorMessageList,
    preferredLocationNSAPage.errorMessageOptionNotSelectedText
  );
  await verifyContainsText(
    preferredLocationNSAPage.errorMessageRadioLocator,
    preferredLocationNSAPage.errorMessageOptionNotSelectedText
  );
...
test("Verify a validation error message is displayed when entering more than 4000 characters and the text is retained", async () => {
  ...
  await verifyExactText(
    preferredLocationNSAPage.errorMessageList,
    preferredLocationNSAPage.errorMessageTooManyCharsText
  );
  await verifyContainsText(
    preferredLocationNSAPage.errorMessageTextAreaLocator,
    preferredLocationNSAPage.errorMessageTooManyCharsText
  );
  ...
  await t
    ...
    .eql(Constants.stringWith4001Chars);
});

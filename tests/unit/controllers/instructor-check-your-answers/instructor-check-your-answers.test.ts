...
import { InstructorCheckYourAnswersController } from "@controllers/instructor-check-your-answers/instructor-check-your-answers";
...
describe("Instructor Check Your Answers controller", () => {
  ...
  beforeEach(() => {
    ...
    checkYourAnswers = new InstructorCheckYourAnswersController(
  ...
  describe("GET request", () => {
    test("renders check-your-answers template", () => {
      ...
      expect(res.render).toHaveBeenCalledWith("instructor/check-your-answers", {
        firstNames: "Test",
        surname: "User",
        dateOfBirth: "1990-01-01",
        licenceNumber: "JONES061102W97YT",
  ...
  describe("POST request", () => {
    ...
    test("render slot unavailable error page if slot is unavailable", async () => {
      ...
      expect(res.render).toHaveBeenCalledWith("error-slot-unavailable");
    });
    ...
    test("throws error when reserve/create booking call fails", async () => {
      ...
      await expect(checkYourAnswers.post(req, res)).rejects.toThrow(
        "expected error"
      );
    ...
    test("if candidate has booking of same test type the redirected to booking exists page and blocked", async () => {
      ...
      mockCrmGateway.doesCandidateHaveExistingBookingsByTestType.mockResolvedValue(
        [req.session.currentBooking]
      );
    ...
    test("if existing booking feature toggle is disable - do not check for existing booking", async () => {
      ...
      expect(
        mockCrmGateway.doesCandidateHaveExistingBookingsByTestType

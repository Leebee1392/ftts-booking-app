...
describe('Instructor Candidate Details controller', () => {
  ...
  describe('GET', () => {
    test('renders the candidate details page', () => {
      ...
      expect(res.render).toHaveBeenCalledWith('instructor/candidate-details', expect.objectContaining({
        details: {},
        support: false,
      }));
    });
  ...
  describe('POST', () => {
    ...
    test('handles 429 response from eligibility api', async () => {
      ...
      expect(res.render).toHaveBeenCalledWith('instructor/error-eligibility-retry');
    });

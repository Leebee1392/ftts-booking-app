...
test("Non-Standard Accommodations GB - Verify that session data is persisted when using the back links - from start to check you details", async (t) => {
  ...
  await verifyTitleContainsText(
    `${voicemailConsentPage.pageHeading} ${generalTitle}`
  );
  await voicemailConsentPage.goBack();

  await verifyTitleContainsText(
    `${telephoneContactPage.pageHeading} ${generalTitle}`
  );
  await t.expect(Selector(telephoneContactPage.yesButton).checked).ok();
  await t
    .expect(Selector(telephoneContactPage.telephoneField).value)
    .eql(sessionData.candidate.telephone.toString());
  await telephoneContactPage.goBack();

  await verifyTitleContainsText(
    `${contactDetailsPage.pageHeadingNSA} ${generalTitle}`
  );
  await t
  ...
  await verifyTitleContainsText(
    `${preferredLocationNSAPage.pageHeading} ${generalTitle}`
  );
  await t
    .expect(
      Selector(
        preferredLocationNSAPage.getRadioSelector(
    ...
    .ok();
  await t
    .expect(Selector(preferredLocationNSAPage.preferredLocationTextArea).value)
    .eql(sessionData.currentBooking.preferredLocation);
  await preferredLocationNSAPage.goBack();

  await verifyTitleContainsText(
    `${preferredDayNSAPage.pageHeading} ${generalTitle}`
  );
  await t
    .expect(
      Selector(
        preferredDayNSAPage.getRadioSelector(
    ...
    .ok();
  await t
    .expect(Selector(preferredDayNSAPage.preferredDayTextArea).value)
    .eql(sessionData.currentBooking.preferredDay);
  await preferredDayNSAPage.goBack();

  await verifyTitleContainsText(
    `${stayingNsaPage.pageHeadingEvidence} ${generalTitle}`
  );
  await stayingNsaPage.goBack();

  await verifyTitleContainsText(
    `${customSupportPage.pageHeading} ${generalTitle}`
  );
  await t
  ...
  await verifyTitleContainsText(
    `${selectSupportTypePage.pageHeading} ${generalTitle}`
  );
  ...
  for (let index = 0; index < supportTypes.length; index++) {
    ...
    await t
      .expect(
        Selector(selectSupportTypePage.getOptionSelector(supportTypes[index]))
          .checked
      )
      .ok();
  }
...
test("From Non-Standard moving to Standard Accommodations GB - Verify that session data is persisted when using the back links - from start to check you details", async (t) => {
  ...
  await verifyTitleContainsText(
    `${chooseATestCentrePage.pageHeading} ${generalTitle}`
  );
  await verifyExactText(
  ...
  await verifyTitleContainsText(
    `${findATheoryTestCentrePage.pageHeading} ${generalTitle}`
  );
  await t
    .expect(Selector(findATheoryTestCentrePage.searchLocationTermTextBox).value)
    .eql(sessionData.testCentreSearch.searchQuery);
  await findATheoryTestCentrePage.goBack();

  await verifyTitleContainsText(
    `${contactDetailsPage.pageHeading} ${generalTitle}`
  );
  await t
    .expect(Selector(contactDetailsPage.emailAddressTextBox).value)
    .eql(sessionData.candidate.email);
  ...
  await verifyTitleContainsText(
    `${leavingNsaPage.pageHeading} ${generalTitle}`
  );
...
test("Non-Standard Accommodations NI - Verify that session data is persisted when using the back links - from start to check you details", async (t) => {
  ...
  await verifyTitleContainsText(
    `${voicemailConsentPage.pageHeading} ${generalTitleNI}`
  );
  await voicemailConsentPage.goBack();

  await verifyTitleContainsText(
    `${telephoneContactPage.pageHeading} ${generalTitleNI}`
  );
  await t.expect(Selector(telephoneContactPage.yesButton).checked).ok();
  await t
    .expect(Selector(telephoneContactPage.telephoneField).value)
  ...
  await verifyTitleContainsText(
    `${contactDetailsPage.pageHeadingNSA} ${generalTitleNI}`
  );
  await t
    .expect(Selector(contactDetailsPage.emailAddressTextBox).value)
    .eql(sessionData.candidate.email);
  ...
  await verifyTitleContainsText(
    `${preferredLocationNSAPage.pageHeading} ${generalTitleNI}`
  );
  await t
    .expect(
      Selector(
  ...
  await verifyTitleContainsText(
    `${preferredDayNSAPage.pageHeading} ${generalTitleNI}`
  );
  await t
    .expect(
      Selector(
  ...
  await verifyTitleContainsText(
    `${stayingNsaPage.pageHeadingEvidence} ${generalTitleNI}`
  );
  await stayingNsaPage.goBack();

  await verifyTitleContainsText(
    `${customSupportPage.pageHeading} ${generalTitleNI}`
  );
  await t
    .expect(Selector(customSupportPage.supportTextArea).value)
    .eql(sessionData.currentBooking.customSupport);
  ...
  await verifyTitleContainsText(
    `${voiceoverPage.pageHeading} ${generalTitleNI}`
  );
  await t
    .expect(
      Selector(
  ...
  await verifyTitleContainsText(
    `${selectSupportTypePage.pageHeading} ${generalTitleNI}`
  );
  const supportTypes = sessionData.currentBooking.selectSupportType;
  for (let index = 0; index < supportTypes.length; index++) {
    // eslint-disable-next-line no-await-in-loop
...
test("From Non-Standard moving to Standard Accommodations NI - Verify that session data is persisted when using the back links - from start to check you details", async (t) => {
  ...
  await verifyTitleContainsText(
    `${preferredDatePage.pageHeading} ${generalTitleNI}`
  );
  await verifyContainsText(
    preferredDatePage.pageHeadingLocator,
    preferredDatePage.pageHeading
  ...
  await verifyTitleContainsText(
    `${chooseATestCentrePage.pageHeading} ${generalTitleNI}`
  );
  await verifyExactText(
    chooseATestCentrePage.searchQueryValue,
    sessionData.testCentreSearch.searchQuery
  ...
  await verifyTitleContainsText(
    `${findATheoryTestCentrePage.pageHeading} ${generalTitleNI}`
  );
  await t
    .expect(Selector(findATheoryTestCentrePage.searchLocationTermTextBox).value)
    .eql(sessionData.testCentreSearch.searchQuery);
  ...
  await verifyTitleContainsText(
    `${contactDetailsPage.pageHeading} ${generalTitleNI}`
  );
  await t
    .expect(Selector(contactDetailsPage.emailAddressTextBox).value)
    .eql(sessionData.candidate.email);
  ...
  await verifyTitleContainsText(
    `${leavingNsaPage.pageHeading} ${generalTitleNI}`
  );
  await leavingNsaPage.goBack();

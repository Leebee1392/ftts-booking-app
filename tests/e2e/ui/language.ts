...
import { Language, Locale, Target } from "../../../src/domain/enums";
...
test("Verify page UI contents are displayed correctly", async () => {
  ...
  await verifyExactText(
    languagePage.languageHintLocator,
    languagePage.languageHintText
  );
...
test("Verify error prefix appears in the title when there is an error", async () => {
  ...
  await verifyTitleContainsText(
    `${languagePage.pageHeading} ${Constants.generalTitle}`
  );
});

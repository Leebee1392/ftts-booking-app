...
describe("TestVoiceover", () => {
  describe("fromCRMVoiceover", () => {
    test("correctly maps Arabic Voiceover from CRM", () => {
      expect(TestVoiceover.fromCRMVoiceover(CRMVoiceOver.Arabic)).toEqual(
        Voiceover.ARABIC
      );
    });
    test("correctly maps Cantonese Voiceover from CRM", () => {
      expect(TestVoiceover.fromCRMVoiceover(CRMVoiceOver.Cantonese)).toEqual(
        Voiceover.CANTONESE
      );
    });
    test("correctly maps English Voiceover from CRM", () => {
      expect(TestVoiceover.fromCRMVoiceover(CRMVoiceOver.English)).toEqual(
        Voiceover.ENGLISH
      );
    });
    test("correctly maps Farsi Voiceover from CRM", () => {
      expect(TestVoiceover.fromCRMVoiceover(CRMVoiceOver.Farsi)).toEqual(
        Voiceover.FARSI
      );
    });
    test("correctly maps Polish Voiceover from CRM", () => {
      expect(TestVoiceover.fromCRMVoiceover(CRMVoiceOver.Polish)).toEqual(
        Voiceover.POLISH
      );
    });
    test("correctly maps Portuguese Voiceover from CRM", () => {
      expect(TestVoiceover.fromCRMVoiceover(CRMVoiceOver.Portuguese)).toEqual(
        Voiceover.PORTUGUESE
      );
    });
    test("correctly maps Turkish Voiceover from CRM", () => {
      expect(TestVoiceover.fromCRMVoiceover(CRMVoiceOver.Turkish)).toEqual(
        Voiceover.TURKISH
      );
    });
    test("correctly maps Welsh Voiceover from CRM", () => {
      expect(TestVoiceover.fromCRMVoiceover(CRMVoiceOver.Welsh)).toEqual(
        Voiceover.WELSH
      );
    });
    test("correctly maps No Voiceover from CRM", () => {
      expect(TestVoiceover.fromCRMVoiceover(CRMVoiceOver.None)).toEqual(
        Voiceover.NONE
      );
    });
    test("returns None if the voiceover is not supported by the booking app", () => {
      expect(TestVoiceover.fromCRMVoiceover(CRMVoiceOver.Mirpuri)).toEqual(
        Voiceover.NONE
      );
    });
  });
  ...
  describe("availableOptions", () => {
    test("returns all DVSA Options if the target is GB, for any OCB test type", () => {
      ...
      expect(result).toStrictEqual([Voiceover.ENGLISH, Voiceover.WELSH]);
    });
    ...
    test("returns limited DVA Options if the target is NI, for non-car/motorcycle tests", () => {
      ...
      expect(result).toStrictEqual([Voiceover.ENGLISH]);
    });

    test.each([TestType.ADIP1, TestType.ADIHPT])(
      "%s test type is valid voiceover option",
      (testType: TestType) => {
    ...
    test.each([TestType.ADIP1DVA, TestType.AMIP1])(
      "%s test type should not have any voice over options",
      (testType: TestType) => {
  ...
  describe("isAvailable", () => {
    test.each([TestType.ADIP1DVA, TestType.AMIP1])(
      "DVA Instructor %s test type should return false (no voiceover options available)",
      (testType: TestType) => {
    ...
    test.each([TestType.ADIP1, TestType.CAR])(
      "DVSA/DVA %s test type should return true (voiceover is available)",
      (testType: TestType) => {
        const result = TestVoiceover.isAvailable(testType);

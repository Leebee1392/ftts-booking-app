...
import { EvidencePath, Target, TestType } from "../../../src/domain/enums";
...
export class BookingConfirmationPage extends BasePage {
  ...
  importantSubheadingNsa =
    'h3[data-automation-id="booking-confirmation-important"]';

  importantBookingReferenceEvidenceNsa = `${this.importantSubheadingNsa} ~ p.govuk-body > strong`;

  otherWaysToSendEvidenceSubheadingNsa =
    'h3[data-automation-id="other-ways-to-send-evidence"]';

  yourEvidenceSubheadingNsa = 'h3[data-automation-id="your-evidence"]';

  whenWeReceiveYourEvidenceSubheadingNsa =
    'h3[data-automation-id="when-we-get-your-evidence"]';

  evidenceMaybeRequiredSubheadingNsa =
    'h3[data-automation-id="when-evidence-might-be-needed"]';
  ...
  async checkDataMatchesSession(sessionData: SessionData): Promise<void> {
    ...
    await verifyContainsText(
      this.bookingConfirmationMessageLocator,
      this.bookingConfirmationText
    );
    await verifyContainsText(
      this.bookingConfirmationMessageLocator,
      this.referenceText
  ...
  async checkHeadingsAppearCorrectly(sessionData: SessionData): Promise<void> {
    ...
    await verifyContainsText(
      this.importantSubheading,
      this.importantHeaderText
    );
    await verifyContainsText(this.centreSubheading, this.centreHeaderText);
    await verifyContainsText(
      this.belongingsSubheading,
      this.bleongingsHeaderText
    );
  ...
  async checkDataMatchesSessionNSA(
    sessionData: SessionData,
    evidencePath: EvidencePath
  ): Promise<void> {
    await this.checkHeadingsAppearCorrectlyNSA(evidencePath);

    let confirmationPageTitle = "";
    let bookingRefLocator = "";
    if (evidencePath === EvidencePath.EVIDENCE_REQUIRED) {
      confirmationPageTitle = this.pageHeadingNSAEvidence;
      bookingRefLocator = this.bookingReferenceLocatorNSAEvidence;
    } else if (
      evidencePath === EvidencePath.EVIDENCE_NOT_REQUIRED ||
      evidencePath === EvidencePath.EVIDENCE_MAY_BE_REQUIRED
    ) {
    ...
    if (evidencePath === EvidencePath.EVIDENCE_REQUIRED) {
      await verifyExactText(
        this.importantBookingReferenceEvidenceNsa,
        sessionData.currentBooking.bookingRef
      );
    }
  }

  async checkHeadingsAppearCorrectlyNSA(
    evidencePath: EvidencePath
  ): Promise<void> {
    ...
    await verifyContainsText(
      this.nsaTestDetailsKey,
      this.testSupportRequested,
      2
    );
    await verifyContainsText(
      this.nsaTestDetailsKey,
      this.testSupportTypesSelected,
      3
    );
    ...
    await verifyContainsText(
      this.bookingConfirmationMessageLocator,
      this.keepReferenceSafe
    );
    if (evidencePath === EvidencePath.EVIDENCE_REQUIRED) {
      await verifyContainsText(
        this.bookingConfirmationMessageLocator,
        this.nsaEvidenceRequiredText
      );
      await verifyContainsText(
        this.nsaTestDetailsHeading,
        this.testSupportDetails
      );
      await verifyContainsText(this.whatToDoNextHeadingNsa, this.whatToDoNow);
      await verifyContainsText(
        this.emailEvidenceSubheadingNsa,
        this.emailEvidence
      );
      await verifyContainsText(
        this.importantSubheadingNsa,
        this.importantHeaderText
      );
      await verifyContainsText(
        this.otherWaysToSendEvidenceSubheadingNsa,
        this.otherWaysToSendEvidence
      );
      await verifyContainsText(
        this.yourEvidenceSubheadingNsa,
        this.yourEvidence
      );
      await verifyContainsText(
        this.whenWeReceiveYourEvidenceSubheadingNsa,
        this.whenWeReceiveEvidence
      );
      await verifyContainsText(
        this.contactDetailsSubheadingNsa,
        this.contactUs
      );
    } else if (evidencePath === EvidencePath.EVIDENCE_NOT_REQUIRED) {
      await verifyNotContainsText(
        this.bookingConfirmationMessageLocator,
        this.nsaEvidenceRequiredText
      );
      await verifyContainsText(
    ...
    } else if (evidencePath === EvidencePath.EVIDENCE_MAY_BE_REQUIRED) {
      await verifyNotContainsText(
        this.bookingConfirmationMessageLocator,
        this.nsaEvidenceRequiredText
      );
      await verifyContainsText(
        this.evidenceMaybeRequiredSubheadingNsa,
        this.evidenceMaybeRequired
      );
      await verifyContainsText(
        this.contactDetailsSubheadingNsa,
        this.contactUs

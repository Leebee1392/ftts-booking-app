...
test("Verify page title, heading and UI contents of Manage bookings page", async () => {
  ...
  await verifyTitleContainsText(
    `${ManageBookingsPage.pageHeading} ${generalTitle}`
  );
  ...
  await verifyExactText(
    ManageBookingsPage.tableHead,
    ManageBookingsPage.column1,
    0
  );
  await verifyExactText(
    ManageBookingsPage.tableHead,
    ManageBookingsPage.column2,
    1
  );
  await verifyExactText(
    ManageBookingsPage.tableHead,
    ManageBookingsPage.column3,
    2
  );
  await verifyExactText(
    ManageBookingsPage.tableHead,
    ManageBookingsPage.column4,
    3
  );
  await verifyExactText(
    ManageBookingsPage.tableHead,
    ManageBookingsPage.column5,
    4
  );
...
if (runningTestsLocally()) {
  ...
  test("Verify the user can see an information banner about a failed booking reference in 'Change In Progress' status (from a previously failed reschedule)", async () => {
    ...
    await loginPage.login(
      Constants.bookingReference4,
      Constants.drivingLicenceErrorHandling
    );

    await verifyIsVisible(ManageBookingsPage.bookingsWithErrorsBanner);
    await verifyExactText(
      ManageBookingsPage.bookingsWithErrorsBannerTitle,
      "Important"
    );
    await verifyExactText(
      ManageBookingsPage.bookingsWithErrorsBannerHeader,
      ManageBookingsPage.infoBannerHeaderText
    );
    await verifyContainsText(
      ManageBookingsPage.bookingsWithErrorsBannerBookingRefList,
      Constants.bookingReference4
    );
  });
}
...
async function checkTableData(
  rowNumber: number,
  expTestType: string,
  unitInFuture: dayjs.OpUnitType,
  valueInFuture: number,
  bookingRef: string
) {
  const rowOffset = (rowNumber - 1) * 4;
  await verifyContainsText(
    ManageBookingsPage.tableCell,
    expTestType,
    ManageBookingsPage.TEST_TYPE_INDEX + rowOffset
  );
  ...
  await verifyContainsText(
    ManageBookingsPage.tableCell,
    expTestTime,
    ManageBookingsPage.TEST_DATE_INDEX + rowOffset
  );
  await verifyContainsText(
    ManageBookingsPage.tableCell,
    expTestDate,
  ...
  await verifyContainsText(
    ManageBookingsPage.tableCell,
    bookingRef,
    ManageBookingsPage.BOOKING_REFERENCE_INDEX + rowOffset
  );
  await verifyContainsText(
    ManageBookingsPage.tableCell,
    "BOOKED",
    ManageBookingsPage.STATUS_INDEX + rowOffset
  );
  await verifyContainsText(
    ManageBookingsPage.tableCell,
    "View booking",
    ManageBookingsPage.ACTIONS_INDEX + rowOffset
  );
}

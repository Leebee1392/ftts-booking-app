...
export class ManageBookingCancelController {
  ...
  public get = (req: Request, res: Response): void => {
    if (!req.session.manageBooking) {
      logger.warn(
        "ManageBookingCancelController::get: No session manageBooking set"
      );
  ...
  public post = async (req: Request, res: Response): Promise<void> => {
    if (!req.session.manageBooking) {
      throw Error(
        "ManageBookingCancelController::post: No session manageBooking set"
      );
    }
    const { candidate } = req.session.manageBooking;
    ...
    try {
      ...
      const testType: TestType = fromCRMProductNumber(
        booking.details.product?.productnumber as CRMProductNumber
      );
      ...
      logger.error(
        error as Error,
        "ManageBookingCancelController::post: Cancelling failed",
        { bookingReference }
      );
  ...
  private setCancelInProgressInCRM = async (
    booking: Booking
  ): Promise<void> => {
    try {
      logger.info(
        "ManageBookingCancelController::setCancelInProgressInCRM: Attempting to set booking status of Cancellation in Progress",
        {
          bookingRef: booking.details.reference,
        }
      );
      return await this.crmGateway.updateBookingStatus(
        booking.details.bookingId,
        CRMBookingStatus.CancellationInProgress,
        booking?.details?.origin === CRMOrigin.CustomerServiceCentre
      );
    } catch (error) {
      logger.error(
        error as Error,
        "ManageBookingCancelController::setCancelInProgressInCRM: Failed to set status of Cancellation in Progress in CRM after 3 retries",
        {
          bookingRef: booking.details.reference,
        }
      );
      logger.event(
        BusinessTelemetryEvents.CDS_FAIL_BOOKING_CANCEL,
        "ManageBookingCancelController::setCancelInProgressInCRM: Failed to set status of Cancellation in Progress in CRM after 3 retries",
        {
          error,
          bookingRef: booking.details.reference,
  ...
  private handlePayment = async (
    ...
    if (!candidate.address || !candidate.personReference) {
      throw new Error(
        "ManageBookingCancelController::handlePayment: Missing required candidate session data"
      );
    }
    if (booking.isRefundable()) {
      const payload: PaymentCandidate.BatchRefundPayload =
        buildPaymentRefundPayload(candidate, booking, target);
      logger.info(
        "ManageBookingCancelController::handlePayment: Attempting to request refund",
        {
          candidateId: candidate.candidateId,
        }
      );
      logger.debug(
        "ManageBookingCancelController::handlePayment: Refund payload:",
        {
          payload,
        }
      );
      try {
        const response = await this.paymentClient.requestRefund(
          payload,
        ...
        if (
          response?.code &&
          Number(response.code) !== PaymentStatus.REFUND_SUCCESS
        ) {
          logger.warn(
        ...
        logger.event(
          BusinessTelemetryEvents.PAYMENT_REFUND_FAIL,
          "ManageBookingCancelController::handlePayment: Refund attempt failed",
          {
            error,
            candidateId: candidate.candidateId,
      ...
      logger.info(
        "ManageBookingCancelController::handlePayment: Attempting to recognise income",
        {
          candidateId: candidate.candidateId,
          bookingProductId: booking.details.bookingProductId,
        }
      ...
      try {
        ...
        logger.event(
          BusinessTelemetryEvents.PAYMENT_INCOME_SUCCESS,
          "ManageBookingCancelController::handlePayment: Income recognised",
          {
            bookingProductId: booking.details.bookingProductId,
            candidateId: candidate.candidateId,
            personReference: candidate.personReference,
        ...
        logger.event(
          ...
          "ManageBookingCancelController::handlePayment: Income recognition failure",
          {
            error,
            bookingProductId: booking.details.bookingProductId,
            candidateId: candidate.candidateId,
  ...
  private cancelBookingInCRM = async (booking: Booking): Promise<void> => {
    try {
      logger.info(
        "ManageBookingCancelController::cancelBookingInCRM: Cancelling booking in CRM",
        {
          reference: booking.details.reference,
        }
      );
      await this.crmGateway.markBookingCancelled(
        booking.details.bookingId,
        booking.details.bookingProductId,
        booking?.details?.origin === CRMOrigin.CustomerServiceCentre
      );
    } catch (error) {
      logger.error(
        error as Error,
        "ManageBookingCancelController::cancelBookingInCRM: Cancelling booking in CRM failed",
        {
          reference: booking.details.reference,
        }
      );
  ...
  private deleteScheduledSlot = async (booking: Booking): Promise<void> => {
    try {
      await this.schedulingGateway.deleteBooking(
        booking.details.bookingProductRef,
        booking.details.testCentre.region
      );
      await this.crmGateway.updateTCNUpdateDate(
        booking.details.bookingProductId
      );
    } catch (error) {
      logger.error(
        error as Error,
        "ManageBookingCancelController::deleteScheduledSlot: Unable to successfully delete scheduled slot",
        {
          reference: booking.details.reference,
        }
      );
      logger.event(
        BusinessTelemetryEvents.SCHEDULING_FAIL_CONFIRM_CANCEL,
        "ManageBookingCancelController::deleteScheduledSlot: Failed to cancel slot while cancelling a booking with the Scheduling API",
        {
          error,
          reference: booking.details.reference,
        }
  ...
  private sendCancellationEmail = async (
    booking: Booking,
    candidate: Candidate,
    target: Target,
    lang: Locale
    ...
    try {
      ...
      logger.info(
        "ManageBookingCancelController::sendCancellationEmail: Sending cancellation email",
        {

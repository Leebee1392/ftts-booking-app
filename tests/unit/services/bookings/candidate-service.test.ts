...
describe("CandidateService", () => {
  ...
  beforeEach(() => {
    ...
    globalCandidateService = new CandidateService(
      mockCrmGateway as any,
      mockEligibilityGateway as any
    );
  });
  ...
  describe("alignCandidateDataInCRM", () => {
    describe("candidate has existing licence in CRM", () => {
      describe("and nothing has to be updated", () => {
        test("then candidate and licence are not updated", async () => {
          mockCrmGateway.getLicenceNumberRecordsByCandidateId = jest
            .fn()
            .mockResolvedValueOnce(mockCandidateResponse);
          mockCrmGateway.updateCandidate = jest
            .fn()
            .mockResolvedValueOnce(mockCandidateResponse);

          const result = await globalCandidateService.alignCandidateDataInCRM(
      ...
      describe("and crm licence address has nulls while eligibility licence address has empty strings", () => {
        test("then candidate and licence are not updated", async () => {
          ...
          mockCrmGateway.getLicenceNumberRecordsByCandidateId = jest
            .fn()
            .mockResolvedValueOnce(mockCrmCandidate);
      ...
      describe("and licence has to be updated", () => {
        test("then licence and candidate are updated", async () => {
          ...
          expect(mockCrmGateway.updateLicence).toHaveBeenCalledWith(
            mockLicenceId,
            mockCandidateId,
            mockCandidateResponse.address
          );
          expect(mockCrmGateway.updateCandidate).toHaveBeenCalledTimes(1);
          expect(mockCrmGateway.updateCandidate).toHaveBeenCalledWith(
            mockCandidateId,
            mockCandidateResponse
          );
          expect(mockCrmGateway.createLicence).not.toHaveBeenCalled();
        ...
        test("and crmGateway.updateLicence cause errors then error is thrown", async () => {
          ...
          const promise = globalCandidateService.alignCandidateDataInCRM(
            mockCandidateResponse,
            mockLicenceNumber
          );
    ...
    describe("candidate do not have existing licence in CRM", () => {
      test("then new licence is created and candidate is updated", async () => {
        ...
        mockCrmGateway.createLicence = jest
          .fn()
          .mockResolvedValueOnce(mockLicenceId);
        mockCrmGateway.updateCandidate = jest
          .fn()
          .mockResolvedValueOnce(mockCandidateResponse);
        ...
        expect(mockCrmGateway.createLicence).toHaveBeenCalledWith(
          mockLicenceNumber,
          mockCandidateResponse.address,
          mockCandidateId
        );
  ...
  describe("isDrivingLicenceValid", () => {
    test("valid driving licence returns true", () => {
      const result = CandidateService.isDrivingLicenceValid(
        "JONES061102W97YT",
        meta as any as Meta
      );
    ...
    test("incorrect format driving licences are not valid", () => {
      expect(() =>
        CandidateService.isDrivingLicenceValid(
          "JONES06110",
          meta as any as Meta
        )
    ...
    test("blank driving licences are not valid", () => {
      expect(() =>
        CandidateService.isDrivingLicenceValid("", meta as any as Meta)
  ...
  describe("doesCandidateMatchEligibility", () => {
    test("valid candidate details is eligible", () => {
      const result = globalCandidateService.doesCandidateMatchEligibility(
        candidateDetails,
        mockCandidateResponse,
        Target.GB,
        Locale.GB
    ...
    test("firstnames not matching is invalid", () => {
      ...
      const fieldError = new Error(
        "First name did not match eligibility response"
      );
      ...
      try {
        globalCandidateService.doesCandidateMatchEligibility(
          candidateDetailsInput,
          mockCandidateResponse,
          Target.GB,
          Locale.GB
        ...
        expect((error as MultipleCandidateMismatchError).errors).toStrictEqual([
          fieldError,
        ]);
      }
    });
    ...
    test("surname not matching is invalid", () => {
      expect.assertions(1);
      const fieldError = new Error(
        "Surname did not match eligibility response"
      );
      const candidateDetailsInput = {
        ...candidateDetails,
  ...
  describe("createOrUpdateCandidate", () => {
    describe("creating a candidate", () => {
      test("creates candidate in CRM", async () => {
        ...
        const result = await globalCandidateService.createOrUpdateCandidate(
          eligibilityResponse,
          mockLicenceNumber
        );
      ...
      test("throws error if CRM call fails to create candidate in CRM", async () => {
        ...
        await expect(
          globalCandidateService.createOrUpdateCandidate(
            eligibilityResponse,
            mockLicenceNumber
          )
    ...
    describe("retrieving a candidate", () => {
      test("if candidate exists in CRM, align candidate details", async () => {
        ...
        jest
          .spyOn(globalCandidateService, "alignCandidateDataInCRM")
          .mockResolvedValue({
            licenceId: mockLicenceId,
  ...
  describe("getEligibility", () => {
    test("calls eligbility api with details", async () => {
      ...
      const updateCandidateMock = jest
        .spyOn(globalCandidateService, "createOrUpdateCandidate")
        .mockResolvedValue(mockCandidateResponse);

      const result = await globalCandidateService.getEligibility(
        mockLicenceNumber,
        candidateDetails,
        Target.GB,
        Locale.GB
    ...
    test("throws EligibilityLicenceNotFoundError if licence is not found", async () => {
      ...
      await expect(
        globalCandidateService.getEligibility(
          mockLicenceNumber,
          candidateDetails,
          Target.GB,
          Locale.GB
    ...
    test("throws EligbilityCandidateMismatchError if candidate does not match eligibility response", async () => {
      jest
        .spyOn(globalCandidateService, "doesCandidateMatchEligibility")
  ...
  describe("getManageBookingEligibility", () => {
    test("calls eligbility api with details", async () => {
      ...
      const result = await globalCandidateService.getManageBookingEligibility(
        mockLicenceNumber,
        Target.GB,
        Locale.GB
      );
      ...
      expect(result).toStrictEqual(mockCandidateResponse);
      expect(mockEligibilityGateway.getEligibility).toHaveBeenCalledWith(
        "JONES061102W97YT",
        true,
        Target.GB,
        Locale.GB
    ...
    test("throws EligibilityLicenceNotFoundError if licence is not found", async () => {
      ...
      await expect(
        globalCandidateService.getManageBookingEligibility(
          mockLicenceNumber,
          Target.GB,
          Locale.GB
        )
  ...
  describe("replaceFirstnameWithThreeDashesIfEmptyGB", () => {
    test("returns dash if firstname is empty and target is GB", () => {
      expect(
        globalCandidateService.replaceFirstnameWithThreeDashesIfEmptyGB(
          Target.GB,
          ""
        )
      ).toStrictEqual("---");

...
export class NSANavigator {
  private dynamicRoutes: DynamicRoute[] = [
    {
      option: SupportType.VOICEOVER,
      url: "change-voiceover",
    },
    {
      option: SupportType.TRANSLATOR,
      url: "translator",
    },
    {
      option: SupportType.OTHER,
      url: "custom-support",
    },
  ];
  ...
  public getNextPage = (req: Request): string => {
    const selectedSupportOptions =
      req.session.currentBooking?.selectSupportType;

    if (selectedSupportOptions) {
      this.selectedRoutes = this.dynamicRoutes.filter((page) =>
        selectedSupportOptions.includes(page.option)
      );
    }
    ...
    if (!targetUrl) {
      ...
      if (
        ...
        isOnlyStandardSupportSelected(selectedSupportOptions)
      ) {
  ...
  private getPathFromUrl = (req: Request): string =>

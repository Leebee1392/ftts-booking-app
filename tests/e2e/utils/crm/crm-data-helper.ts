...
export async function createCandidateAndLicence(
  sessionData: SessionData
): Promise<void> {
  const crmGateway = new CRMGateway(dynamicsWebApiClient());

  const crmCandidateId = await crmGateway.createCandidate(
    sessionData.candidate
  );
  const crmLicenceId = await crmGateway.createLicence(
    sessionData.candidate.licenceId,
...
export async function createNewBookingInCrm(
  sessionData: SessionData,
  newCandidateAndLicence = false
): Promise<void> {
  const crmGateway = new CRMGateway(dynamicsWebApiClient());
  ...
  if (newCandidateAndLicence) {
    ...
    const licenceAndProduct = await crmGateway.getLicence(
  ...
  const priceListId =
    sessionData.target === Target.NI ? dvaPriceList : dvsaPriceList;

  const priceList: PriceListItem[] = await crmGateway.getPriceList(
    priceListId,
    [sessionData.currentBooking.testType]
  );
  [sessionData.currentBooking.priceList] = priceList;

  // create booking and bookingproduct records
  const bookingResponse = await crmGateway.createBooking(
    candidateData,
    sessionData.currentBooking,
    getAdditionalSupport(sessionData.currentBooking),
    sessionData.journey.standardAccommodation,
    priceListId,
    sessionData.overrideCreatedOnDate
  );
  ...
  sessionData.currentBooking.bookingProductId =
    await crmGateway.createBookingProduct(
      candidateData,
      sessionData.currentBooking,
      bookingResponse,
      sessionData.journey.standardAccommodation,
  ...
  if (isTestTypeZeroCost) {
    await crmGateway.updateZeroCostBooking(
    ...
    await crmGateway.updatePaymentStatus(
      sessionData.currentBooking.bookingProductId
    );
  ...
  if (sessionData.currentBooking.eligibilityBypass) {
    await crmGateway.enableEligibilityBypass(
      sessionData.currentBooking.bookingId
    );
  }
...
export async function createNewBookingForPaymentsNotifications(
  sessionData: SessionData,
  cancelPayment = true
): Promise<void> {
  const crmGateway = new CRMGateway(dynamicsWebApiClient());
  ...
  const cardPaymentResponse: CardPaymentResponse = await initiateCardPayment(
    sessionData
  );
  ...
  console.log(
    `Booking ref: ${bookingResponse.reference} Booking Id: ${
      bookingResponse.id
    } Receipt Reference: ${cardPaymentResponse.receiptReference} Payment Id: ${
      cardPaymentResponse.paymentId
    } Created on: ${dayjs().toISOString()}`
  );

  if (cancelPayment) {
    await t.navigateTo(cardPaymentResponse.gatewayUrl);
    await new PaymentsPage().cancelPayment(
      sessionData.paymentDetails as CpmsPaymentModel
    );
  }
}
...
function getAdditionalSupport(currentBooking: Booking): CRMAdditionalSupport {
  ...
  if (
    ...
    currentBooking?.voiceover.toLowerCase() === "welsh"
  ) {
    return CRMAdditionalSupport.VoiceoverWelsh;
  }
  if (
    currentBooking?.voiceover &&
    currentBooking?.voiceover.toLowerCase() === "english"
  ) {

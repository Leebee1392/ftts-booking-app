...
export class CRMGateway {
  ...
  public async createBooking(
    ...
    try {
      ...
      if (!candidate.candidateId || !candidate.licenceId) {
        throw new Error(
          "CRMGateway::createBooking: Missing required candidate data"
        );
      }
      ...
      this.createBookingRequest(
        ...
        priceListId,
        overrideCreatedOnDate
      );
      const response = await this.dynamicsWebApi.executeBatch();
      const bookingResponse: BookingResponse =
        mapCRMBookingResponseToBookingResponse(
          response[0] as CRMBookingResponse
        );

      return bookingResponse;
    } catch (error) {
      logger.critical(
        "CRMGateway::createBooking: Could not create the booking in CRM",
        { error }
      );
      throw error;
    }
  ...
  private async createBookingRequest(
    ...
    const request = {
      ...
      entity: {
        ...
        overriddencreatedon: overrideCreatedOnDate
  ...
  public async getLicence(
    licenceNumber: string
  ): Promise<LicenceBatchResponse> {
    try {
      logger.info(
        "CRMGateway::getLicence: Attempting to retrive Candidate, Licence and Product from CRM"
      );

      const licences: CRMLicenceResponse[] = await this.getLicenceRequest(
        licenceNumber
      );
      ...
      logger.error(
        error,
        "CRMGateway::getLicence: Could not retrieve licence from CRM"
      );
      throw error;
    }
  }
  ...
  private async getLicenceRequest(licenceNumber: string): Promise<any> {
    const response =
      await this.dynamicsWebApi.retrieveMultipleRequest<CRMLicenceResponse>({
        collection: "ftts_licences",
  ...
  public async getPersonReference(candidateId: string): Promise<any> {
    const response =
      await this.dynamicsWebApi.retrieveMultipleRequest<CRMContact>({
        collection: "contacts",
  ...
  public async updateZeroCostBooking(bookingId: string): Promise<void> {
    ...
    try {
      ...
      logger.info("CRMGateway::updateZeroCostBooking: Raw Request", {
        request,
        bookingId,
      });
      await this.dynamicsWebApi.updateRequest(request);
  ...
  public async createPayment(sessionData: SessionData): Promise<string> {
    try {
      ...
      logger.error(
        error,
        "CRMGateway::createPayment: Could not create record in CRM"
      );
      throw error;
    }
  }

  public async createFinanceTransaction(
    sessionData: SessionData,
    paymentId: string
  ): Promise<void> {
    try {
      logger.info(
        "CRMGateway::createFinanceTransaction: Attempting to create finance transaction"
      );
      ...
      logger.info(
        "CRMGateway::createFinanceTransaction: Created finance transaction successfully"
      );
    } catch (error) {
      logger.error(
        error,
        "CRMGateway::createFinanceTransaction: Could not create record in CRM"
      );
      throw error;
    }
  }
  ...
  public async updatePaymentStatus(bookingProductId: string): Promise<void> {
    try {
      logger.info(
        "CRMGateway:updatePaymentStatus: Attempting to update payment status"
      );
  ...
  public async enableEligibilityBypass(bookingId: string): Promise<void> {
    try {
      logger.info(
        "CRMGateway:enableEligibilityBypass: Attempting to update eligibility bypass",
        { bookingId }
      );
  ...
  public async setCompensationBookingAssigned(
    bookingId: string,
    assignedDate: string
  ): Promise<void> {
    try {
      logger.info(
        "CRMGateway:setCompensationBookingAssigned: Attempting to update compensation booking assigned",
        { bookingId }
      );
  ...
  public async setCompensationBookingRecognised(
    bookingId: string,
    recognisedDate: string
  ): Promise<void> {
    try {
      logger.info(
        "CRMGateway:setCompensationBookingRecognised: Attempting to update compensation booking recognised",
        { bookingId }
      );
  ...
  public async assignTestSupportNeedsToBooking(
    bookingId: string,
    ...
    try {
      logger.info(
        "CRMGateway:assignTestSupportNeedsToBooking: Attempting to assign test support needs",
        { bookingId }
      );
  ...
  public async cleanUpBookingProducts(bookingProductId: string): Promise<void> {
    if (bookingProductId) {
      try {
        logger.info(
          "CRMGateway:cleanUpBookingProducts: Attempting to cancel booking product",
          { bookingProductId }
        );
  ...
  public async cleanUpBookingProductsByBookingRef(
    ...
    if (bookingReference) {
      try {
        ...
        const response = await this.dynamicsWebApi.retrieveMultipleRequest<any>(
          requestBookingProductId
        );

        if (response.value[0].ftts_bookingproductid) {
          await this.cleanUpBookingProducts(
            response.value[0].ftts_bookingproductid
          );
        }
      } catch (error) {
        console.error(error);

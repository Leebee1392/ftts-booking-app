...
test("Verify a candidate is able to change the date and time of their booking - NI", async (t) => {
  ...
  const timeAndDateOnly =
    await whatDoYouWantToChangePage.selectTimeAndDateOnly();
  const chooseAppointment =
    await timeAndDateOnly.selectPreferredDateWithAppointments(
...
test("Verify a candidate is able to change the location of their booking - NI", async (t) => {
  ...
  const findATheoryTestCentrePage =
    await whatDoYouWantToChangePage.selectLocationOnly();
  const chooseATestCentrePage =
    await findATheoryTestCentrePage.findATheoryTestCentre("Newry");
  const preferredDatePage = await chooseATestCentrePage.selectATestCentre(

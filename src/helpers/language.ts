...
import { Locale, Target, TARGET_LOCALE_MAP } from "../domain/enums";
...
export const changeLanguageToLocale = async (
  req: Request,
  res: Response,
  locale: Locale
): Promise<void> => {
  if (locale === Locale.CY) {
    ...
    res.locals.surveyUrl =
      req.session.locale === Locale.NI ? config.survey.ni : config.survey.gb;
  ...
  try {
    ...
    logger.error(
      e as Error,
      "changeLanguageToLocale:: could not change language"
    );
    throw e;
  }
...
export const isLocaleAvailableForTarget = (
  locale: Locale,
  target: Target
): boolean => {
...
export const getDefaultLocale = (target: Target): Locale =>
  target === Target.NI ? Locale.NI : Locale.GB;
...
export const setCorrectLanguage = async (
  req: Request,
  res: Response,
  target: Target
): Promise<Locale> => {

...
async function navigateToSelectSupportTypePage(target: Target, locale: Locale) {
  ...
  await t.navigateTo(
    `${process.env.BOOKING_APP_URL}/${selectSupportTypePage.pathUrl}?target=${target}`
  );
}
...
test("Verify a validation error message is displayed when not selecting any support types", async () => {
  ...
  await verifyContainsText(
    selectSupportTypePage.errorMessageLocator,
    selectSupportTypePage.errorMessageHeader
  );
  await verifyContainsText(
    selectSupportTypePage.errorMessageList,
    selectSupportTypePage.errorMessageNothingSelectText
  );
  await verifyContainsText(
    selectSupportTypePage.errorMessageFieldLocator,
    selectSupportTypePage.errorMessageNothingSelectText
  );
...
test("Verify a validation error message is displayed when attempting to select Onscreen BSL and Voiceover together", async () => {
  ...
  await verifyContainsText(
    selectSupportTypePage.errorMessageList,
    selectSupportTypePage.errorMessageBslVoiceoverSelectedText
  );
  await verifyContainsText(
    selectSupportTypePage.errorMessageFieldLocator,
    selectSupportTypePage.errorMessageBslVoiceoverSelectedText

...
export class NavigationHelperInstructor {
  ...
  async navigateToChooseSupportPage(): Promise<ChooseSupportPage> {
    ...
    await t.navigateTo(
      `${process.env.BOOKING_APP_URL}/instructor?target=${this.sessionData.target}`
    );
  ...
  async navigateToCandidateDetailsPage(): Promise<CandidateDetailsPage> {
    ...
    if (this.sessionData.journey.support) {
      throw new Error(
        "Please use NavigationHelperNSA for Non-Standard Accommodation journeys!"
      );
    }
  ...
  async navigateToContactDetailsPage(): Promise<ContactDetailsPage> {
    if (this.sessionData.target === Target.NI) {
      ...
      await candidateDetailsPage.enterInstructorCandidateDetailsAndSubmit(
        this.sessionData.candidate,
        this.sessionData.target as Target
      );
  ...
  async navigateToNiTestTypePage(): Promise<TestTypePage> {
    const contactDetailsPage = await this.navigateToContactDetailsPage();
    const testTypePage = await contactDetailsPage.enterContactDetails(
      this.sessionData.candidate.email,
      this.sessionData.candidate.email
    );
    return testTypePage;
  ...
  async navigateToVoiceoverPage(): Promise<void> {
    if (this.sessionData.hasComeFromNsaJourney) {
      ...
      this.sessionData.target === Target.GB &&
      this.sessionData.currentBooking.testType !== TestType.ERS
    ) {
    ...
    if (
      this.sessionData.target === Target.GB &&
      !this.sessionData.hasComeFromNsaJourney
    ) {
  ...
  async navigateToChooseTestCentrePage(): Promise<ChooseATestCentrePage> {
    ...
    const chooseATestCentrePage =
      await findATheoryTestCentrePage.findATheoryTestCentre(
        this.sessionData.testCentreSearch.searchQuery
      );
  ...
  async navigateToPreferredDatePage(): Promise<PreferredDatePage> {
    ...
    const preferredDatePage =
      await chooseATestCentrePage.selectATestCentreWithName(
        this.sessionData.currentBooking.centre.name,
        this.sessionData.currentBooking.centre
      );
  ...
  async navigateToChooseAppointmentPage(): Promise<ChooseAppointmentPage> {
    ...
    if (this.sessionData.testDateLessThan3Days === true) {
      chooseAppointmentPage =
        await preferredDatePage.selectPreferredDateLessThan3Days(
          this.sessionData.testCentreSearch
        );
    } else if (this.sessionData.specificDate) {
      const preferredDate = dayjs(
        this.sessionData.testCentreSearch.selectedDate
      );
      chooseAppointmentPage =
  ...
  async navigateToCheckBookingPage(): Promise<CheckYourDetailsPage> {
    ...
    const checkYourBookingPage = await chooseAppointmentPage.chooseAppointment(
      this.sessionData.currentBooking
    );
  ...
  async createANewBooking(): Promise<BookingConfirmationPage> {
    ...
    if (
      this.sessionData.target === Target.GB &&
      !this.sessionData.isCompensationBooking &&
      !runningTestsLocally()
    ) {
      const makePaymentPage = new PaymentsPage();
      await makePaymentPage.makePayment(
        this.sessionData.paymentDetails,
        this.sessionData.candidate.email
      );
    }
    ...
    this.sessionData.currentBooking.bookingRef = await getText(
      bookingConfirmationPage.bookingReferenceLocator
    );

...
export class BookingConfirmation {
  ...
  public get = (req: Request, res: Response): void => {
    ...
    const hasSupportNeedsInCRM = inSupportMode
      ? hasCRMSupportNeeds(req.session.candidate as Candidate)
      : false;
    const typeOfEvidenceRequired = inSupportMode
      ? determineEvidenceRoute(selectedSupportOptions, hasSupportNeedsInCRM)
      : undefined;
    ...
    if (inSupportMode) {
      ...
      const preferLocation =
        preferredLocationOption === PreferredLocation.DecideLater
          ...
          : preferredLocation;
      return res.render(
        this.getEvidenceViewPath(typeOfEvidenceRequired as EvidencePath),
        {
          bookingRef,
          inSupportMode,
  ...
  private getSelectedSupportOptions(req: Request): SupportType[] {
    try {
      ...
      throw new Error(
        "BookingConfirmation::getSelectedSupportOptions: No support options provided"
      );
    }
  }
...
export default new BookingConfirmation(CRMGateway.getInstance());

...
test("Verify page UI contents are displayed correctly - GB", async () => {
  const sessionDataNSA = new SessionData(
    Target.GB,
    Locale.GB,
    true,
    false,
  ...
  await verifyTitleContainsText(
    `${checkYourDetailsNsaPage.pageHeading} ${generalTitle}`
  );
  await verifyContainsText(
    checkYourDetailsNsaPage.pageHeadingLocator,
    checkYourDetailsNsaPage.pageHeading
  );

  await verifyExactText(
    checkYourDetailsNsaPage.personalDetailsHeadingLocator,
    "Personal details"
  );
  await verifyExactText(checkYourDetailsNsaPage.personalDetailsKey, "Name", 0);
  await verifyExactText(
    checkYourDetailsNsaPage.personalDetailsKey,
    "Date of birth",
    1
  );
  await verifyExactText(
  ...
  await verifyExactText(
    checkYourDetailsNsaPage.personalDetailsKey,
    "Consent to leave you phone messages",
    5
  );

  await verifyExactText(
    checkYourDetailsNsaPage.testDetailsHeadingLocator,
    "Test details"
  );
  await verifyExactText(checkYourDetailsNsaPage.testDetailsKey, "Test type", 0);
  await verifyExactText(
    checkYourDetailsNsaPage.testDetailsKey,
    "On-screen language",
    1
  );

  await verifyExactText(
    checkYourDetailsNsaPage.supportDetailsHeadingLocator,
    "Support Details"
  );
  await verifyExactText(
    checkYourDetailsNsaPage.supportDetailsKey,
    "Support requested",
    0
  );
  await verifyExactText(
    checkYourDetailsNsaPage.supportDetailsKey,
    "Support types you selected",
    1
  );
  await verifyExactText(
    checkYourDetailsNsaPage.supportDetailsKey,
...
test("Change Support Requested for GB from yes to no - save and the updated option is shown", async () => {
  ...
  const supportRequestedPage =
    await checkYourDetailsNsaPage.changeSupportRequested();
...
test("Change Voiceover - the correct content is displayed", async () => {
  ...
  await t
    .expect(
      Selector(
        voiceoverPage.getRadioSelector(sessionData.currentBooking.voiceover)
      ).checked
    )
    .ok();
  await verifyExactText(
...
test("Change Translator (NI only) - the correct content is displayed", async () => {
  ...
  const translatorPage =
    await checkYourDetailsNsaPage.changeTranslatorLanguage();
  await verifyContainsText(
  ...
  await verifyContainsText(
    translatorPage.updateBanner,
    translatorPage.updateBannerText
  );
  await t
    .expect(Selector(translatorPage.translatorTextArea).value)
    .eql(sessionData.currentBooking.translator);
  await verifyContainsText(
    translatorPage.continueButton,
    translatorPage.updateButtonText
  );
  await verifyContainsText(
    translatorPage.cancelButton,
    translatorPage.cancelButtonText
  );
});
...
test("Change Translator (NI only) - save and the new translator details are shown", async () => {
  ...
  await translatorPage.enterTranslatorDetails(
...
test("Change Custom suppport - the correct content is displayed", async () => {
  ...
  await verifyExactText(
    customSupportPage.updateBanner,
    customSupportPage.updateBannerText
  );
  await t
    .expect(Selector(customSupportPage.supportTextArea).value)
    .eql(sessionData.currentBooking.customSupport);
  await verifyExactText(
    customSupportPage.continueButton,
    customSupportPage.updateButtonText
  );
  await verifyExactText(
    customSupportPage.cancelButton,
    customSupportPage.cancelButtonText
  );
});
...
test("Change Custom suppport - save and the new custom support details are shown", async () => {
  ...
  sessionData.currentBooking.customSupport =
    "My new support requirements are...";
  await customSupportPage.enterSupportInformation(
    sessionData.currentBooking.customSupport
  );
  await checkYourDetailsNsaPage.checkDataMatchesSession(sessionData);
});
...
test("Change Preferred day - the correct content is displayed", async () => {
  ...
  await verifyExactText(
    preferredDayPage.pageHeadingLocator,
    preferredDayPage.pageHeading
  );
  await verifyIsNotVisible(preferredDayPage.backLink);
  await verifyExactText(
    preferredDayPage.updateBanner,
    preferredDayPage.updateBannerText
  );
  await t
    .expect(Selector(preferredDayPage.preferredDayTextArea).value)
    .eql(sessionData.currentBooking.preferredDay);
  await verifyExactText(
    preferredDayPage.continueButton,
    preferredDayPage.updateButtonText
  );
  await verifyExactText(
    preferredDayPage.cancelButton,
    preferredDayPage.cancelButtonText
  );
});
...
test("Change Preferred day - save and the new preferred day details are shown", async () => {
  ...
  await preferredDayPage.enterPreferredDayInformation(
...
test("Change Preferred location - the correct content is displayed", async () => {
  ...
  const preferredLocationPage =
    await checkYourDetailsNsaPage.changePreferredLocation();
  await verifyExactText(
    preferredLocationPage.pageHeadingLocator,
    preferredLocationPage.pageHeading
  );
  await verifyIsNotVisible(preferredLocationPage.backLink);
  await verifyExactText(
    preferredLocationPage.updateBanner,
    preferredLocationPage.updateBannerText
  );
  await t
    .expect(Selector(preferredLocationPage.preferredLocationTextArea).value)
    .eql(sessionData.currentBooking.preferredLocation);
  await verifyExactText(
    preferredLocationPage.continueButton,
    preferredLocationPage.updateButtonText
  );
  await verifyExactText(
    preferredLocationPage.cancelButton,
    preferredLocationPage.cancelButtonText
  );
});
...
test("Change Preferred location - save and the new preferred location details are shown", async () => {
  ...
  await preferredLocationPage.enterPreferredLocationInformation(
...
test("Change Selected support types - the correct content is displayed", async () => {
  ...
  const selectSupportTypePage =
    await checkYourDetailsNsaPage.changeSupportTypes();
  await verifyExactText(
  ...
  await verifyContainsText(
    selectSupportTypePage.updateBanner,
    selectSupportTypePage.updateBannerText
  );
  await verifyContainsText(
    selectSupportTypePage.updateBanner,
    selectSupportTypePage.updateBannerWarningText
  );
  for (
    let index = 0;
    index < sessionData.currentBooking.selectSupportType.length;
    index++
  ) {
    ...
    await t
      .expect(
        Selector(
          selectSupportTypePage.getOptionSelector(
            sessionData.currentBooking.selectSupportType[index]
          )
        ).checked
      )
      .ok();
  ...
  await verifyExactText(
    selectSupportTypePage.continueButton,
    selectSupportTypePage.updateButtonText
  );
  await verifyExactText(
    selectSupportTypePage.cancelButton,
    selectSupportTypePage.cancelButtonText
  );
});
...
test("Change Selected support types (GB) - save, go through remaining NSA journey and the new selected support types are shown", async () => {
  ...
  sessionData.currentBooking.voiceover = Voiceover.WELSH;
  await selectSupportTypePage.selectSupportTypes(
    sessionData.currentBooking.selectSupportType
  );
...
test("Change Telephone number and Voicemail - updated details are displayed", async () => {
  ...
  const nsaVoicemailPage = await telephoneNsaPage.enterTelephoneNumber(
    "00000000000"
  );
  ...
  const nsaNewVoicemailPage = await newTelephoneNsaPage.enterTelephoneNumber(
    "11111111111"
  );
...
test("Change Email Address - updated details are displayed", async () => {
  ...
  await contactDetailsPage.enterContactDetails(
    "test@hotmail.co.uk",
    "test@hotmail.co.uk"
  );
...
test("Change Email Address - and cancel using the cancel button - updated details are ignored", async () => {
  ...
  const contactDetailsPage = await checkYourDetailsNsaPage.changeEmail();
  await contactDetailsPage.editEmailAddress(
    "test@hotmail.co.uk",
    "test@hotmail.co.uk"
  );

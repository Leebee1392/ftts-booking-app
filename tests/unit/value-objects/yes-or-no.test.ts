...
describe("Yes or No processor", () => {
  ...
  test("invalid string", () => {
    expect(() => YesOrNo.from(invalid)).toThrow(
      TypeError("Please choose yes or no.")
    );
  });

  test("invalid object", () => {
    expect(() => YesOrNo.fromBoolean({})).toThrow(
      TypeError("The value provided is not of type boolean.")
    );
  });
});

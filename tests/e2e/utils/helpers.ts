...
function getSelectorObject(selector: any): Selector {
  ...
  if (selector instanceof Selector || selector instanceof Function) {
    ...
    throw new Error(
      `Unknown type for ${selector} - ${selector.constructor.name}`
    );
  }
  return selectorObj;
}
...
export async function verifyActualEqualsExpectedText(
  actualText: string,
  expectedText: string
): Promise<void> {
  await t
    .expect(actualText.replace(REMOVE_WHITESPACE_REGEX, ""))
    .eql(expectedText);
}

export async function verifyExactText(
  selector: string,
  expectedText: string,
  index = 0,
  timeout = 5000
): Promise<void> {
  await t.expect(await getText(selector, index)).eql(expectedText, { timeout });
}

export async function verifyTitleContainsText(
  expectedText: string,
  timeout = 30000
): Promise<void> {
  ...
  await t
    .expect(title.exists)
    .ok(
      `Expected title to contain '${expectedText}' but was '${await Selector(
        "title"
      ).innerText}'`,
      { timeout }
    );
...
export async function verifyContainsText(
  ...
  await t
    .expect(await getText(selector, index))
    .contains(expectedText, { timeout });
}

export async function verifyNotContainsText(
  selector: string,
  expectedText: string,
  index = 0
): Promise<void> {
  await t.expect(await getText(selector, index)).notContains(expectedText);
}
...
export async function verifyNoValue(
  selector: string,
  expectedText: string,
  index = 0
): Promise<void> {
...
export async function verifyIsVisible(
  ...
  await t
    .expect(await isVisible(selector, index, timeout))
    .eql(true, errorMessage, { timeout });
}

export async function verifyIsNotVisible(
  selector: string,
  index = 0,
  timeout = 5000
): Promise<void> {
...
export async function isVisible(
  ...
  const selectorObj = getSelectorObject(selector);
  return (
    (await Selector(selectorObj.nth(index), { timeout })().filterVisible()
      .count) !== 0
  );
}
...
export async function enter(selector: string, value: string): Promise<void> {
  ...
  await t
    .expect(selectorObj.visible)
    .ok(errorMessage)
    .selectText(selectorObj)
    .typeText(selectorObj, value, { paste: true, replace: true });
}

export const fillInFields = ClientFunction(
  (cardNumber, expiryDateMonth, expiryDateYear, cscCode) => {
...
export async function click(selector: string, index = 0): Promise<void> {
  ...
  await t
    .expect(selectorObj.visible)
    .ok(errorMessage)
    .expect(selectorObj.nth(index).visible)
    .ok(errorMessage)
...
export async function clickWithText(
  ...
  const selectorObj = getSelectorObject(selector);
  const errorMessage = getErrorMessage(
    `expected element to be visible with text ${text}`,
    selectorObj
  );

  await t
    .expect(selectorObj.withText(text).visible)
    .ok(errorMessage)
...
export async function link(value: string): Promise<void> {
  ...
  const errorMessage = getErrorMessage(
    `expected link with value ${value} to be visible`,
    selectorObj
  );

  await t
    .expect(selectorObj.withExactText(value).visible)
    .ok(errorMessage)
    .click(selectorObj.withExactText(value));
}

export async function clickLinkContainsURL(
  linkText: string,
  expectedUrlText: string
): Promise<void> {
  const linkValue = decodeURIComponent(
    await Selector("a").withText(linkText).getAttribute("href")
  );
...
export async function setCookie(
  ...
  if (cookie) {
    ...
    if (cookie.includes("ftts=")) {
      signature = cookie.substring(
        cookie.indexOf(".") + 1,
        cookie.indexOf(";")
      );
      const cookieStrDecoded = decodeURIComponent(cookie);
      sessionId = cookieStrDecoded.substring(
        cookieStrDecoded.indexOf("ftts=s:") + 7,
        cookieStrDecoded.indexOf(".")
      );
    ...
    await RedisClient.writeCookieDataToRedis(
      parsedCookieData.sessionId,
      sessionData
    );
  }
}

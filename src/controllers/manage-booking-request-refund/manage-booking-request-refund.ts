...
} from "../../helpers/start-page-navigator";
...
export class ManageBookingRequestRefundController {
  ...
  public get = (req: Request, res: Response): void =>
    this.renderRequestRefundLink(req, res);

  public post = async (req: Request, res: Response): Promise<void> => {
    if (!req.session.journey?.inManageBookingMode) {
      throw Error(
        "ManageBookingRequestRefundController::post: No session journey inManageBookingMode set"
      );
    }
    if (!req.session.manageBooking) {
      throw Error(
        "ManageBookingRequestRefundController::post: No session manageBooking set"
      );
    }
    if (!req.session.manageBooking.candidate) {
      throw Error(
        "ManageBookingRequestRefundController::post: No session candidate is set"
      );
    }
    ...
    if (!booking) {
      throw Error(
        "ManageBookingRequestRefundController::post: No session booking is set"
      );
    }
    ...
    try {
      ...
      if (candidate.email) {
        await this.sendRefundRequestNotification(
          booking,
          candidate,
          target as Target,
          locale as Locale
        ...
        logger.info(
          "ManageBookingRequestRefundController::post: Candidate does not have an email set",
          {
      ...
      logger.error(
        error as Error,
        "ManageBookingRequestRefundController::post: Couldn't request a refund",
        {
          bookingId: booking.details.bookingId,
          bookingRef: booking.details.reference,
          bookingProductId: booking.details.bookingProductId,
  ...
  private async requestRefund(
    ...
    logger.info(
      "ManageBookingRequestRefundController::requestRefund: Refund is being requested",
      {
        bookingId: booking.details.bookingId,
        bookingRef: booking.details.reference,
        bookingProductId: booking.details.bookingProductId,
    ...
    if (
      ...
      throw Error(
        "ManageBookingRequestRefundController::requestRefund: Missing candidate id, person reference or address"
      );
    }

    if (!booking.isCompensationTestEligible() || booking.isZeroCost()) {
      throw Error(
        "ManageBookingRequestRefundController::requestRefund: Booking is not refundable"
      );
    }
    ...
    await this.crm.updateCompensationBooking(
  ...
  private async sendRefundRequestNotification(
    ...
    const emailContent = buildRefundRequestEmailContent(
      emailDetails,
      target,
      locale
    );
    try {
      logger.info(
        "ManageBookingRequestRefundController::sendRefundRequestNotification: Attempting to send refund request email",
        {
          bookingId: booking.details.bookingId,
          bookingRef,
          bookingProductId: booking.details.bookingProductId,
      ...
      logger.warn(
        "ManageBookingRequestRefundController::sendRefundRequestNotification: Cannot send refund request confirmation email",
        {
          bookingId: booking.details.bookingId,
          bookingRef,
          bookingProductId: booking.details.bookingProductId,
  ...
  private renderRequestRefundLink(req: Request, res: Response): void {
    ...
    if (!ref) {
      logger.warn(
        "ManageBookingRequestRefundController::renderRequestRefundLink: Booking ref is empty in query params"
      );
      res.redirect("/manage-booking/home");
      return;
    }

    const booking = store.manageBooking.getBooking(
      req,
      ref as string
    ) as Booking;
    if (
      ...
      logger.warn(
        "ManageBookingRequestRefundController::renderRequestRefundLink: booking was not found as a compensation test",
        {
          ref,
        }
      );
    ...
    const bookTheoryTestLink = isInstructorBooking
      ? getInstructorBackLinkToStartPage(req)
      : getBackLinkToStartPage(req);
  ...
  private async renderRefundConfirmationLink(
    booking: Booking,
    candidate: Candidate,
    req: Request,
    res: Response

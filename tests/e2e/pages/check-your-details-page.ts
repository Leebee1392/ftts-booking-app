...
export class CheckYourDetailsPage extends BasePage {
  ...
  paymentAlertMessage =
    "Complete the payment process within 30 minutes. If you do not, this booking will be cancelled but you may still be charged. You will have to start your booking again and the test slot you were trying to book may no longer be available.";
  ...
  async checkDataMatchesSession(sessionData: SessionData): Promise<void> {
    ...
    if (
      ...
      (sessionData.target === Target.NI && !sessionData.journey.isInstructor)
    ) {
      if (sessionData.isCompensationBooking) {
        const priceDetailsText = (
          await getText(this.testDetailsValue, OFFSET_INDEX)
        ).toUpperCase();
        await t
          .expect(priceDetailsText)
          .contains(
            `${TestTypeToPrice.get(sessionData.currentBooking.testType)}`
          );
    ...
    if (
      sessionData.isCompensationBooking ||
      (sessionData.journey.isInstructor && sessionData.target === Target.NI)
    ) {
      await verifyIsNotVisible(this.paymentAlertMessageLocator);
    } else {
      await verifyContainsText(
        this.paymentAlertMessageLocator,
        this.paymentAlertMessage
      );
    }
  }
}

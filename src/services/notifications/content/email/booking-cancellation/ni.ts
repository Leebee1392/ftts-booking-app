...
export default {
  ...
  buildBody: (details: BookingCancellationDetails): string => {
    ...
    if (INSTRUCTOR_TEST_TYPES.includes(details.testType)) {
      bookTheoryTestLink = translate(
        "generalContent.footer.instructorBookingLinkUrl"
      );
      prepareForTestUrl =
    ...
    return dedent`
    # Your ${translate(
      `generalContent.testTypes.${details.testType}`
    )} driving theory test is cancelled

    # Booking reference ${details.bookingRef}

    We've cancelled the driving theory test you were due to take at ${asLocalTime(
      details.testDateTime
    )} on ${asFullDateWithWeekday(details.testDateTime)}

    ---
    If you did not cancel this test please contact us immediately. You can reply to this email or telephone ${translate(

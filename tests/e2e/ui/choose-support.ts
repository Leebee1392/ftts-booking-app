...
test("Verify page title and heading are displayed correctly", async () => {
  await verifyTitleContainsText(
    `${chooseSupportPage.pageHeading} ${generalTitle}`
  );
...
test("Verify error prefix appears in the title when there is an error", async () => {
  ...
  await verifyTitleContainsText(
    `${chooseSupportPage.pageHeading} ${Constants.generalTitle}`
  );
});
...
test("Verify the user is unable to proceed without selecting an option and an error message is displayed, clicking on the link takes user to the field", async () => {
  await click(chooseSupportPage.continueButton);
  await verifyExactText(
    chooseSupportPage.errorMessageLocator,
    chooseSupportPage.errorMessageHeader
  );
  await verifyExactText(
    chooseSupportPage.errorMessageList,
    chooseSupportPage.errorMessageText
  );
  await verifyContainsText(
    chooseSupportPage.errorMessageRadioLocator,
    chooseSupportPage.errorMessageText
  );

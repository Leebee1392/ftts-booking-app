...
describe("CRMSupportTypeBuilder", () => {
  ...
  describe("preferredCommunicationMethod", () => {
    test("preferred communication method set to phone is telephone is provided", () => {
      const result = CRMSupportHelper.preferredCommunicationMethod(
  ...
  describe("isPreferredDayOrLocation", () => {
    test("returns preferred text if preferred day is set to a particular day", () => {
      const result = CRMSupportHelper.getPreferredDayOrLocation(
        req.session.currentBooking.preferredDayOption,
        req.session.currentBooking.preferredDay
      );
    ...
    test("returns preferred text if preferred location is set to a particular location", () => {
      ...
      const result = CRMSupportHelper.getPreferredDayOrLocation(
        req.session.currentBooking.preferredLocationOption,
        req.session.currentBooking.preferredLocation
      );
  ...
  describe("isTranslator", () => {
    test("returns translator option is translator type is in support types", () => {
      ...
      const result = CRMSupportHelper.getTranslator(
        req.session.currentBooking.selectSupportType,
        req.session.currentBooking.translator
      );
  ...
  describe("isVoicemail", () => {
    test("returns voicemail option if telephone is supplied", () => {
      const result = CRMSupportHelper.isVoicemail(
        req.session.candidate.telephone,
        req.session.currentBooking.voicemail
      );
  ...
  describe("toString", () => {
    test("structures support types and custom support properly", () => {
      ...
      const supportTypeBuilder = new CRMSupportHelper(
        req.session.currentBooking.selectSupportType,
        req.session.currentBooking.customSupport
      );

      expect(supportTypeBuilder.toString()).toBe(
        `- ${CRMSupportHelper.supportTypeMap.get(
          req.session.currentBooking.selectSupportType[0]
        )}\n- ${CRMSupportHelper.supportTypeMap.get(
          req.session.currentBooking.selectSupportType[1]
        )}\n- ${CRMSupportHelper.supportTypeMap.get(
          req.session.currentBooking.selectSupportType[2]
        )}\n- ${CRMSupportHelper.supportTypeMap.get(
          req.session.currentBooking.selectSupportType[3]
        )}\n\n${req.session.currentBooking.customSupport}`
      );
    });
    ...
    test("structures translator type properly", () => {
      ...
      expect(supportTypeBuilder.toString(language)).toBe(
        `- ${CRMSupportHelper.supportTypeMap.get(
          SupportType.TRANSLATOR
        )} (${language})\n\n`
      );

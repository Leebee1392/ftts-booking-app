import changeLocationTime from '@controllers/instructor-change-location-time/instructor-change-location-time';
...
describe('InstructorChangeLocationTime', () => {
  ...
  describe('get', () => {
    test('renders the change location time template', () => {
      ...
      expect(res.render).toHaveBeenCalledWith('instructor/change-location-time', {
        options: radioItems,
  ...
  describe('POST', () => {
    ...
    test('renders an error on the same template when validation fails', () => {
      ...
      expect(res.render).toHaveBeenCalledWith('instructor/change-location-time', expect.objectContaining({
        errors: req.errors,
      }));
    });

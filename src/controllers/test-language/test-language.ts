...
export class TestLanguageController {
  public get = (req: Request, res: Response): void => {
    ...
    if (req.session.journey.isInstructor) {
      if (
        !TestLanguage.canChangeTestLanguage(
  ...
  public post = (req: Request, res: Response): void => {
    ...
    const canChangeLanguage = TestLanguage.canChangeTestLanguage(
      req.session.target || Target.GB,
      testType
    );
    ...
    if (!canChangeLanguage) {
      throw Error(
        `TestLanguageController::post: Cannot change test language for test type: ${testType} and target ${req.session.target}`
      );
    }

    const { testLanguage } = req.body as SelectLanguageBody;
    const language =
      testLanguage === Language.ENGLISH ? Language.ENGLISH : Language.WELSH;
  ...
  private selectedLanguage(req: Request): TestLanguage | undefined {
    if (!req.session.currentBooking) {
      throw Error(
        "TestLanguageController::selectedLanguage: No currentBooking set"
      );
    }

...
export class AppointmentService {
  ...
  public async getSlots(
    ...
    const dateFrom = this.calculateDateFrom(
      selectedDate,
      testEligibility?.eligibleFrom
    );
    const dateTo = this.calculateDateTo(
      selectedDate,
      testEligibility?.eligibleTo
    );
    if (dateFrom > dateTo) {
      logger.debug("AppointmentService::getSlots:dateFrom>dateTo");
      return { slotsByDate: {} };
    }
    const slots = await this.scheduling.availableSlots(
      dateFrom,
      dateTo,
      selectedCentre,
      testType,
  ...
  public getPreviousDateMobile(selectedDate: Dayjs): string {
    const startOfWeekDiff = selectedDate.diff(
  ...
  private calculateDateFrom(
    ...
    if (eligibleFromDate) {
      earliestAvailableDate = dayjs.max(
        earliestAvailableDate,
        dayjs(eligibleFromDate).tz()
      );
    }
    return toISODateString(earliestAvailableDate);
  }

  private calculateDateTo(
    selectedDate: Dayjs,
    eligibleToDate?: string
  ): string {
    const endOfWeek = selectedDate.endOf("isoWeek");
    const today = dayjs().tz().startOf("day");
    ...
    if (eligibleToDate) {
      latestAvailableDate = dayjs.min(
        latestAvailableDate,
        dayjs(eligibleToDate).tz()
      );
    }
  ...
  private groupSlotsByDate(
    ...
    while (!currentDate.isAfter(dayjs(dateTo).tz(), "date")) {
      ...
      const slotsOnDate = slots.filter(
        (slot) => toISODateString(slot.startDateTime) === currentDateIsoString
      );

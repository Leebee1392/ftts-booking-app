...
describe("crm-helper.ts", () => {
  ...
  describe("mapToCRMLicence", () => {
    ...
    test("correctly maps data to a CRM Licence", () => {
      expect(
        mapToCRMLicence(candidateId, licenceNumber, mockCandidate)
      ).toStrictEqual({
    ...
    test("correctly maps data if no licence is given", () => {
      expect(
        mapToCRMLicence(candidateId, undefined, mockCandidate)
      ).toStrictEqual({
        "ftts_Person@odata.bind": `contacts(${candidateId})`,
        "ownerid@odata.bind": `teams(${dvsaTeam})`,
        ftts_address1_street1: convertedAddress.address1_line1,
    ...
    test("correctly maps data if no candidate given", () => {
      expect(
        mapToCRMLicence(candidateId, licenceNumber, undefined)
      ).toStrictEqual({
        "ftts_Person@odata.bind": `contacts(${candidateId})`,
  ...
  describe("mapToCRMBooking", () => {
    test("correctly maps data to a CRM Booking", () => {
      expect(
        ...
        "ftts_selectedtestcentrelocation@odata.bind":
          "accounts(first-selected-test-centre-id)",
    ...
    test("correctly maps support requirements to a CRM Booking when isNonStandardAccommodation", () => {
      ...
      expect(payload.ftts_supportrequirements).toBe(
  ...
  describe("mapCRMProductToProductResponse", () => {
    ...
    test("should throw an error if no product was found", () => {
      expect(() => {
        mapCRMProductToProductResponse([]);
      }).toThrow("No Product Found in CRM");
    });
  });
  ...
  describe("mapCRMBookingResponseToBookingResponse", () => {
    test("should map a CRMBookingResponse to a BookingResponse", () => {
      ...
      expect(
        mapCRMBookingResponseToBookingResponse(mockCRMBookingResponse)
      ).toStrictEqual({
  ...
  describe("mapFromCrmXmlBookingDetailsToCRMBookingDetails", () => {
    ...
    test("should successfully map from CRM xml booking details to CRM booking details", () => {
      ...
      const result = mapFromCrmXmlBookingDetailsToCRMBookingDetails(
        mockCRMXmlBookingDetails
      );
    ...
    test("should successfully map nsa bookings from CRM xml booking details to CRM booking details", () => {
      ...
      mockCRMXmlBookingDetails["booking.ftts_voicemailmessagespermitted"] =
        undefined;
      ...
      mockCRMXmlBookingDetails["booking.ftts_nsastatus"] =
        CRMNsaStatus.AwaitingCscResponse;
      mockCRMXmlBookingDetails["booking.ftts_nivoiceoveroptions"] =
  ...
  describe("mapToBookingDetails", () => {
    ...
    test("should map CRMBookingDetails to BookingDetails", () => {
      expect(mapToBookingDetails(mockCRMBookingDetails)).toStrictEqual({
        ...
        compensationAssigned:
          mockCRMBookingDetails.ftts_bookingid.ftts_owedcompbookingassigned,
        compensationRecognised:
          mockCRMBookingDetails.ftts_bookingid.ftts_owedcompbookingrecognised,
        owedCompensationBooking: false,
        governmentAgency:
          mockCRMBookingDetails.ftts_bookingid.ftts_governmentagency,
        ...
        enableEligibilityBypass:
          mockCRMBookingDetails.ftts_bookingid.ftts_enableeligibilitybypass,
        nonStandardAccommodation:
          mockCRMBookingDetails.ftts_bookingid.ftts_nonstandardaccommodation,
        voicemailmessagespermitted: true,
        nsaStatus: mockCRMBookingDetails.ftts_bookingid.ftts_nsastatus,
        isZeroCostBooking:
          mockCRMBookingDetails.ftts_bookingid.ftts_zerocostbooking,
        ...
        testCentre: {
          accountId:
            mockCRMBookingDetails.ftts_bookingid.ftts_testcentre?.accountid,
          siteId:
            mockCRMBookingDetails.ftts_bookingid.ftts_testcentre?.ftts_siteid,
          name: mockCRMBookingDetails.ftts_bookingid.ftts_testcentre?.name,
          addressLine1:
            mockCRMBookingDetails.ftts_bookingid.ftts_testcentre
              ?.address1_line1,
          addressLine2:
            mockCRMBookingDetails.ftts_bookingid.ftts_testcentre
              ?.address1_line2,
          addressCity:
            mockCRMBookingDetails.ftts_bookingid.ftts_testcentre?.address1_city,
          addressCounty:
            mockCRMBookingDetails.ftts_bookingid.ftts_testcentre
              ?.address1_county,
          addressPostalCode:
            mockCRMBookingDetails.ftts_bookingid.ftts_testcentre
              ?.address1_postalcode,
          remit:
            mockCRMBookingDetails.ftts_bookingid.ftts_testcentre?.ftts_remit,
          testCentreId:
            mockCRMBookingDetails.ftts_bookingid.ftts_testcentre
        ...
        foreignlanguageselected:
          mockCRMBookingDetails.ftts_bookingid.ftts_foreignlanguageselected,
        product: {
    ...
    test("if there are any missing fields, throws an error and logs", () => {
      ...
      expect(() => mapToBookingDetails(mockCRMBookingDetails)).toThrow(
        new Error(
    ...
    describe("draft nsa bookings", () => {
      ...
      test("successfully maps voiceover language to nivoiceoverlanguage and maps nsa booking slots if it is an NSA draft booking", () => {
        expect(mapToBookingDetails(mockCRMBookingDetails)).toEqual(
          expect.objectContaining({
            voiceoverLanguage:
              mockCRMBookingDetails.ftts_bookingid.ftts_nivoiceoveroptions,
            nsaBookingSlots: mockCRMBookingDetails.ftts_nsabookingslots,
            testSupportNeed: JSON.parse(
              `[ ${mockCRMBookingDetails.ftts_bookingid.ftts_testsupportneed} ]`
            ),
          })
        );
      });
  ...
  describe("getMissingFields", () => {
    ...
    `(
      'given booking status is "$bookingStatus" it should throw an error when "$missingField" is undefined',
      ({ bookingStatus, missingField }) => {
    ...
    `(
      'given booking status is "$bookingStatus" it should throw an error when "$missingField" in bookingid entity is undefined',
      ({ bookingStatus, missingField }) => {
        const isNsaBooking = bookingStatus === CRMBookingStatus.Draft;
        mockCRMBookingDetails = crmBookingDetailsBuilder(
          isNsaBooking,
  ...
  describe("mapCRMRegionToTCNRegion", () => {
    ...
    test("should throw an error if all the region booleans are false", () => {
      expect(() => mapCRMRegionToTCNRegion({} as CRMTestCentre)).toThrow(
        new Error(
  ...
  describe("mapCRMRemitToCRMCalendarName", () => {
    test("Maps English Test Centres to an English Calendar Name", () => {
      expect(mapCRMRemitToCRMCalendarName(CRMRemit.England)).toEqual(
        CRMCalendarName.England
      );
    });

    test("Maps Scotland Test Centres to an Scotland Calendar Name", () => {
      expect(mapCRMRemitToCRMCalendarName(CRMRemit.Scotland)).toEqual(
        CRMCalendarName.Scotland
      );
    });

    test("Maps English Test Centres to an Wales Calendar Name", () => {
      expect(mapCRMRemitToCRMCalendarName(CRMRemit.Wales)).toEqual(
        CRMCalendarName.Wales
      );
    });

    test("Maps NI Test Centres to an NI Calendar Name", () => {
      expect(mapCRMRemitToCRMCalendarName(CRMRemit.NorthernIreland)).toEqual(
        CRMCalendarName.NorthernIreland
      );
    });
  });
  ...
  describe("mapPaymentInformationToBookingDetails", () => {
    test("Successfully maps payment information into a booking detail object", () => {
      ...
      expect(
        mapPaymentInformationToBookingDetails(paymentInfo, bookingDetails)
      ).toStrictEqual({
        financeTransaction: {
  ...
  describe("mapVoiceoverToCRMVoiceover", () => {
    test("Correctly maps Arabic Voiceover", () => {
      expect(mapVoiceoverToCRMVoiceover(Voiceover.ARABIC)).toEqual(
        CRMVoiceOver.Arabic
      );
    });

    test("Correctly maps Cantonese Voiceover", () => {
      expect(mapVoiceoverToCRMVoiceover(Voiceover.CANTONESE)).toEqual(
        CRMVoiceOver.Cantonese
      );
    });

    test("Correctly maps English Voiceover", () => {
      expect(mapVoiceoverToCRMVoiceover(Voiceover.ENGLISH)).toEqual(
        CRMVoiceOver.English
      );
    });

    test("Correctly maps Farsi Voiceover", () => {
      expect(mapVoiceoverToCRMVoiceover(Voiceover.FARSI)).toEqual(
        CRMVoiceOver.Farsi
      );
    });

    test("Correctly maps No Voiceover", () => {
      expect(mapVoiceoverToCRMVoiceover(Voiceover.NONE)).toEqual(
        CRMVoiceOver.None
      );
    });
    ...
    test("Correctly maps Polish Voiceover", () => {
      expect(mapVoiceoverToCRMVoiceover(Voiceover.POLISH)).toEqual(
        CRMVoiceOver.Polish
      );
    });

    test("Correctly maps Portuguese Voiceover", () => {
      expect(mapVoiceoverToCRMVoiceover(Voiceover.PORTUGUESE)).toEqual(
        CRMVoiceOver.Portuguese
      );
    });

    test("Correctly maps Turkish Voiceover", () => {
      expect(mapVoiceoverToCRMVoiceover(Voiceover.TURKISH)).toEqual(
        CRMVoiceOver.Turkish
      );
    });

    test("Correctly maps Welsh Voiceover", () => {
      expect(mapVoiceoverToCRMVoiceover(Voiceover.WELSH)).toEqual(
        CRMVoiceOver.Welsh
      );
    });
  });
  ...
  describe("mapPeopleTitleStringToCRMPeopleTitle", () => {
    test("correctly maps Mr title", () => {
      expect(mapPeopleTitleStringToCRMPeopleTitle("Mr")).toEqual(
        CRMPeopleTitle.MR
      );
    });

    test("correctly maps Ms title", () => {
      expect(mapPeopleTitleStringToCRMPeopleTitle("Ms")).toEqual(
        CRMPeopleTitle.MS
      );
    });

    test("correctly maps Mrs title", () => {
      expect(mapPeopleTitleStringToCRMPeopleTitle("Mrs")).toEqual(
        CRMPeopleTitle.MRS
      );
    });

    test("correctly maps Miss title", () => {
      expect(mapPeopleTitleStringToCRMPeopleTitle("Miss")).toEqual(
        CRMPeopleTitle.MISS
      );
    });

    test("correctly maps Mx title", () => {
      expect(mapPeopleTitleStringToCRMPeopleTitle("Mx")).toEqual(
        CRMPeopleTitle.MX
      );
    });

    test("correctly maps Dr title", () => {
      expect(mapPeopleTitleStringToCRMPeopleTitle("Dr")).toEqual(
        CRMPeopleTitle.DR
      );
    });

    test("correct maps 'doctor'", () => {
      expect(mapPeopleTitleStringToCRMPeopleTitle("Doctor")).toEqual(
        CRMPeopleTitle.DR
      );
    });
  ...
  describe("mapToSupportType", () => {
    test.each([
      ...
      [
        [CRMTestSupportNeed.ExtraTimeWithBreak],
        [TestSupportNeed.ExtraTimeWithBreak],
      ],
      [
        [CRMTestSupportNeed.ForeignLanguageInterpreter],
        [TestSupportNeed.ForeignLanguageInterpreter],
      ],
      [[CRMTestSupportNeed.HomeTest], [TestSupportNeed.HomeTest]],
      [[CRMTestSupportNeed.LipSpeaker], [TestSupportNeed.LipSpeaker]],
      [
        [CRMTestSupportNeed.NonStandardAccommodationRequest],
        [TestSupportNeed.NonStandardAccommodationRequest],
      ],
      [
        [CRMTestSupportNeed.OralLanguageModifier],
        [TestSupportNeed.OralLanguageModifier],
      ],
      [[CRMTestSupportNeed.OtherSigner], [TestSupportNeed.OtherSigner]],
      [[CRMTestSupportNeed.Reader], [TestSupportNeed.Reader]],
      [
        [CRMTestSupportNeed.FamiliarReaderToCandidate],
        [TestSupportNeed.FamiliarReaderToCandidate],
      ],
      ...
      [
        ...
        [TestSupportNeed.SpecialTestingEquipment],
      ],
      ...
      (
        supportTypeRequested: CRMTestSupportNeed[] | null | undefined,
        expectedResult: TestSupportNeed[]
      ) => {
        expect(
          mapToSupportType(
            supportTypeRequested as unknown as CRMTestSupportNeed[]
          )
  ...
  describe("mapGenderEnumToCRMGenderCode", () => {
    test("correctly maps Male gender", () => {
      expect(
        mapGenderEnumToCRMGenderCode(ELIG.CandidateDetails.GenderEnum.M)
      ).toEqual(CRMGenderCode.Male);
    });

    test("correctly maps Female gender", () => {
      expect(
        mapGenderEnumToCRMGenderCode(ELIG.CandidateDetails.GenderEnum.F)
      ).toEqual(CRMGenderCode.Female);
    });

    test("correctly maps Unknown gender", () => {
      expect(
        mapGenderEnumToCRMGenderCode(ELIG.CandidateDetails.GenderEnum.U)
      ).toEqual(CRMGenderCode.Unknown);
    });
  ...
  describe("mapCRMPeopleTitleToString", () => {
    test.each([
      ...
      "for %s returns %s",
      (
        input: CRMPeopleTitle | undefined,
  ...
  describe("mapCRMGenderCodeToGenderEnum", () => {
    test.each([
      ...
      "for %s returns %s",
      (
        input: CRMGenderCode | undefined,
        expectedOutput: ELIG.CandidateDetails.GenderEnum | undefined
      ) => {
  ...
  describe("hasCRMSupportNeeds", () => {
    test("returns true if the user has a support need in the crm with approved status", () => {
      const mockedCandidate = {
        ...mockCandidate,
        supportNeedName: "BSL- Interpreter",
        supportEvidenceStatus: "675030000",
      };

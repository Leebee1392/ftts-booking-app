...
const createAzureCookie = (cookieString: string): Cookie => {
  ...
  const sameSite = getValueFromParsedCookie(
    parsedCookie,
    "samesite"
  ) as SameSiteHeader;
...
const getValueFromParsedCookie = (
  parsedCookie: ParsedCookie,
  searchTerm: string
): string | undefined => {
  const validKey = Object.keys(parsedCookie).find(
    (key) => key.toLowerCase() === searchTerm.toLowerCase()
  );
  return validKey ? parsedCookie[`${validKey}`] : undefined;
};

export { setCookieHeader, createAzureCookie };

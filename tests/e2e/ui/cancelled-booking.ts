...
test("Verify UI contents of the Cancelled theory test booking page", async (t) => {
  ...
  await verifyContainsText(
    cancelledBookingPage.refNumberLocator,
    cancelledBookingPage.refNumberText
  );
  await verifyContainsText(cancelledBookingPage.refNumberLocator, bookingRef);
  await verifyExactText(
    cancelledBookingPage.messageLocator,
    cancelledBookingPage.messageText
  );
  ...
  await verifyExactText(
    cancelledBookingPage.changeAnotherBookingButtonLocator,
    cancelledBookingPage.changeAnotherBookingButtonText
  );

  // field names
  await verifyExactText(cancelledBookingPage.testDetailsKey, "Test type", 0);
  await verifyExactText(
    cancelledBookingPage.testDetailsKey,
    "Test time and date",
    1
  );
  await verifyExactText(
  ...
  // field values
  await verifyExactText(
    cancelledBookingPage.testDetailsValue,
  ...
  await verifyContainsText(
    changeBookingPage.testDetailsValue,
...
if (!runningTestsLocally()) {
  // only run against an environment due to data constraints

import { InstructorVoicemailController } from '@controllers/instructor-voicemail/instructor-voicemail';
...
describe('InstructorVoicemailController', () => {
  let voicemailController: InstructorVoicemailController;
  let req;
  let res;

  beforeEach(() => {
    voicemailController = new InstructorVoicemailController();
    req = {
      body: {},
      session: {
        currentBooking: {},
  ...
  describe('get', () => {
    test('it should render the correct page', () => {
      ...
      expect(res.render).toHaveBeenCalledWith('instructor/voicemail', expect.any(Object));
    });
    ...
    test('checks voicemail \'yes\' option if voicemail set to true in session', () => {
      ...
      expect(res.render).toHaveBeenCalledWith('instructor/voicemail', expect.objectContaining({
        voicemailYes: true,
        voicemailNo: false,
      }));
    });

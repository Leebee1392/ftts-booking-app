...
): Partial<Omit<CRMContact, "contactid">> {
  return {
...
export function transformOtherTitle(
  title: string | undefined
): string | undefined {
  return title === "OTHER" ? "Other" : title;
}

export function mapToCRMLicence(
  candidateId: string,
  ...
  const contactToLicenceAddress: Partial<CRMLicence> = {
    ...
    "ownerid@odata.bind": candidateDetails?.ownerId
      ? `teams(${candidateDetails?.ownerId})`
      : undefined,
  };
...
export function mapToCRMBooking(
  candidate: Candidate,
  booking: Booking,
  candidateId: string,
  licenceId: string,
  ...
  if (
    ...
    throw new Error(
      "CRMHelper::mapToCRMBooking: Missing required candidate data"
    );
  }
  if (!booking.dateTime || !booking.centre?.accountId || !booking.testType) {
    if (isStandardAccommodation) {
      throw new Error(
        "CRMHelper::mapToCRMBooking: Missing required booking data"
      );
    }
  }
  return {
    ...
    ftts_pricepaid: booking.compensationBooking
      ...
      : (booking.priceList?.price as number),
    ...
    "ftts_testcentre@odata.bind": isStandardAccommodation
      ? `accounts(${booking.centre?.accountId})`
      : undefined,
    ...
    ftts_language:
      booking.language?.toLowerCase() === Language.WELSH.toLowerCase()
    ...
    "ftts_selectedtestcentrelocation@odata.bind": isStandardAccommodation
      ? `accounts(${
          booking.firstSelectedCentre?.accountId || booking.centre?.accountId
        })`
      : undefined,
    ftts_governmentagency:
      booking.governmentAgency === Target.GB
    ...
    ftts_dateavailableonorbeforepreferreddate:
      booking.dateAvailableOnOrBeforePreferredDate,
    ftts_dateavailableonorafterpreferreddate:
      booking.dateAvailableOnOrAfterPreferredDate,
    ftts_testsselected: true,
    "ownerid@odata.bind": `teams(${config.crm.ownerId.dvsa})`,
  };
}

function mapNsaAttributesToBooking(
  candidate: Candidate,
  booking: Booking,
  isStandardAccommodation: boolean
): NsaBookingFields {
  return {
    ...
    ftts_supportrequirements: isStandardAccommodation
      ? undefined
      : new CRMSupportHelper(
          booking.selectSupportType as SupportType[],
          booking.customSupport as SupportType
        ).toString(booking.translator),
    ftts_preferreddateandtime: isStandardAccommodation
      ? undefined
      : CRMSupportHelper.getPreferredDayOrLocation(
          booking.preferredDayOption,
          booking.preferredDay as string
        ),
    ...
    ftts_preferredtestcentrelocation: isStandardAccommodation
      ...
      : CRMSupportHelper.getPreferredDayOrLocation(
          ...
          booking.preferredLocation as string
        ),
    ftts_preferredcommunicationmethod: isStandardAccommodation
      ? CRMPreferredCommunicationMethod.Email
      : CRMSupportHelper.preferredCommunicationMethod(candidate.telephone), // If email is provided then put 'Email' here. If telephone contact is yes, then put 'Phone' here.
    ftts_proxypermitted: false,
    ftts_voicemailmessagespermitted: isStandardAccommodation
      ? undefined
      : CRMSupportHelper.isVoicemail(
          candidate.telephone,
          booking.voicemail as boolean
        ),
    "ownerid@odata.bind": `teams(${config.crm.ownerId.dvsa})`,
  };
}

export function mapFromCrmXmlBookingDetailsToCRMBookingDetails(
  crmXmlBookingDetails: CRMXmlBookingDetails
): CRMBookingDetails {
  const mappedCRMBookingDetail: CRMBookingDetails = {
    ...
    ftts_additionalsupportoptions:
      crmXmlBookingDetails.ftts_additionalsupportoptions ?? null,
    ftts_bookingid: {
      ftts_governmentagency:
        crmXmlBookingDetails["booking.ftts_governmentagency"] ?? null,
      ...
      ftts_enableeligibilitybypass:
        crmXmlBookingDetails["booking.ftts_enableeligibilitybypass"] ?? null,
      ftts_nonstandardaccommodation:
        crmXmlBookingDetails["booking.ftts_nonstandardaccommodation"] ?? null,
      ftts_nsastatus: crmXmlBookingDetails["booking.ftts_nsastatus"] ?? null,
      ftts_owedcompbookingassigned:
        crmXmlBookingDetails["booking.ftts_owedcompbookingassigned"] ?? null,
      ftts_owedcompbookingrecognised:
        crmXmlBookingDetails["booking.ftts_owedcompbookingrecognised"] ?? null,
      ftts_zerocostbooking:
        crmXmlBookingDetails["booking.ftts_zerocostbooking"] ?? null,
      ftts_testsupportneed:
        crmXmlBookingDetails["booking.ftts_testsupportneed"] ?? null,
      ftts_foreignlanguageselected:
        crmXmlBookingDetails["booking.ftts_foreignlanguageselected"] ?? null,
      ftts_voicemailmessagespermitted:
        crmXmlBookingDetails["booking.ftts_voicemailmessagespermitted"] ?? null,
      ftts_nivoiceoveroptions:
        crmXmlBookingDetails["booking.ftts_nivoiceoveroptions"] ?? null,
    },
  ...
  if (!crmXmlBookingDetails["account.ftts_tcntestcentreid"]) {
    ...
    mappedCRMBookingDetail.ftts_bookingid.ftts_testcentre = {
      ...
      address1_county: crmXmlBookingDetails[
        "account.address1_county"
      ] as string,
      address1_postalcode: crmXmlBookingDetails[
      ...
      ftts_tcntestcentreid:
        crmXmlBookingDetails["account.ftts_tcntestcentreid"],
      parentaccountid: {
        ftts_regiona: crmXmlBookingDetails[
          "parentaccountid.ftts_regiona"
        ] as boolean,
        ftts_regionb: crmXmlBookingDetails[
          "parentaccountid.ftts_regionb"
        ] as boolean,
        ftts_regionc: crmXmlBookingDetails[
          "parentaccountid.ftts_regionc"
        ] as boolean,
      },
    };
  }
...
export function mapToCRMBookingProduct(
  candidate: Candidate,
  booking: Booking,
...
export function mapCRMProductToProductResponse(
...
export function mapCRMBookingResponseToBookingResponse(
  crmBookingResponse: CRMBookingResponse
): BookingResponse {
  if (!crmBookingResponse.ftts_reference) {
    throw new Error(
      "CRMHelper::mapCRMBookingResponseToBookingResponse: Missing ftts_reference in crm response"
    );
  }
  return {
...
export function mapToBookingDetails(
  crmBookingDetails: CRMBookingDetails
): BookingDetails {
  ...
  if (missingFields.length > 0) {
    ...
    throw new Error(
      "CRMHelper::mapToBookingDetails: Missing expected fields in crm response"
    );
  }

  const owedCompensationBooking =
    crmBookingDetails.ftts_bookingid.ftts_owedcompbookingassigned !== null &&
  ...
  const booking: BookingDetails = {
    ...
    voiceoverLanguage: (isAnNsaDraftBooking
      ...
      : crmBookingDetails.ftts_voiceoverlanguage) as CRMVoiceOver,
    additionalSupport: crmBookingDetails.ftts_additionalsupportoptions,
    nonStandardAccommodation:
      crmBookingDetails.ftts_bookingid.ftts_nonstandardaccommodation,
    ...
    governmentAgency: crmBookingDetails.ftts_bookingid
      .ftts_governmentagency as CRMGovernmentAgency,
    ...
    enableEligibilityBypass:
      crmBookingDetails.ftts_bookingid.ftts_enableeligibilitybypass,
    compensationAssigned:
      crmBookingDetails.ftts_bookingid.ftts_owedcompbookingassigned,
    compensationRecognised:
      crmBookingDetails.ftts_bookingid.ftts_owedcompbookingrecognised,
    isZeroCostBooking: crmBookingDetails.ftts_bookingid.ftts_zerocostbooking,
    owedCompensationBooking,
    testSupportNeed: JSON.parse(
      `[ ${crmBookingDetails.ftts_bookingid.ftts_testsupportneed} ]`
    ),
    foreignlanguageselected:
      crmBookingDetails.ftts_bookingid.ftts_foreignlanguageselected,
    nsaBookingSlots: crmBookingDetails.ftts_nsabookingslots,
    voicemailmessagespermitted:
      crmBookingDetails.ftts_bookingid.ftts_voicemailmessagespermitted,
  };
  ...
  if (crmBookingDetails.ftts_bookingid.ftts_testcentre) {
    booking.testCentre = {
      testCentreId:
        crmBookingDetails.ftts_bookingid.ftts_testcentre.ftts_tcntestcentreid,
      name: crmBookingDetails.ftts_bookingid.ftts_testcentre.name,
      addressLine1:
        crmBookingDetails.ftts_bookingid.ftts_testcentre.address1_line1,
      addressLine2:
        crmBookingDetails.ftts_bookingid.ftts_testcentre.address1_line2,
      addressCity:
        crmBookingDetails.ftts_bookingid.ftts_testcentre.address1_city,
      addressCounty:
        crmBookingDetails.ftts_bookingid.ftts_testcentre.address1_county,
      addressPostalCode:
        crmBookingDetails.ftts_bookingid.ftts_testcentre.address1_postalcode,
      remit: crmBookingDetails.ftts_bookingid.ftts_testcentre
        .ftts_remit as CRMRemit,
      accountId: crmBookingDetails.ftts_bookingid.ftts_testcentre.accountid,
      region: mapCRMRegionToTCNRegion(
        crmBookingDetails.ftts_bookingid.ftts_testcentre
      ),
      siteId: crmBookingDetails.ftts_bookingid.ftts_testcentre
        .ftts_siteid as string,
      ftts_tcntestcentreid: crmBookingDetails.ftts_bookingid.ftts_testcentre
        .ftts_tcntestcentreid as string,
    };
  }
  return booking;
}
...
export function getMissingFields(
    ...
    possibleMissingBookingIdFieldsNsa.forEach((missingField) => {
      const field =
        crmBookingDetails.ftts_bookingid[
          missingField as keyof CRMBookingDetails["ftts_bookingid"]
        ];
  ...
  if (crmBookingDetails.ftts_bookingid.ftts_testcentre) {
    possibleMissingTestCentreFields.forEach((missingField) => {
      const field = (
        crmBookingDetails.ftts_bookingid.ftts_testcentre as CRMTestCentre
      )[
        missingField as keyof CRMBookingDetails["ftts_bookingid"]["ftts_testcentre"]
      ];
      if (!field) {
        actualMissingFields.push(missingField);
      }
...
export function mapCRMRegionToTCNRegion(centre: CRMTestCentre): TCNRegion {
  ...
  throw new Error(
    "Unable to map the test centre to a region, all region values are false/missing"
  );
}

export function mapPaymentInformationToBookingDetails(
  paymentInformation: GetPaymentInformationResponse,
...
export function mapToCandidate(
  response: CRMLicenceCandidateResponse
): Candidate {
  return {
...
export function mapToCandidateResponse(
  response: CRMContact
): Partial<CandidateResponse> {
  return {
...
export function mapCRMRemitToCRMCalendarName(remit: CRMRemit): CRMCalendarName {
  switch (remit) {
    case CRMRemit.England:
      return CRMCalendarName.England;
    case CRMRemit.NorthernIreland:
      return CRMCalendarName.NorthernIreland;
    case CRMRemit.Scotland:
      return CRMCalendarName.Scotland;
    case CRMRemit.Wales:
      return CRMCalendarName.Wales;
    default:
      throw Error(`Invalid value for remit. Value: ${remit}`);
  }
}

export function mapVoiceoverToCRMVoiceover(
  voiceover: Voiceover | undefined
): CRMVoiceOver {
  switch (voiceover) {
    case Voiceover.ARABIC:
      return CRMVoiceOver.Arabic;
    case Voiceover.CANTONESE:
      return CRMVoiceOver.Cantonese;
    case Voiceover.ENGLISH:
      return CRMVoiceOver.English;
    case Voiceover.FARSI:
      return CRMVoiceOver.Farsi;
    case Voiceover.POLISH:
      return CRMVoiceOver.Polish;
    case Voiceover.PORTUGUESE:
      return CRMVoiceOver.Portuguese;
    case Voiceover.TURKISH:
      return CRMVoiceOver.Turkish;
    case Voiceover.WELSH:
      return CRMVoiceOver.Welsh;
    case Voiceover.NONE:
    default:
      return CRMVoiceOver.None;
  }
}

export function mapPeopleTitleStringToCRMPeopleTitle(
  title?: string
): CRMPeopleTitle | undefined {
  switch (title?.toLowerCase().trim()) {
    case "mr":
      return CRMPeopleTitle.MR;
    case "ms":
      return CRMPeopleTitle.MS;
    case "mrs":
      return CRMPeopleTitle.MRS;
    case "miss":
      return CRMPeopleTitle.MISS;
    case "mx":
      return CRMPeopleTitle.MX;
    case "dr":
      return CRMPeopleTitle.DR;
    case "doctor":
      return CRMPeopleTitle.DR;
    default:
      return undefined;
...
export function mapToSupportType(
  supports: CRMTestSupportNeed[] | null
): TestSupportNeed[] {
  ...
  return supports.map((support) => {
    switch (support) {
      case CRMTestSupportNeed.BSLInterpreter:
        return TestSupportNeed.BSLInterpreter;
      case CRMTestSupportNeed.ExtraTime:
        return TestSupportNeed.ExtraTime;
      case CRMTestSupportNeed.ExtraTimeWithBreak:
        return TestSupportNeed.ExtraTimeWithBreak;
      case CRMTestSupportNeed.ForeignLanguageInterpreter:
        return TestSupportNeed.ForeignLanguageInterpreter;
      case CRMTestSupportNeed.HomeTest:
        return TestSupportNeed.HomeTest;
      case CRMTestSupportNeed.LipSpeaker:
        return TestSupportNeed.LipSpeaker;
      case CRMTestSupportNeed.NonStandardAccommodationRequest:
        return TestSupportNeed.NonStandardAccommodationRequest;
      case CRMTestSupportNeed.OralLanguageModifier:
        return TestSupportNeed.OralLanguageModifier;
      case CRMTestSupportNeed.OtherSigner:
        return TestSupportNeed.OtherSigner;
      case CRMTestSupportNeed.Reader:
        return TestSupportNeed.Reader;
      case CRMTestSupportNeed.FamiliarReaderToCandidate:
        return TestSupportNeed.FamiliarReaderToCandidate;
      case CRMTestSupportNeed.Reader_Recorder:
        return TestSupportNeed.Reader_Recorder;
      case CRMTestSupportNeed.SeperateRoom:
        return TestSupportNeed.SeperateRoom;
      case CRMTestSupportNeed.TestInIsolation:
        return TestSupportNeed.TestInIsolation;
      case CRMTestSupportNeed.SpecialTestingEquipment:
        return TestSupportNeed.SpecialTestingEquipment;
      default:
        return TestSupportNeed.NoSupport;
    }
  });
...
export function mapCRMPeopleTitleToString(
  title?: CRMPeopleTitle
): string | undefined {
  switch (title) {
    case CRMPeopleTitle.MR:
      return "mr";
    case CRMPeopleTitle.MS:
      return "ms";
    case CRMPeopleTitle.MRS:
      return "mrs";
    case CRMPeopleTitle.MISS:
      return "miss";
    case CRMPeopleTitle.MX:
      return "mx";
    case CRMPeopleTitle.DR:
      return "dr";
    default:
      return undefined;
  }
...
export function mapGenderEnumToCRMGenderCode(
  gender?: ELIG.CandidateDetails.GenderEnum
): CRMGenderCode | undefined {
  switch (gender) {
    case undefined:
      return undefined;
    case ELIG.CandidateDetails.GenderEnum.M:
      return CRMGenderCode.Male;
    case ELIG.CandidateDetails.GenderEnum.F:
      return CRMGenderCode.Female;
    default:
      return CRMGenderCode.Unknown;
  }
}

export function mapCRMGenderCodeToGenderEnum(
  gender?: CRMGenderCode
): ELIG.CandidateDetails.GenderEnum | undefined {
  switch (gender) {
    case undefined:
      return undefined;
    case CRMGenderCode.Male:
      return ELIG.CandidateDetails.GenderEnum.M;
    case CRMGenderCode.Female:
      return ELIG.CandidateDetails.GenderEnum.F;
    default:
      return ELIG.CandidateDetails.GenderEnum.U;
  }
}

function setNsaStatus(booking: Booking): CRMNsaStatus {
  if (
    !booking.selectSupportType ||
    booking.hasSupportNeedsInCRM === undefined
  ) {
    throw new Error(
      "CRMHelper::setNsaStatus: Missing candidate support data from booking"
    );
  }
  const evidenceRoute = determineEvidenceRoute(
    booking.selectSupportType,
    booking.hasSupportNeedsInCRM
  );

  if (evidenceRoute === EvidencePath.EVIDENCE_REQUIRED) {

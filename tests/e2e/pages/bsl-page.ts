...
export class BslPage extends BasePage {
  ...
  pageHeadingManageBooking =
    "Do you want On-screen British Sign Language used during the test?";
  ...
  async selectBslRequired(yesOrNo: boolean): Promise<VoiceoverPage> {
    const bslSelector = this.bslYesNo.replace(
      "<>",
      YesOrNo.fromBoolean(yesOrNo).toString()
    );

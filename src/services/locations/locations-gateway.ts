...
export class LocationsGateway {
  constructor(
    ...
    private axiosRetryClient = new AxiosRetryClient(
      config.location.retryPolicy
    ).getClient()
  ) {}

  public async fetchCentres(
    term: string,
    region: string,
    ...
    try {
      ...
      logger.debug(
        `LocationsGateway::fetchCentres: Attempting to get test centres using url ${url}`
      );
      const centresResponse = await this.axiosRetryClient.get<CentreResponse>(
        url,
        authHeader
      );
      logger.debug("LocationsGateway::fetchCentres: Locations Response", {
      ...
      logger.error(
        error,
        `LocationsGateway::fetchCentres: Error retrieving test centres from locations api. Term: ${term} Region: ${region}`
      );
      if (
